...
-----------------------------------------------------------------------------*/
/******************************************************************************
 Test for MPI_Send(): ROOT send to ALL with BYTE OFFSETS
      and MPI_Recv()


This is a modified version of the  MPI_Send  INTER-COMMUNICATOR version
of the SEND FROM ROOT TO ALL. The program uses only the CHARACTER data type.
...
The program selects each node, in turn, to be the root.  The ROOT SENDS to
...
/*------------------------------  MPI_Init  ---------------------------------*/
    ...
    sprintf(testname, "MPI_Send_off: character OFFset");
...
*/
    ...
    for (length_count = 0; length_count < MPITEST_num_message_lengths();
         length_count++) {
...
*/
            for (root = 0; root < MPITEST_nump; root++) {
                if (print_node_flag) {
                    sprintf(info_buf,
                            "length %d commsize %d data_type %d root %d",
...
-----------------------------------------------------------------------------*/
                if (MPITEST_me != root) {
                    ...
                    ierr = MPI_Recv(recv_buffer + recv_off, length + 1,
                                    MPITEST_mpi_datatypes[test_type],
                                    root, root, comm, &recv_stat);
                    ...
                    error =
                        MPITEST_buffer_errors_inc(test_type, length, value,
                                                  recv_buffer + recv_off);
                    ...
                    if (error) {
                        sprintf(info_buf,
                                "%d errors in buffer len %d commsize %d data_type %d root %d",
                                error, length, MPITEST_nump, test_type,
                                root);
                        loop_fail++;
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                    } else {
                    ...
                    if (error) {
                        sprintf(info_buf,
                                ...
                                recv_stat.MPI_SOURCE, length, MPITEST_me,
                                cnt_len);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        loop_fail++;
                    }
...
-----------------------------------------------------------------------------*/
                ...
                if (MPITEST_me == root) {
                    ...
                    for (i = 0; i < MPITEST_nump; i++) {
                        ...
                        ierr = MPI_Send(send_buffer + send_off, length,
                                        MPITEST_mpi_datatypes[test_type],
                                        i, root, comm);
