...
(in mpitest_cfg.h) using MPI_Type_hindexed().  The root's rank will then send
...
MPI Calls dependencies for this test:
  ...
  MPI_Type_hindexed(), MPI_Type_struct(), MPI_Type_commit(),
...
int main(int argc, char *argv[])
{
    ...
    int *blklens, *blklens1, num_types, *type_sizes, basic_types;

    MPI_Aint *displs, *displs1, extent;
    ...
    sprintf(testname, "MPI_Type_hindexed_types");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (length_count = 0;
                ...
                ierr =
                    MPI_Type_hindexed(length, blklens1, displs1, oldtype,
                                      &newtype);
                if (ierr != MPI_SUCCESS) {
                    fail++;
