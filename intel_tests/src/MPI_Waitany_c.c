...
-----------------------------------------------------------------------------*/
/******************************************************************************
*                          Functional test for MPI_Waitany
*
*  Waitany references:
...
*      communications completed by a call to MPI_Waitany fail, the function
...
*  the Request Objects are freed.  This test Does a Waitany on messages
*  that have already been Waitanyed on.  The code verifies that a successful
...
int main(int argc, char *argv[])
{
    ...
    MPI_Status one_stat,        /* Single status variable                    */
    ...
    if (MPITEST_me == 0) {
        ...
        for (i = 0; i < 4 * NUMMESG; i++) {
            total_cnt++;
            ierr =
                MPI_Waitany(4 * NUMMESG, send_request, &index, &one_stat);

            sprintf(info_buf, "Waitany on send: Loop  %d   Index %d ", i,
                    index);
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Waitany on send",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
        ...
        /*  At this point all messages that were sent have been Waitany ed on */
 ...
 /*---------------------------------  Irecv  -----------------------------*/
    if (MPITEST_me == 1) {
        ...
        for (i = 0; i < 4 * NUMMESG; i++) {
            ierr =
                MPI_Waitany(4 * NUMMESG, recv_request, &index, &one_stat);

            sprintf(info_buf, "Waitany on receive: Loop %d  Index %d ", i,
                    index);
            MPITEST_message(MPITEST_INFO1, info_buf);
            ...
            if (ierr != MPI_SUCCESS) {
                fail++;
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Waitany on receive",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
...
/*  
	There should be no active handles, so this Second Waitany should
	return with an index = MPI_UNDEFINED 
*/
        ierr = MPI_Waitany(4 * NUMMESG, recv_request, &index, &one_stat);

        if (ierr != MPI_SUCCESS) {
            sprintf(info_buf,
                    "Non-zero return code (%d) from second MPI_Waitany on receive",
                    ierr);
            MPITEST_message(MPITEST_NONFATAL, info_buf);
        ...
        if (index != MPI_UNDEFINED) {
            sprintf(info_buf,
                    "Second Waitany:  index  not equal to  MPI_UNDEFINED (%d)",
                    index);
            MPITEST_message(MPITEST_NONFATAL, info_buf);
            fail++;
        }
        ...
        for (i = 0; i < 4 * NUMMESG; i++) {
            if (recv_request[i] != MPI_REQUEST_NULL) {
                sprintf(info_buf,
                        "recv_request[%d] not equal to  MPI_REQUEST_NULL(%p/%p) after second Waitany",
