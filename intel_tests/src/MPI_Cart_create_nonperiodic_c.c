...
/******************************************************************************
 ...
 Reference:  MPI Standard,  Chapter 6:  Process Topologies
  ...
  The test program is limited to testing not more than MAXNDIMS dimensions
                         ...
                         reorder = FALSE
  The ranks within the cartesian communicator are checked to make sure
  they are the same as the rank in the input communicator.
  Each process compares its rank in the original communicator with its rank
  in the newly created Cartesian communicator, and prints an error if they
  are not the same
...
int main(int argc, char *argv[])
{
   ...
   /*------------------------------  MPI_Init  -----------------------------*/
    ...
    sprintf(testname, "MPI_Cart_create with reorder = false");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            /*---------------------  MPI_Cart_create  -----------------------*/
                ...
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
                            "Non-zero return code (%d) from MPI_Cart_rank",
                            ierr);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_FATAL, info_buf);
                ...
                if (rank_cart == 0)
                    test_val = 1;
                /*
                 * Broacast value from node zero of the Cartesian communicator
                 */
                ...
                /* reorder = FALSE, check original rank and new rank are equal   */


                if (MPITEST_current_rank != rank_cart) {
                    fail++;
                    sprintf(info_buf,
                            "ERROR -- Ranks reordered  old/new = %3d/%3d  ",
                            MPITEST_current_rank, rank_cart);
                    MPITEST_message(MPITEST_FATAL, info_buf);
                }

                ierr = MPI_Comm_free(&comm_cart);
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf, "MPI_Comm_free() returned %d", ierr);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_FATAL, info_buf);
                    fail++;
