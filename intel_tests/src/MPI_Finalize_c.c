...
{
    int
     fail, ierr;                /* Return value from MPI calls     */
    ...
    if (ierr != MPI_SUCCESS) {
        fail++;
        sprintf(info_buf, "Non-zero return code (%d) from MPI_Finalize()",
                ierr);
        MPITEST_message(MPITEST_FATAL, info_buf);
    ...
    if (ierr == MPI_SUCCESS) {
        fail++;
        sprintf(info_buf,
                "MPI_Init after MPI_Finalize() FAILED, returned Success!");
        MPITEST_message(MPITEST_FATAL, info_buf);
    }
    ...
    if (ierr == MPI_SUCCESS) {
        fail++;
        sprintf(info_buf,
                "MPI_Barrier after MPI_Finalize() FAILED, returned Success!");
        MPITEST_message(MPITEST_FATAL, info_buf);
    }
