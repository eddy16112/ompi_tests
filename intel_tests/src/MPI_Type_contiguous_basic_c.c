...
All rank will call MPI_Type_contiguous() and then MPI_Type_commit() to create
...
The input count to MPI_Type_contiguous() is determined by each message length
in mpitest_cfg.h and the input oldtype to MPI_Type_contiguous() is determined
...
Test history:
   1  07/01/96     simont       Original version
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Type_contiguous_basic");
    ...
    /* find the maximum size buffer we will use */
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                MPITEST_get_gpu_buffer(test_type, max_length + 1,
                                   (void *) &buffer);
#else
                MPITEST_get_buffer(test_type, max_length + 1,
                                   (void *) &buffer);
#endif
                ...
                for (length_count = 0;
                    ...
                    ierr =
                        MPI_Type_contiguous(length,
                                            MPITEST_mpi_datatypes
                                            [test_type], &newtype);
                    if (ierr != MPI_SUCCESS) {
                        fail++;
                        sprintf(info_buf,
                                "MPI_Type_contiguous() returned %d", ierr);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        MPI_Error_string(ierr, &info_buf[0], &size);
                        MPITEST_message(MPITEST_FATAL, info_buf);
                    }
