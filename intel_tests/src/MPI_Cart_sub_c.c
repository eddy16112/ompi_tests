...
int main(int argc, char *argv[])
{
 ...
 /*---------------------- MPI_Cart_sub   ------------------------*/
                ...
                ierr = MPI_Cart_sub(comm_cart, rem_dims, &comm_sub);
            ...
            /*----------------------- MPI_Cart_get   ------------------------*/
                ...
                ierr = MPI_Cart_get(comm_sub,
                                    ndims,
                                    sub_dim_size, sub_periods, sub_coords);
                ...
                if (ndims > 1) {
                  ...
                  /*--------------------- MPI_Cart_get   --------------------*/
                        ...
                        ierr = MPI_Cart_get(comm_sub,
                                            sub_dims,
                                            sub_dim_size,
                                            sub_periods, sub_coords);

                        if (ierr != MPI_SUCCESS) {
                            fail++;
                            sprintf(info_buf, "MPI_Cart_get error (%d)",
                                    ierr);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            MPI_Error_string(ierr, &info_buf[0], &size);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                        ...
                        ierr =
                            MPI_Bcast(&test_val, 1, MPI_INT, 0, comm_sub);
                        if (ierr != MPI_SUCCESS) {
                            sprintf(info_buf,
                                    "Non-zero return code (%d) from MPI_Bcast",
                                    ierr);
                        ...
                        for (k = 0; k < sub_dims; k++) {
                            if (sub_dim_size[k] != test_dim_size[k]) {
                                sprintf(info_buf,
                                        "Error in Sub-communicator dimensions for dimension %3d:  expected %3d   actual = %d",
                                        k, test_dim_size[k],
                                        sub_dim_size[k]);
