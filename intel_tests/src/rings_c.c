...
int Random, use_buffers;

int buf_size, max_len, seed;
int ierr, size;
...
main(int argc, char *argv[])
{
    ...
    if (MPITEST_me == 0) {
        sprintf(info_buf,
                ...
                minutes, MPITEST_nump, num_rings, Random, seed,
                use_buffers);
        MPITEST_message(MPITEST_INFO0, info_buf);
    }
    ...
    if (snd_buf == NULL) {
        MPITEST_message(MPITEST_FATAL,
                        "allocation of snd_buf[] *** FAILED ***");
    }

    rings =
        (struct ring_type **) calloc(num_rings,
                                     sizeof(struct ring_type *));
    if (rings == NULL) {
        MPITEST_message(MPITEST_FATAL,
                        "allocation of rings[][] *** FAILED ***");
    }
    ...
    for (i = 0; i < num_rings; i++) {
        ierr =
            MPI_Bcast(rings[i], 2 * MPITEST_nump, MPI_LONG, 0,
                      MPI_COMM_WORLD);
        if (ierr != MPI_SUCCESS) {
    ...
    ierr = MPI_Bcast(&seed, 1, MPI_INT, 0, MPI_COMM_WORLD);
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf, "MPI_Bcast() returned %d", ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
    ...
    if (setjmp(to_vec) == 0) {
        ...
        if (MPITEST_me == 0) {
            if (sig_caught == SIGINT) {
                ...
                sprintf(info_buf,
                        "(%d): RINGS: %d min Time Limit Reached!, %d shifts\n",
 ...
 *
 ****************************************************************************/
void check_buf(char *buf, int ring, int msg)
{
    ...
    if (info_node != rings[ring][MPITEST_me].from_node) {
        failed++;
        sprintf(info_buf,
                "(%d): Ring %d Msg %d: *** FAILED *** Not from expected node, %d != %d\n",
                MPITEST_me, ring, msg, rings[ring][MPITEST_me].from_node,
                info_node);
        MPITEST_message(MPITEST_INFO0, info_buf);
    }
    ...
    if (info_len != header_len + sizeof(struct header)) {
        failed++;
        sprintf(info_buf,
                "(%d): Ring %d Msg %d: *** FAILED *** info_len != header_len, %d != %d\n",
                MPITEST_me, ring, msg, info_len,
                header_len + sizeof(struct header));
        MPITEST_message(MPITEST_NONFATAL, info_buf);
    }
    ...
    if (info_node != header_node) {
        failed++;
        sprintf(info_buf,
                "(%d): Ring %d Msg %d: *** FAILED *** info_node != header_node, %d != %d\n",
                MPITEST_me, ring, msg, info_len,
                header_len + sizeof(struct header));
        MPITEST_message(MPITEST_NONFATAL, info_buf);
    }
    ...
    if (info_type != header_type) {
        failed++;
        sprintf(info_buf,
                "(%d): Ring %d Msg %d: *** FAILED *** info_type != header_type, %d != %d\n",
    ...
    if (failed > error_cnt) {
        sprintf(info_buf,
                "(%d): Ring %d Msg %d: *** FAILED *** %d bad fill_char(s) in buf\n",
    ...
    if (failed > error_cnt) {
        sprintf(info_buf,
                "(%d): Ring %d Msg %d: *** FAILED *** %d missing zero(s) in buf\n",
                MPITEST_me, ring, msg, failed - error_cnt);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
    }
}                               /* check_buf */
 ...
 *
 ****************************************************************************/
void fill_buf(char *rcv_buf, char *snd_buf, long msg_len)
{
    char fill_char;
...
void snd_rcv_nbuf()
{
    ...
    for (ring = 0; ring < num_rings; ring++) {
        ...
        ierr =
            MPI_Irecv(rcv_buf, buf_size, MPI_BYTE, MPI_ANY_SOURCE,
                      10000 + ring, MPI_COMM_WORLD, &mpi_request);
        if (ierr != MPI_SUCCESS) {
        ...
        ierr = MPI_Send(snd_buf, snd_len + sizeof(struct header), MPI_BYTE,
                        rings[ring][MPITEST_me].to_node, 10000 + ring,
                        MPI_COMM_WORLD);
        if (ierr != MPI_SUCCESS) {
        ...
        sprintf(info_buf, "(%d): Ring %d Msg %d: Sent to: %d, %d bytes\n",
                MPITEST_me, ring, cnt,
                rings[ring][MPITEST_me].to_node, snd_len);
        MPITEST_message(MPITEST_INFO1, info_buf);

        sprintf(info_buf,
                "(%d): Ring %d Msg %d: Received from: %d, %d bytes\n",
                MPITEST_me, ring, cnt, mpi_status.MPI_SOURCE, mpi_count);
        MPITEST_message(MPITEST_INFO1, info_buf);
...
void snd_rcv_buf()
{
    ...
    for (i = 0; i < num_mod; i++) {
        ...
        for (j = start; j < end; j++) {
            ...
            ierr =
                MPI_Send(snd_buf, snd_len + sizeof(struct header),
                         MPI_BYTE, rings[j][MPITEST_me].to_node, 10000 + j,
                         MPI_COMM_WORLD);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf, "MPI_Send() returned %d", ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            sprintf(info_buf,
                    "(%d): Ring %d Msg %d: Sent to: %d, %d bytes\n",
                    MPITEST_me, j, cnt, rings[j][MPITEST_me].to_node,
                    snd_len);
        ...
        for (j = end - 1; j >= start; j--) {
            ...
            sprintf(info_buf,
                    "(%d): Ring %d Msg %d: Received from: %d, %d bytes\n",
                    MPITEST_me, j, cnt, mpi_status.MPI_SOURCE, mpi_count);
            MPITEST_message(MPITEST_INFO1, info_buf);
