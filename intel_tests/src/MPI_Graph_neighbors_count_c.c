...
int main(int argc, char *argv[])
{
   ...
   /*------------------------------  MPI_Init  -----------------------------*/
    ...
    sprintf(testname, "MPI_Graph_neighbors_count");
            ...
            /*-----------------------------  MPI_Graph_create  --------------------------*/
               ...
               /*------------------------  MPI_Graph_neighbors_count  ---------------------*/
                    ...
                    for (i = 0; i < graph_nump; i++) {
                        ...
                        ierr = MPI_Graph_neighbors_count(comm_graph,
                                                         i, &neighbors);

                        if (ierr != MPI_SUCCESS) {
                            fail++;
                            sprintf(info_buf,
                                    "MPI_Graph_neighbor_count error (%d)",
                                    ierr);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            MPI_Error_string(ierr, &info_buf[0], &size);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                        ...
                        if (i > 0)
                            ncalc = index[i] - index[i - 1];

                        if (neighbors != ncalc) {
                            sprintf(info_buf,
                                    "Error in Graph_neighbor_count for rank (%3d)  Actual = (%d)   Expected (%d)",
                                    i, neighbors, ncalc);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                        }
