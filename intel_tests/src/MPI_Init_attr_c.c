    ...
    *attr,                      /* Attribute returned              */
    ...
    if (MPITEST_me == 0) {
        ...
        MPITEST_message(MPITEST_VERIFY,
                        "Verify values of the following environmental attributes:");
    }
    ...
    for (i = 0; i < MPITEST_nump; i++) {
        if (i == MPITEST_me) {
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Attr_get(MPI_TAG_UB)",
                        ierr);
    ...
    for (i = 0; i < MPITEST_nump; i++) {
        if (i == MPITEST_me) {
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Attr_get(MPI_HOST)",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
     ...
     */
    ierr =
        MPI_Attr_get(MPI_COMM_WORLD, MPI_WTIME_IS_GLOBAL, &attr, &found);
    for (i = 0; i < MPITEST_nump; i++) {
        if (i == MPITEST_me) {
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Attr_get(MPI_WTIME_IS_GLOBAL)",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                ...
                verify++;
                sprintf(info_buf,
                        "MPI_WTIME_IS_GLOBAL=%d (1=synchronized, else 0)",
                        *attr);
                MPITEST_message(MPITEST_VERIFY, info_buf);
            }
        }
    ...
    for (i = 0; i < MPITEST_nump; i++) {
        if (i == MPITEST_me) {
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Attr_get(MPI_IO)",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
