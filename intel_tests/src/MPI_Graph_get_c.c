...
int main(int argc, char *argv[])
{
    int
     ...
     maxedges = 0,              /* length of edges vector                            */
    ...
    int
     ...
     graph_index[MPITEST_MAX_RANKS],    /* index array from Graph_get   */
     graph_edges[MPITEST_MAX_RANKS * MPITEST_MAXN];     /* list from Graph_get    */
   ...
   /*------------------------------  MPI_Init  -----------------------------*/
    ...
    sprintf(testname, "MPI_Graph_get");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            /*------------------------- MPI_Graph_get   ----------------------------*/
                ...
                ierr = MPI_Graph_get(comm_graph,
                                     maxindex,
                                     maxedges, graph_index, graph_edges);

                if (ierr != MPI_SUCCESS) {
                    fail++;
                    sprintf(info_buf, "MPI_Graph_get error (%d)", ierr);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_FATAL, info_buf);
                }
                ...
                for (i = 0; i < maxindex; i++) {
                    if (graph_index[i] != index[i]) {
                        sprintf(info_buf,
                                "Error in Graph_get for node (%3d):  Actual = (%d)   Expected (%d)",
                                i, graph_index[i], index[i]);
                        MPITEST_message(MPITEST_FATAL, info_buf);
                    }
                }
                ...
                for (j = 0; j < maxedges; j++) {
                    if (graph_edges[j] != edges[j]) {
                        sprintf(info_buf,
                                "Error in Graph_get for edge (%3d):  Actual = (%d)   Expected (%d)",
                                j, graph_edges[j], edges[j]);
                        MPITEST_message(MPITEST_FATAL, info_buf);
                    }
                }
