...
int main(int argc, char *argv[])
{
    ...
    for (length_count = 0; length_count < MPITEST_num_message_lengths();
        ...
        if (MPITEST_current_rank == 0) {
            sprintf(info_buf, "(%d) length %d", length_count, length);
            MPITEST_message(MPITEST_INFO1, info_buf);
        }

        /* allocate address space */
        buffer = (signed char *) malloc(sizeof(signed char) * length);
        if (!buffer) {
            fail++;
            sprintf(info_buf,
                    "Cannot allocate enough buffer space of size %lu bytes",
                    length * sizeof(char));
            ...
            for (i = 0; i < length; i++) {
                ...
                if (ierr != MPI_SUCCESS) {
                    ...
                    if (((long) out_addr) < ((long) MPI_BOTTOM)) {
                        fail++;
                        sprintf(info_buf,
                                "Returned address (%ld) from MPI_Address is less than MPI_BOTTOM (%ld)",
                                (long) out_addr, (long) MPI_BOTTOM);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                    }
                }
            }
