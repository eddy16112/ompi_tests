...
int main(int argc, char *argv[])
{
    ...
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Group_intersection(group1, group2)",
                ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_FATAL, info_buf);
    ...
    if (result != MPI_UNDEFINED) {
        fail++;
        sprintf(info_buf,
                "MPI_Group_rank(group1, group2) returned %d, expected MPI_UNDEFINED",
                result);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
    }
    ...
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Group_intersection(group2, group1)",
                ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
    ...
    if (result != MPI_UNDEFINED) {
        fail++;
        sprintf(info_buf,
                "MPI_Group_rank(group2, group1) returned %d, expected MPI_UNDEFINED",
                result);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
    }
