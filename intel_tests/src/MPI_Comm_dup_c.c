...
int main(int argc, char *argv[])
{
    int
     ...
     result, resultdup, buffer1, buffer2, buffer3, size;
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_dup (comm_index %d)",
                        ierr, comm_index);
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_test_inter #1 (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (ierr != MPI_SUCCESS) {
                ...
                sprintf(info_buf,
                        "MPI_Comm_test_inter returned %d, expected %d (comm_index %d)",
                        resultdup, result, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            loop_cnt++;
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_size #1 (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (ierr != MPI_SUCCESS) {
                ...
                sprintf(info_buf,
                        ...
                        resultdup, MPITEST_current_rank, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            loop_cnt++;
            ...
            if ((test_nump >= 2) && (intercomm == 0)) {
                if (MPITEST_current_rank == 0) {        /* Sender */
                    ...
                    MPI_Isend(&buffer2, 1, MPI_INT, 1, 1, comm2,
                              &request2);
                    MPI_Isend(&buffer3, 1, MPI_INT, 1, 1, comm2,
                              &request3);
                    MPI_Barrier(comm2);
                    ...
                    buffer3 = 0;
                    MPI_Barrier(comm2);
                    MPI_Irecv(&buffer2, 1, MPI_INT, 0, 1, comm2,
                              &request2);
                    MPI_Irecv(&buffer3, 1, MPI_INT, 0, 1, comm2,
                              &request3);
                    ...
                    if ((buffer1 != 1) || (buffer2 != 2) || (buffer3 != 3)) {
                        sprintf(info_buf,
                                "Receive comms misfiltered messages (Isend): %d/%d/%d, expected 1/2/3 (comm_index %d)",
                    ...
                    if ((buffer1 != 1) || (buffer2 != 2) || (buffer3 != 3)) {
                        sprintf(info_buf,
                                "Receive comms misfiltered messages (Send): %d/%d/%d, expected 1/2/3 (comm_index %d)",
                                buffer1, buffer2, buffer3, comm_index);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        fail++;
                    }
            ...
            if ((test_nump >= 2) && (intercomm == 0)) {
                if (MPITEST_current_rank == 0) {        /* Sender */
                    MPI_Isend(&buffer1, 1, MPI_INT, 1, 1, comm2,
                              &request1);
                } else if (MPITEST_current_rank == 1) { /* Receiver */
                    buffer1 = 0;
                    MPI_Irecv(&buffer1, 1, MPI_INT, 0, 1, comm2,
                              &request1);
                }
            }
            ...
            if ((test_nump >= 2) && (intercomm == 0)) {
                if (MPITEST_current_rank == 0) {        /* Sender */
                    ...
                    if (buffer1 != 1) {
                        sprintf(info_buf,
                                "Receive while freeing comm received incorrect data %d, expected 1 (comm_index %d)",
                                buffer1, comm_index);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        fail++;
                    }
