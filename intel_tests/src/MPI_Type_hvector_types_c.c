...
(in mpitest_cfg.h) using MPI_Type_hvector().  The root's rank will then send
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Type_hvector_types");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (length_count = 0;
                ...
                ierr =
                    MPI_Type_hvector(length, 1, extent, oldtype, &newtype);
                if (ierr != MPI_SUCCESS) {
                    fail++;
