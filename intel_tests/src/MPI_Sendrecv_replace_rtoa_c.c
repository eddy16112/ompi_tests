...
int main(int argc, char *argv[])
{
    int
     ...
     root_all_length, length_count,     /*  loop counter for message length loop              */
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                for (length_count = 0;
                    ...
                    for (grp_lup = 1; grp_lup <= ntimes; grp_lup++) {
                        ...
                        for (root = 0; root < senders; root++) {
                            ...
                            if (print_node_flag) {
                                sprintf(info_buf,
                                        "(%d,%d,%d) node_all_length %d commsize %d commtype %d data_type %d root %d",
                                        length_count, comm_count,
                                        type_count, node_all_length,
			    ...
			    -------------------------------------------------*/
                            ...
                            if ((!inter_comm
                                ...
                                ierr =
                                    MPI_Sendrecv_replace(node_all_buffer,
                                                         node_all_length,
                                ...
                                if (ierr != MPI_SUCCESS) {
                                    sprintf(info_buf,
                                            "Non-zero return code (%d) from node %d  MPI_Sendrecv_replace",
                                ...
                                if (error) {
                                    sprintf(info_buf,
                                            "Node Send/Receive lengths differ - Sender(node/length)=%d/%d,  Receiver(node/length)=%d/%d",
                                    ...
                                    && MPITEST_current_rank == root))
                            {
                                ...
                                for (i = 0; i < send_to; i++) {
                                    ...
                                    ierr =
                                        ...
                                        (root_all_buffer, root_all_length,
                                         MPITEST_mpi_datatypes[test_type],
                                         i, root, i, i, comm, &root_stat);


                                    if (ierr != MPI_SUCCESS) {
                                        sprintf(info_buf,
                                                "Non-zero return code (%d) from root MPI_Sendrecv_replace",
                                                ierr);
                                        MPITEST_message(MPITEST_NONFATAL,
                                                        info_buf);
                                    ...
                                    if (ierr != MPI_SUCCESS) {
                                        sprintf(info_buf,
                                                "Non-zero return code (%d) from Root:  MPI_Get_count",
                                                ierr);
                                        MPITEST_message(MPITEST_NONFATAL,
                                                        info_buf);
                                        MPI_Error_string(ierr,
                                    ...
                                    if (error) {
                                        sprintf(info_buf,
                                                "Root: Send/Recv lengths differ - Sender(node/length)=%d/%d,  Receiver(node/length)=%d/%d",
                                                root_stat.MPI_SOURCE,
                                                root_all_length, root,
                                                root_cnt_len);
                                        MPITEST_message(MPITEST_NONFATAL,
                                                        info_buf);
                                        loop_fail++;
