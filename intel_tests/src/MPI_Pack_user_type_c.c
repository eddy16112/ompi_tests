...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Pack_user_type");
            ...
            for (length_count = 0;
                ...
                for (root = 0; root < test_nump; root++) {
                    ...
                    if (MPITEST_current_rank == root) {
                        ...
                        for (i = 0; i < length; i++) {
                            ierr = MPI_Pack(buf_ptr, 1, newtype,
                                            packed_buffer,
                                            pack_size +
                                            MPITEST_ADD_BUFSIZE, &position,
                                            comm);
                            if (ierr != MPI_SUCCESS) {
                                fail++;
                                sprintf(info_buf, "MPI_Pack() returned %d",
                                        ierr);
                        ...
                        /* Must receive with type MPI_PACKED because the data 
                           is extracted with MPI_Unpack */
                        ierr =
                        ...
                        if (ierr != MPI_SUCCESS) {
                            ...
                            /* The following test is not allowed by the MPI standard.
                               MPI_Get_count must be called with type MPI_PACKED, not
                               newtype.  The return value of count is then relative
                               to MPI_PACKED, not newtype */
                        ...
                        for (i = 0; i < length; i++) {
                            ierr =
                                MPI_Unpack(packed_buffer,
                                           pack_size + MPITEST_ADD_BUFSIZE,
                                           &position, buf_ptr, 1, newtype,
                                           comm);
                            if (ierr != MPI_SUCCESS) {
                                fail++;
                                sprintf(info_buf,
                                        "MPI_Unpack() returned %d", ierr);
                                MPITEST_message(MPITEST_NONFATAL,
                                                info_buf);
                                MPI_Error_string(ierr, &info_buf[0],
                                                 &size);
