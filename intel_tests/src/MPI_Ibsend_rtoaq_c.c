...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname,
            "MPI_Ibsend_rtoaq: Root TO All Quality of implementation");
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                MPI_Pack_size(max_length, MPITEST_mpi_datatypes[test_type],
                              comm, &ps);
                buffsize = test_nump * (ps + MPI_BSEND_OVERHEAD);
                                    ...
                                    while (errc == MPI_ERR_BUFFER) {
                                        ...
                                        if (ierr != MPI_SUCCESS) {
                                            ...
                                            if (errc != MPI_ERR_BUFFER) {
                                                sprintf(info_buf,
                                                        "Non-zero return code (%d/%d) from MPI_Ibsend",
                                                        ierr, errc);
                                                MPITEST_message
                                                    (MPITEST_NONFATAL,
                                                     info_buf);
                                                sprintf(info_buf,
                                                        "MPI_Error_string message:  ");
                                        ...
                                        if (errc == 0) {
                                            ierr =
                                                MPI_Wait(&send_request,
                                                         &send_stat);
                                            if (ierr != MPI_SUCCESS) {
                                                MPI_Error_class(ierr,
                                                                &errc);
                                                if (errc != MPI_ERR_BUFFER) {
                                                    sprintf(info_buf,
                                                            "Non-zero return code (%d) from MPI_Wait on Ibsend",
                                                            ierr);
                                                    MPITEST_message
                                                        (MPITEST_NONFATAL,
                                                         info_buf);
