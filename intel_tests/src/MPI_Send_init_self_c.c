...
-----------------------------------------------------------------------------*/
/******************************************************************************
	 Test for Persistent  MPI_Send_init()  to self
	                with  MPI_Recv_init()  to receive messages
This tests the persistent MPI_Send_init operation from a node to itself.
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Send_init_self: all persistent Send to SELF");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if ((MPITEST_current_rank != MPI_UNDEFINED) &&
            ...
            if (MPITEST_current_rank == 0) {
                if (test_nump > 1) {
                    /*
                     * Node 1 will send a message when completed; wait for
                     * it, and fail after 30 seconds
                     */
                    ierr = MPI_Recv_init(&irecv_buff,
                                         0,
                    ...
                    while (flag == 0) {
                        ...
                        if (ierr != MPI_SUCCESS) {
                            sprintf(info_buf,
                                    "Non-zero return code (%d) from MPI_Test",
                                    ierr);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            MPI_Error_string(ierr, &info_buf[0], &size);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                        ...
                        if (loop_fail > 300) {
                            sprintf(info_buf,
                                    "Nodes sending to self not responding, test FAILED");
                            MPITEST_message(MPITEST_FATAL, info_buf);
                        }
                    }           /* while */
                    ...
                    ierr = MPI_Request_free(&recv_req);
                    if (ierr != MPI_SUCCESS) {
                        sprintf(info_buf,
                ...
                for (type_count = 0; type_count < MPITEST_num_datatypes();
                    ...
                    max_length =
                        MPITEST_byte_to_element(test_type,
                                                max_byte_length);
                    ...
                    for (length_count = 0;
                         ...
                         length_count++) {
                        byte_length =
                            MPITEST_get_message_length(length_count);
		        ...
		        -------------------------------------------------*/
                        ...
                        ierr = MPI_Send_init(send_buffer, length,
 ...
 /*-------- Call MPI_Request_free -------*/
                        ...
                        /*
                         * Print non-fatal error if Received length not equal
                         * to send length
                         */
                ...
                if (MPITEST_current_rank == 1) {
                    ierr =
                        MPI_Send_init(send_buffer, 0, MPI_INT, 0, 1, comm,
                                      &send_request);
                    ...
                    if (ierr != MPI_SUCCESS) {
                        ...
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        sprintf(info_buf,
                                "Non-zero return code (%d) from MPI_Isend signal",
                                ierr);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        MPI_Error_string(ierr, &info_buf[0], &size);
                        MPITEST_message(MPITEST_FATAL, info_buf);
