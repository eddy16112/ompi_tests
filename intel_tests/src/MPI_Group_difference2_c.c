...
non-overlapping groups created from a subset of MPI_COMM_WORLD in both
directions.

group1 = all ranks EXCEPT 0, 1 and the last, in order.
group2 = last and first rank (in that order).

What should be an identical group is compared to
that created by MPI_Group_difference.
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Group_difference2");
    ...
    ierr = MPI_Group_compare(group, group1, &result);
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
