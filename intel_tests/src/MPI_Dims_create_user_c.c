    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            default:
                fail++;
                sprintf(info_buf,
                        "Error in dimension calculation:  ndims = %d",
                        ndims);
                MPITEST_message(MPITEST_FATAL, info_buf);
            }
            for (i = 0; i < ndims; i++) {
         ...
         /*-----------------------  MPI_Dims_create  ----------------------*/
            ...
            if (ierr != MPI_SUCCESS) {
                ...
                sprintf(info_buf,
                        "MPI_Dims_create error (%d)  Errorclass = %d",
                        ierr, errorclass);
            ...
            if (MPITEST_current_rank == 0) {
                if (nzeros) {
                    sprintf(info_buf,
                            "Dimension sizes < 1 after call to MPI_Dims_create  %d bad values",
                            nzeros);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    fail++;
                }
            ...
            if (tot_dim_size != nnodes) {
                sprintf(info_buf,
                        "Nnodes does not equal product of dimension sizes:  Nnodes = %d   tot_dim_size = %d",
                        nnodes, tot_dim_size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            if (error) {
                sprintf(info_buf,
                        "User assigned dimensions changed by MPI_Dims_create in  %d  cases",
                        error);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            if (error) {
                sprintf(info_buf,
                        "Error:  system assigned values increase in   %d  case",
                        error);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
