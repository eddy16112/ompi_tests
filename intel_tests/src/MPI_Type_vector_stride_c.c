...
(in mpitest_cfg.h) using MPI_Type_vector() using 2 * extent as
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Type_vector_stride");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (length_count = 0;
                ...
                ierr = MPI_Type_vector(length, 1, 2, oldtype, &newtype);
                if (ierr != MPI_SUCCESS) {
                    fail++;
                ...
                for (root = 0; root < test_nump; root++) {
                    ...
                    if (MPITEST_current_rank == root) {
                        ...
                        if (buffer[length * (extent + extent)] !=
