...
Each rank (except the root rank) will do a probe() with MPI_ANY_TAG
...
The test only send 1 MPI_INT in each request.
...
int main(int argc, char *argv[])
{
    int
     ...
     tag,                       /*  message tag                                       */
     maxtag, maxnp, *attr_ub, found, i, j;
    ...
    void *buffer[MPITEST_MAX_NP][MPITEST_MAX_TAG];      /* message buffer                          */
    ...
    MPI_Request *recv_request,
        send_request[MPITEST_MAX_NP][MPITEST_MAX_TAG];
    ...
    sprintf(testname, "MPI_Probe_tag");
    ...
    if (!found) {
        ...
        sprintf(info_buf,
                "Attribute MPI_TAG_UB (%d) is less than the required minimum (%d)",
                *attr_ub, MPITEST_TAG_UB_MIN);
        MPITEST_message(MPITEST_FATAL, info_buf);
    }
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            recv_request =
                (MPI_Request *) malloc(maxtag * sizeof(MPI_Request));
            if (recv_request == NULL) {
                sprintf(info_buf,
                        "Unable to allocate enough space for receive requests array, FAILED");
                MPITEST_message(MPITEST_FATAL, info_buf);
            }
            ...
            for (j = 0; j < maxnp; j++) {
                for (i = 0; i < maxtag; i++) {
                    MPITEST_get_buffer(test_type, max_length,
                                       &buffer[j][i]);
                }
            }
            ...
            for (root = 0; root < maxnp; root++) {
                ...
                if ((MPITEST_current_rank != root)
                    ...
                    /* Call MPI_Probe() with MPI_ANY_TAG first */

                    sprintf(info_buf,
                            "Probing source: %d, tag: MPI_ANY_TAG (%d)",
                            root, MPI_ANY_TAG);
                    MPITEST_message(MPITEST_INFO1, info_buf);

                    ierr = MPI_Probe(root, MPI_ANY_TAG, comm, &status);
                    if (ierr != MPI_SUCCESS) {
                        fail++;
                        sprintf(info_buf, "MPI_Probe() returned %d", ierr);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        MPI_Error_string(ierr, &info_buf[0], &size);
                        MPITEST_message(MPITEST_FATAL, info_buf);
                    } else {
                        ...
                        if (status.MPI_SOURCE != root) {
                            fail++;
                            sprintf(info_buf,
                                    "status object returned from MPI_Probe() has unexpected MPI_SOURCE (%d) field",
                                    status.MPI_SOURCE);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                        }
                        ...
                        if (status.MPI_TAG < 0) {
                            fail++;
                            sprintf(info_buf,
                                    "status object returned	from MPI_Probe() has negative MPI_TAG (%d).",
                                    status.MPI_TAG);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                        } else if (status.MPI_TAG >= maxtag) {
                            fail++;
                            sprintf(info_buf,
                                    "status object returned	from MPI_Probe() has too large MPI_TAG (%d).",
                                    status.MPI_TAG);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                        }
                        ...
                        if (ierr != MPI_SUCCESS) {
                            ...
                            if (count != length) {
                                fail++;
                                sprintf(info_buf,
                                        "status object returned from MPI_Probe() contains unexpected length."
                                        "  Expected: %d, Actual: %d",
                                        length, count);
                    ...
                    for (tag = maxtag - 1; tag >= 0; tag--) {
                        sprintf(info_buf, "Probing source: %d, tag: %d",
                                root, tag);
                        MPITEST_message(MPITEST_INFO1, info_buf);

                        ierr = MPI_Probe(root, tag, comm, &status);
                        if (ierr != MPI_SUCCESS) {
                            fail++;
                            sprintf(info_buf, "MPI_Probe() returned %d",
                                    ierr);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            MPI_Error_string(ierr, &info_buf[0], &size);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                    ...
                    for (dest = 0; dest < maxnp; dest++) {
                        if (dest != root) {
                            for (tag = 0; tag < maxtag; tag++) {
                                sprintf(info_buf,
                                        "Sending from source: %d to %d, tag: %d",
                                        MPITEST_current_rank, dest, tag);
                                MPITEST_message(MPITEST_INFO1, info_buf);

                                ierr =
                                    MPI_Isend(buffer[dest][tag], length,
                                              MPITEST_mpi_datatypes
                                              [test_type], dest, tag, comm,
                                              &(send_request[dest][tag]));

                                if (ierr != MPI_SUCCESS) {
                                    fail++;
                                    sprintf(info_buf,
                                            "MPI_Isend() returned %d",
                                            ierr);
                                    MPITEST_message(MPITEST_NONFATAL,
                                                    info_buf);
                                    MPI_Error_string(ierr, &info_buf[0],
                ...
                if ((MPITEST_current_rank != root) &&
                    (MPITEST_current_rank < maxnp)) {
                    for (tag = maxtag - 1; tag >= 0; tag--) {
                        sprintf(info_buf, "Receiving source: %d, tag: %d",
                                root, tag);
                        MPITEST_message(MPITEST_INFO1, info_buf);

                        ierr =
                            MPI_Irecv(buffer[0][0], length,
                                      MPITEST_mpi_datatypes[test_type],
                                      root, tag, comm,
                                      &(recv_request[tag]));
                        if (ierr != MPI_SUCCESS) {
                        ...
                        ierr = MPI_Wait(&(recv_request[tag]), &status);
                        if (ierr != MPI_SUCCESS) {
                            fail++;
                        ...
                        if (error) {
                            ...
                            sprintf(info_buf,
                                    ...
                                    error, comm_count, length, test_nump,
                                    comm_type, test_type, root);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                        } else {
                ...
                if (MPITEST_current_rank == root) {
                    /* Wait on message sent to all ranks in communicator */
                    for (dest = 0; dest < maxnp; dest++) {
                        if (dest != root) {
                            for (tag = 0; tag < maxtag; tag++) {
                                ierr =
                                    MPI_Wait(&(send_request[dest][tag]),
                                             &status);
                ...
                if (MPITEST_current_rank == root) {
                    ...
                    for (dest = 0; dest < maxnp; dest++) {
                        if (dest != root) {
                            for (tag = 0; tag < maxtag; tag++) {
                                ierr =
                                    MPI_Cancel(&(send_request[dest][tag]));

                                if (ierr != MPI_SUCCESS) {
                                    fail++;
                                    sprintf(info_buf,
                                            "MPI_Cancel() returned %d",
                                            ierr);
                                    MPITEST_message(MPITEST_NONFATAL,
                                                    info_buf);
                                    MPI_Error_string(ierr, &info_buf[0],
