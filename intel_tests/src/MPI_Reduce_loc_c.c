...
-----------------------------------------------------------------------------*/
/******************************************************************************
                       Test for MPI_Reduce_loc()

This code tests the MPI_Reduce() function.  MPI_Reduce() performs a binary,
associative operation in such a way that data from process "i" is
operand exactly once.  The result is accrued only at the root process.
...
int main(int argc, char *argv[])
{
    ...
    *recv_buffer;
    ...
    sprintf(testname, "MPI_Reduce_loc()");
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                if ((send_buffer == NULL) || (recv_buffer == NULL)) {
                    MPITEST_message(MPITEST_FATAL,
                                    "Failed to allocate memory in MPI_Reduce_loc");
                }
                    ...
                    for (op_count = 0; op_count < num_ops; op_count++) {
                        for (root = 0; root < test_nump; root++) {
                            ...
                            if (MPITEST_current_rank == 0) {
                                sprintf(info_buf,
                                        "length %d commsize %d commtype %d data_type %d root %d operation %d",
                            ...
                            if (has_op(op_count, test_type)
                                ...
                                get_reduce_answer(op_count, test_nump,
                                                  &value);
                                ...
                                if (error) {
                                    if (ierr == MPI_SUCCESS)
                                        fail++;
                                    sprintf(info_buf,
                                            "%d errors in buffer, len %d commsize %d commtype %d data_type %d root %d op %d",
