...
int main(int argc, char *argv[])
{
    ...
    int
     ...
     loop_fail, i, j,           /* utility loop index variables                  */
    ...
    int Send_buffer[10],        /* message buffer                        */
     Bsend_buffer[10],
        Rsend_buffer[10],
        Ssend_buffer[10], Isend_buffer[10], recv_buffer[5];
    ...
    MPI_Request send_request,   /* MPI request structure                         */
        recv_request[5];

    MPI_Status send_stat,       /* MPI  status structure                         */
        recv_stat[5];
 ...
 /*----------------- Call MPI_Request_free ------------------*/
            ...
            if (send_request != MPI_REQUEST_NULL) {
                sprintf(info_buf,
                        "request not equal to  MPI_REQUEST_NULL after calling MPI_Request_free (Send)");
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            ...
            ierr = MPI_Bsend_init(Bsend_buffer,
                                  1,
                                  MPI_INT,
                                  1, 1, MPI_COMM_WORLD, &send_request);
 ...
 /*-------- Call MPI_Request_free -------*/
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Request_free for Bsend",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            ierr = MPI_Ssend_init(Ssend_buffer,
                                  1,
                                  MPI_INT,
                                  1, 2, MPI_COMM_WORLD, &send_request);
 ...
 /*--------- Call MPI_Request_free -------*/
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Request_free for Ssend",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (send_request != MPI_REQUEST_NULL) {
                sprintf(info_buf,
                        "request not equal to  MPI_REQUEST_NULL after calling MPI_Request_free (Ssend)");
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            ...
            ierr = MPI_Rsend_init(Rsend_buffer,
                                  1,
                                  MPI_INT,
                                  1, 3, MPI_COMM_WORLD, &send_request);

            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Rend_init",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
 ...
 /*---------  Start the Rsend  ----------*/
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Start for Rsend",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
 ...
 /*------- Call MPI_Request_free ------*/
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Request_free after Rsend",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            ierr = MPI_Isend(Isend_buffer,
                             1,
                             MPI_INT, 1, 4, MPI_COMM_WORLD, &send_request);
...
/*#############################  RECEIVERS  ###############################*/
            ...
            ierr = MPI_Recv_init(&recv_buffer[4],
                                 1,
                                 MPI_INT,
                                 0, 4, MPI_COMM_WORLD, &recv_request[4]);
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Request_free for receive %d ",
                        ierr, i);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
