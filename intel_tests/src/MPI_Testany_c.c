...
int main(int argc, char *argv[])
{
    int
     ...
     send_loop_cnt,             /* count tries on send Testany loop                  */
     recv_loop_cnt,             /* count tries on receive Testany loop               */
    ...
    if (MPITEST_me == 0) {
        ...
        for (i = 0; i < 4 * NUMMESG; i++) {
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code ierr/flag (%d/%d) from MPI_Testany on Isend, after %d calls to MPI_Testany",
            ...
            sprintf(info_buf,
                    "Testany on send:  Index %d   Error %d  #Testany calls %d ",
                    index, ierr, send_loop_cnt);
 ...
 /*---------------------------------  Irecv  -----------------------------*/
    if (MPITEST_me == 1) {
        ...
        for (i = 0; i < 4 * NUMMESG; i++) {
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code ierr/flag (%d/%d) from MPI_Testany on Irecv, after %d calls to MPI_Testany",
