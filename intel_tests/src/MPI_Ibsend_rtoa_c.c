...
-----------------------------------------------------------------------------*/
/******************************************************************************
     Test for MPI_Ibsend():  ROOT send to ALL version
          and MPI_Irecv():   specified source/any tag.


Tests the nonblocking(asynchronous) buffered MPI_Ibsend operation.
The nonblocking(asynchronous) Irecv call is used to receive messages.
MPI_Test is used to test for completion of the Ibsend.
...
An MPI_Wait call is used with the MPI_Ibsend, and MPI_Irecv to verify that
the operation has properly completed.

The Standard  notes, in Section 3.6, that only one buffer can be
...
including itself.  Since MPI_Irecv blocks until, at least, the send has been
...
int main(int argc, char *argv[])
{
    ...
    *values;                    /* Array of dataTemplates for verbose init           */
    ...
    sprintf(testname, "MPI_Ibsend_rtoa: Root Ibsend TO All");
 ...
 /*----------------------  Loop over Communicators  ----------------------*/
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
             ...
             * gets to print.  For INTRA-communicators it is simply node 0. For
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                for (length_count = 0;
                     ...
                     * each root sends to all of the nodes of the communicator.
                     * For an inter-communicator we have two sets of nodes, a
                     * left and right sub-communicator, and we want to loop
                     * through all combinations. When a node is selected in a
                     * sub-communicator, it sends to all nodes in the other
                     * communicator.
                     */
			    ...
			    -------------------------------------------------*/
                                    ...
                                    && MPITEST_current_rank == root))
                            {
                                ...
                                /*
                                 * Reattach the  send buffer for this root, and
                                 * detach it before switching to new root
                                 */
                                ...
                                for (i = 0; i < send_to; i++) {
                                    ...
                                    if (ierr != MPI_SUCCESS) {
                                        sprintf(info_buf,
                                                "Non-zero return code (%d) from MPI_Ibsend",
                                                ierr);
                                    ...
                                    if (ierr != MPI_SUCCESS) {
                                        sprintf(info_buf,
                                                "Non-zero return code (%d) from MPI_Test on Ibsend",
                                                ierr);
                                        MPITEST_message(MPITEST_NONFATAL,
                                                        info_buf);
                                        MPI_Error_string(ierr,
