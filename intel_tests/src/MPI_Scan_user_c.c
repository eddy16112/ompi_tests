...
-----------------------------------------------------------------------------*/
/******************************************************************************
                         Test of MPI_Scan_user()

This code tests the functionality of the MPI_Scan() funtion for user-defined
operations.

This function acts much like a Reduce_scatter(), except the reduced data which
arrives at process "i" has only been reduced with data from processes
0, 1, 2, ..., i.

This test initializes the ith processes send data with "i".  Then the Scan is
performed, then the data is checked.  The checking makes use of the auxiliary
funciton "get_reduce_answer()", which may be found in module reduce.c .
...
int main(int argc, char *argv[])
{
    ...
    *recv_buffer;
    ...
    sprintf(testname, "MPI_Scan_user()");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                /* Prevent overflow especially for character sized datatypes */
                ...
                for (length_count = 0;
                    ...
                    for (op_count = 0; op_count < num_ops; op_count++) {
                        ...
                        if (has_op(op_count, test_type) && (length != 0)) {
                            loop_cnt++;
                            ierr =
                                MPI_Scan(send_buffer, recv_buffer, length,
                                         MPITEST_mpi_datatypes[test_type],
                                         MPITEST_default_ops[op_count],
                                         comm);

                            if (ierr != MPI_SUCCESS) {
                                sprintf(info_buf, "MPI_Scan() return %d",
                                        ierr);
                            ...
                            /*
                             * generate the correct answer, recalling that this
                             * process only reduces with processes of lower rank
                             */
                            get_reduce_answer(op_count,
                                              (MPITEST_current_rank > max_rank) ? max_val : MPITEST_current_rank + 1,
                                              &value);
                            ...
                            if (error) {
                                if (ierr == MPI_SUCCESS)
                                    fail++;
                                sprintf(info_buf,
                                        "%d errors in buffer, length %d commsize %d commtype %d data_type %d operation %d",
