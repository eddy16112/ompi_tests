...
-----------------------------------------------------------------------------*/
/******************************************************************************
		  Test for MPI_Comm_get_name()

This test calls MPI_Comm_get_name() on each rank and prints it.
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Comm_get_name");
    ...
    if (MPITEST_me == 0) {
        ...
        MPITEST_message(MPITEST_VERIFY,
                        "Please verify the following communicator names");
    }
    ...
    ierr = MPI_Comm_get_name(MPI_COMM_WORLD, name, &len);
    for (i = 0; i < MPITEST_nump; i++) {
        if (i == MPITEST_me) {
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_get_name",
                        ierr);
                ...
                if ((len > MPI_MAX_OBJECT_NAME)
                    ...
                    sprintf(info_buf,
                            "Returned string length=%d (OK), communicator name:",
                            len);
                    MPITEST_message(MPITEST_VERIFY, info_buf);
                    sprintf(info_buf, "\"%s\"", name);
                    MPITEST_message(MPITEST_VERIFY, info_buf);
                }
            }
        }
    ...
    ++loop_cnt;
    ierr = MPI_Comm_get_name(MPI_COMM_SELF, name, &len);
    for (i = 0; i < MPITEST_nump; i++) {
        if (i == MPITEST_me) {
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
    ...
    MPITEST_report(loop_cnt - fail, fail, 2, testname);
