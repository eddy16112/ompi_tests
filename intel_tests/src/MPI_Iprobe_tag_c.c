...
int main(int argc, char *argv[])
{
    ...
    *values;                    /*  Array of dataTemplates for verbose init           */
    ...
    MPI_Request *recv_request, send_request[MPITEST_MAX_RANKS][MPITEST_MAX_TAG];
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (root = 0; root < test_nump; root++) {
                ...
                if (MPITEST_current_rank != root) {
                    ...
                    do {
                        ierr =
                            MPI_Iprobe(root, MPI_ANY_TAG, comm, &flag,
                                       &status);
                        if (ierr != MPI_SUCCESS) {
                            fail++;
                            sprintf(info_buf, "MPI_Iprobe() returned %d",
                                    ierr);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            MPI_Error_string(ierr, &info_buf[0], &size);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                    ...
                    for (tag = maxtag - 1; tag >= 0; tag--) {
                        ...
                        do {
                            ierr =
                                MPI_Iprobe(root, tag, comm, &flag,
                                           &status);
                            if (ierr != MPI_SUCCESS) {
                                fail++;
                                sprintf(info_buf,
                                        "MPI_Iprobe() returned %d", ierr);
