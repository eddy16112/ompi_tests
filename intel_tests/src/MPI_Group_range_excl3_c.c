...
other rank of MPI_COMM_WORLD, with exclusion starting at the last rank.
Multiple ranges are excluded if there are enough ranks.
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Group_range_excl3");
    ...
    if (worldsize == 2)         /* only one range if 2 ranks */
        ...
        if (worldsize < 5)      /* only two ranges if 3 or 4 ranks */
            ...
            ranges[2][1] = 1;
            ranges[2][2] = 10;
            ierr = MPI_Group_range_excl(groupworld, 3, ranges, &group);
        }
    }
