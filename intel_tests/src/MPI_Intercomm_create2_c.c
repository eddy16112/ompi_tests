...
int main(int argc, char *argv[])
{
    int
     ...
     result, resultsplit, resultinter, buffer1, buffer2, buffer3, size;
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            if (MPITEST_current_rank == 1) {
                if (commsplit != MPI_COMM_NULL) {
                    fail++;
                    sprintf(info_buf,
                            "MPI_Comm_split w/ color = MPI_UNDEFINED returned %d, expected MPI_COMM_NULL (comm_index %d)",
                            comm, comm_index);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                }
            } else {
                ...
                if (MPITEST_current_rank == 0)
                    ...
                    ierr =
                        MPI_Intercomm_create(commsplit, test_nump - 3,
                                             comm, 0, 65534, &comminter);
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
