...
int main(int argc, char *argv[])
{
    int
     ...
     result, resultsplit, resultmerge, buffer1, buffer2, buffer3, size;
    ...
    sprintf(testname, "MPI_Intercomm_merge2");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            if (MPITEST_current_rank == 1) {
                ...
                if (MPITEST_current_rank == 0)  /* high = TRUE */
                    ierr = MPI_Intercomm_merge(comminter, 1, &comm2);
                else            /* high = FALSE */
                    ierr = MPI_Intercomm_merge(comminter, 0, &comm2);
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
                            "Non-zero return code (%d) from MPI_Intercomm_merge (comm_index %d)",
                            ierr, comm_index);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_FATAL, info_buf);
                ...
                if (resultmerge != test_nump - 1) {
                    sprintf(info_buf,
                            ...
                            resultmerge, test_nump - 1, comm_index);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    fail++;
                }
                loop_cnt++;
                ...
                if (resultmerge == 1)
                    result = -35;
                ierr = MPI_Bcast(&result, 1, MPI_INT, 1, comm2);
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
