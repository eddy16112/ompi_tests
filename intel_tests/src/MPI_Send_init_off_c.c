...
-----------------------------------------------------------------------------*/
/******************************************************************************
 Test for MPI_Send_Init(): Persistent root send to all with byte offsets

 Receive with MPI_Recv_init()

 Persistent I/O version of MPI_Isend_off
...
/*------------------------------  MPI_Init  ---------------------------------*/
    ...
    sprintf(testname,
            "MPI_Send_init_off: persistent send with character OFFset");
...
-----------------------------------------------------------------------------*/
                if (MPITEST_me != root) {
                    ...
                    ierr = MPI_Recv_init(recv_buffer + recv_off,
                                         length + 1,
                                         MPITEST_mpi_datatypes[test_type],
                                         root, root, comm, &recv_request);
                    ...
                    if (ierr != MPI_SUCCESS) {
                        sprintf(info_buf,
                                "Non-zero return code (%d) from MPI_Wait receive",
                                ierr);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        MPI_Error_string(ierr, &info_buf[0], &size);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
...
-----------------------------------------------------------------------------*/
                ...
                if (MPITEST_me == root) {
                    ...
                    for (i = 0; i < MPITEST_nump; i++) {
                        ...
                        ierr = MPI_Send_init(send_buffer + send_off,
                                             length,
                                             MPITEST_mpi_datatypes
                                             [test_type], i, root, comm,
                                             &send_request);
