...
int main(int argc, char *argv[])
{
    int
     ...
     ranges[3][3], worldsize, worldrank, newsize, result, expect, size, i;
    ...
    sprintf(testname, "MPI_Group_range_excl1");
    ...
    if (worldsize > 2) {
        ...
        ranges[0][1] = 1;
        ranges[0][2] = -1;
        ierr = MPI_Group_range_excl(groupworld, 1, ranges, &group);
    } else
        ierr = MPI_Group_range_excl(groupworld, 0, ranges, &group);

    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Group_range_excl #1",
                ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
    ...
    if ((worldsize <= 2) && (result != MPI_IDENT) ||
        ...
        sprintf(info_buf, "MPI_Group_compare returned incorrect value %d",
                result);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    }
    ...
    if (worldsize == 1)
        ...
        ierr = MPI_Group_range_excl(groupworld, 2, ranges, &group);
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Group_range_excl #2",
                ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
    ...
    if (group != MPI_GROUP_EMPTY) {
        sprintf(info_buf,
                "MPI_Group_range_excl of size 0 did not return MPI_GROUP_EMPTY)");
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    }
    ...
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Group_size(empty)",
                ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
    ...
    if (result != 0) {
        sprintf(info_buf, "MPI_Group_size(empty) returned %d, expected %d",
                result, 0);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    }
    ...
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Group_rank(empty)",
                ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
    ...
    if (result != MPI_UNDEFINED) {
        sprintf(info_buf,
                "MPI_Group_rank(empty) returned %d, expected MPI_UNDEFINED",
                result);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    }
    ...
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Group_range_excl #3",
                ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
    ...
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Group_size(all)",
                ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
    ...
    if (result != worldsize) {
        sprintf(info_buf, "MPI_Group_size(all) returned %d, expected %d",
                result, newsize);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    }
    ...
    loop_cnt++;
    if (result != worldrank) {
        sprintf(info_buf, "MPI_Group_rank(all) returned %d, expected %d",
                result, worldrank);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    }
