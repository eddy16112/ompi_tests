...
1).
The root rank will create and start a persistent receive request to the
next rank (circular). The root rank will then do a MPI_Cancel()
followed by MPI_Wait() for the request to be completed.  It then call
MPI_Test_cancelled() to verify that the send request has been cancelled.
All participating ranks will then do a MPI_Barrier().

NOTE: This test assumes that an outstanding Recv_init request can
be successfully cancelled if no matching send has yet been attempted.
2).
If things goes well with the first phase of the test, the root rank will
then make another irecv request using the same user buffer
and request handle.  The next rank (circular) will do a MPI_Send() to send
the data.  The received data will be
verified.  Depending on whether MPITEST_STATUS_CHK is #defined or not,
the status object returned by the MPI_Irecv() may be checked.
...
MPI Calls dependencies for this test:
  MPI_Send), MPI_Cancel(), MPI_Test_cancelled(), MPI_Irecv(), MPI_Init(),
  MPI_Finalize(), MPI_Comm_test_inter(), MPI_Barrier(), MPI_Error_string()
  MPI_Wait(), MPI_Recv_init()
...
int main(int argc, char *argv[])
{
    int
     ...
     source,                    /*  source rank of where message is being sent from   */
    ...
    *values;                    /*  Array of dataTemplates for verbose init           */
    ...
    void *buffer2;              /* message buffer for test second phase    */
    ...
    sprintf(testname, "MPI_Cancel_persist_recv");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            MPITEST_get_buffer(test_type,
                               2 * max_length + MPITEST_MIN_BUFSIZE,
                               &buffer);
            MPITEST_get_buffer(test_type, max_length + MPITEST_MIN_BUFSIZE,
                               &buffer2);
            ...
            for (length_count = 0;
                ...
                for (root = 0; root < test_nump; root++) {
                    ...
                    if (MPITEST_current_rank == 0) {
                        sprintf(info_buf,
                                "(%d, %d) length %d commsize %d commtype %d data_type %d root %d",
                    ...
                    /* Source rank the message is being sent from */
                    source = root + 1;
                    ...
                    if (MPITEST_current_rank == root) {
                        ...
                        sprintf(info_buf,
                                "Constructing receive request from %d, tag: %d",
                                source, tag);
                        MPITEST_message(MPITEST_INFO1, info_buf);

                        ierr =
                            MPI_Recv_init(buffer, length,
                        ...
                        sprintf(info_buf,
                                "Starting receive request from %d, tag: %d",
                                source, tag);
                        MPITEST_message(MPITEST_INFO1, info_buf);
                        ...
                        if (ierr != MPI_SUCCESS) {
                            fail++;
                            sprintf(info_buf, "MPI_Start() returned %d",
                                    ierr);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            MPI_Error_string(ierr, &info_buf[0], &size);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                            ...
                            sprintf(info_buf,
                                    "Cancelling persistent receive request ...");
                            MPITEST_message(MPITEST_INFO1, info_buf);

                            ierr = MPI_Cancel(&recv_request);

                            if (ierr != MPI_SUCCESS) {
                                fail++;
                                sprintf(info_buf,
                                        "MPI_Cancel() returned %d", ierr);
                                ...
                                sprintf(info_buf,
                                        "Waiting for receive request to be cancelled");
                                MPITEST_message(MPITEST_INFO1, info_buf);
                                ...
                                sprintf(info_buf,
                                        "Checking if the receive request has been cancelled");
                                MPITEST_message(MPITEST_INFO1, info_buf);
                                ...
                                if (ierr != MPI_SUCCESS) {
                                    ...
                                    fail++;
                                    sprintf(info_buf,
                                            "MPI_Test_cancelled() return zero flag with a cancelled request");
                                    MPITEST_message(MPITEST_NONFATAL,
                                                    info_buf);
                                }
                            }
                    ...
                    if (MPITEST_current_rank == root) {
                        ...
                        sprintf(info_buf,
                                "Receiving message 2 from %d, tag: %d",
                                source, tag);
                        MPITEST_message(MPITEST_INFO1, info_buf);

                        ierr =
                            MPI_Irecv(buffer, length,
                                      MPITEST_mpi_datatypes[test_type],
                                      source, tag, comm, &recv_request);
                        ...
                        if (ierr != MPI_SUCCESS) {
                            fail++;
                            sprintf(info_buf, "MPI_Wait() returned %d",
                                    ierr);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            MPI_Error_string(ierr, &info_buf[0], &size);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                            ...
                            if (error) {
                                if (ierr == MPI_SUCCESS)
                                    fail++;
                                sprintf(info_buf,
                                        "%d errors in buffer (%d) len %d commsize %d commtype %d data_type %d root %d",
                                        error, comm_count, length,
                            ...
                            if (status.MPI_SOURCE != source) {
                                fail++;
                                sprintf(info_buf,
                                        "status object returned from MPI_Wait() has unexpected MPI_SOURCE (%d) field",
                                        status.MPI_SOURCE);
                                MPITEST_message(MPITEST_NONFATAL,
                                                info_buf);
                            }
                            ...
                            if (status.MPI_TAG != tag) {
                                fail++;
                                sprintf(info_buf,
                                        "status object returned from MPI_Wait() has unexpected MPI_TAG"
                                        "  Ecpected: %d, Actual: %d", tag,
                                        status.MPI_TAG);
                                MPITEST_message(MPITEST_NONFATAL,
                                                info_buf);
                            }
                            ...
                            if (ierr != MPI_SUCCESS) {
                                ...
                                if (count != length) {
                                    fail++;
                                    sprintf(info_buf,
                                            "status object returned from MPI_Wait() contains unexpected length."
                                            "  Expected: %d, Actual: %d",
                                            length, count);
                                    MPITEST_message(MPITEST_NONFATAL,
                                                    info_buf);
                    ...
                    } else if (MPITEST_current_rank == source) {
                        sprintf(info_buf,
                                "Receiving message from source %d to %d, tag: %d",
                                root, source, tag);
                        MPITEST_message(MPITEST_INFO1, info_buf);
