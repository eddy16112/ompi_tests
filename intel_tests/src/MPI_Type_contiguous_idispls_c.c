...
(in mpitest_cfg.h) using MPI_Type_contiguous().  The root's rank will then send
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Type_contiguous_inner_displs");
            ...
            for (length_count = 0;
                ...
                for (type_count = 0; type_count < num_types; type_count++) {
                    ...
                    if (test_type < MPITEST_derived1) {
                        ...
                        ierr =
                            MPI_Type_extent(types[basic_types], &extent);
                        if (ierr != MPI_SUCCESS) {
                            fail++;
                            sprintf(info_buf,
                                    "MPI_Type_extent() returned %d", ierr);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            MPI_Error_string(ierr, &info_buf[0], &size);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                        }
                        ...
                        if (basic_types == 0) {
                            ...
                            displs[basic_types] = displs[basic_types - 1] +
                                type_sizes[basic_types - 1] + byte_length;
                        }
                ...
                MPITEST_get_buffer(MPITEST_byte,
                                   length * extent + MPITEST_ADD_BUFSIZE,
                                   (void *) &buffer);
#if defined (CUDA_TEST)
                ...
                ierr = MPI_Type_contiguous(length, oldtype, &newtype);
                if (ierr != MPI_SUCCESS) {
                    fail++;
                    sprintf(info_buf, "MPI_Type_contiguous() returned %d",
                            ierr);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_FATAL, info_buf);
                ...
                for (root = 0; root < test_nump; root++) {
                    ...
                    if (MPITEST_current_rank != root) {
                        for (i = 0;
                             i < length * extent + MPITEST_ADD_BUFSIZE;
                             i++) {
                            buffer[i] = -1 * (MPITEST_current_rank % 128);
                        }
                    } else {
                        for (i = 0; i < length * extent; i++) {
                            buffer[i] = 0;
                        }
...
#if defined (CUDA_TEST)
                    cudaMemcpy(buffer_gpu, buffer, length * extent + MPITEST_ADD_BUFSIZE, cudaMemcpyHostToDevice);
#endif
                    ...
                    if (MPITEST_current_rank == root) {
...
#if defined (CUDA_TEST)
                        cudaMemcpy(buffer, buffer_gpu, length * extent + MPITEST_ADD_BUFSIZE, cudaMemcpyDeviceToHost);
#endif
                        ...
                        for (i = 0; i < length; i++) {
                            ...
                            for (j = 0; j < basic_types; j++) {
                                ...
                                acc += type_sizes[j] + byte_length;
                                /* Do not verify space between inner types */
                            }
                        }
