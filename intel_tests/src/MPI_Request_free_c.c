...
int main(int argc, char *argv[])
{
    ...
    int
     isendbuf[1], issendbuf[1], irsendbuf[1], ibsendbuf[1], recvbuf[5];
    ...
    MPI_Status recv_stat[6], send_stat; /* MPI  status structure                         */

    MPI_Request recv_request[6],        /*  MPI request structure                        */
    ...
    if (MPITEST_me < 2) {
        ...
        if (MPITEST_me == 0) {  /* sender */
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Request_free on Isend",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (send_request != MPI_REQUEST_NULL) {
                sprintf(info_buf,
                        "Request not set to MPI_REQUEST_NULL by MPI_Request_free on Isend");
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Request_free on Ibsend",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (send_request != MPI_REQUEST_NULL) {
                sprintf(info_buf,
                        "Request not set to MPI_REQUEST_NULL by MPI_Request_free on Ibsend");
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Request_free on Issend",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (send_request != MPI_REQUEST_NULL) {
                sprintf(info_buf,
                        "Request not set to MPI_REQUEST_NULL by MPI_Request_free on Issend");
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Request_free on Irsend",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (send_request != MPI_REQUEST_NULL) {
                sprintf(info_buf,
                        "Request not set to MPI_REQUEST_NULL by MPI_Request_free on Irsend");
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
        ...
        {                       /* receiver */
            for (i = 1; i < 6; i++) {
                loop_cnt++;
                ierr = MPI_Irecv(&recvbuf[i], 1, MPI_INT, 0, i,
                                 MPI_COMM_WORLD, &recv_request[i]);
                if (ierr != MPI_SUCCESS) {
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    sprintf(info_buf,
                            "Non-zero return code (%d) from MPI_Irecv #%d",
                            ierr, i);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            for (i = 1; i < 5; i++) {
                ...
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
                            "Non-zero return code (%d) from MPI_Wait on Irecv #%d",
                            ierr, i);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            ierr = MPI_Request_free(&recv_request[5]);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Request_free on Irecv",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
