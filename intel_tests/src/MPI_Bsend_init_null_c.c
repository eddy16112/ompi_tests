...
-----------------------------------------------------------------------------*/
/***************************************************************************
	 Test for Persistent MPI_Bsend_init() to MPI_PROC_NULL

This code tests the persistent  MPI_Bsend_init/MPI_Start operation to
MPI_PROC_NULL.  No actual messages should be sent.
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname,
            "MPI_Bsend_init_null: Persistent all Bsend_init to MPI_PROC_NULL");
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                for (length_count = 0;
                    ...
                    /*-------------------------------------------------
	                 Persistent Bsend_init/MPI_Start
		         All nodes send a message to MPI_PROC_NULL
		    -------------------------------------------------*/
                    loop_cnt++;
                    ierr = MPI_Bsend_init(send_buffer,
                                          length,
 ...
 /*-------- Call MPI_Request_free -------*/
    ...
    /* Detached the buffer above, now free the dynamic memory */
