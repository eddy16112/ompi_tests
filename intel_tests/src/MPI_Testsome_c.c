...
-----------------------------------------------------------------------------*/
/******************************************************************************
*                          Functional test for MPI_Testsome
*
*  Testsome references:
...
*  the Request Objects are freed.  This test Does a Testsome on messages
*  that have already been Testsomeed on.  The code verifies that a successful
...
*         message count of MPI_UNDEFINED
******************************************************************************/

#define  PROG_NAME   "MPI_Testsome"
...
int main(int argc, char *argv[])
{
    ...
    MPI_Status temp_stat[4 * NUMMESG],  /* Recv Source/Tag information               */
    ...
    if (MPITEST_me == 0) {
        ...
        while (send_cnt != MPI_UNDEFINED) {
            ...
            ierr =
                MPI_Testsome(4 * NUMMESG, send_request, &send_cnt,
                             send_index, temp_stat);
            send_loop_cnt++;
            if (send_cnt > 0) {
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code ierr/flag (%d/%d) from MPI_Testsome on Isend, after %d calls to MPI_Testsome",
                        ierr, flag, send_loop_cnt);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            ...
            sprintf(info_buf,
                    "Testsome on send: Loop  %d   Send_cnt  %d   Error %d",
                    i, send_cnt, ierr);
 ...
 /*---------------------------------  Irecv  -----------------------------*/
    if (MPITEST_me == 1) {
        ...
        while (recv_cnt != MPI_UNDEFINED) {
            ...
            ierr =
                MPI_Testsome(4 * NUMMESG, recv_request, &recv_cnt,
                             recv_index, temp_stat);
            recv_loop_cnt++;

            sprintf(info_buf,
                    "Testsome on receive: Loop %d  recv_cnt %d  Error %d ",
                    recv_loop_cnt, recv_cnt, ierr);
            MPITEST_message(MPITEST_INFO1, info_buf);
            ...
            if (ierr != MPI_SUCCESS) {
                fail++;
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Testsome on receive   count = %d  loops   ",
                        ierr, recv_cnt);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                ...
                if (errorclass == MPI_ERR_IN_STATUS) {
                    sprintf(info_buf,
                            "MPI_ERR_IN_STATUS on receive, printing non-zero statuses");
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    ...
                    for (j = 0; j < 4 * NUMMESG; j++) {
                        ...
                        if (ierr != MPI_SUCCESS) {
                            sprintf(info_buf,
                                    "Error in received record %d, Source = %d,  Tag =  %d,  Error # =  %d",
                                    j, recv_stat[j].MPI_SOURCE,
                                    recv_stat[j].MPI_TAG,
                                    recv_stat[j].MPI_ERROR);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            MPI_Error_string(recv_stat[j].MPI_ERROR,
                                             &info_buf[0], &size);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            fail++;
                        }
        ...
        ierr =
            MPI_Testsome(4 * NUMMESG, recv_request, &recv_cnt, recv_index,
                         recv_stat);

        total_cnt++;
        if (ierr != MPI_SUCCESS) {
            sprintf(info_buf,
                    "Non-zero return code (%d) from second MPI_Testsome on receive",
                    ierr);
            MPITEST_message(MPITEST_NONFATAL, info_buf);
        ...
        sprintf(info_buf,
                "Testsome on receive: Loop %d  recv_cnt %d  Error %d ", i,
                recv_cnt, ierr);
        MPITEST_message(MPITEST_INFO1, info_buf);

        if (recv_cnt != MPI_UNDEFINED) {
            sprintf(info_buf,
                    "Second MPI_Testsome on receive has indicated %d active messages and there should be none",
                    recv_cnt);
            MPITEST_message(MPITEST_NONFATAL, info_buf);
            fail++;
        }
        ...
        for (i = 0; i < 4 * NUMMESG; i++) {
            if (recv_request[i] != MPI_REQUEST_NULL) {
                sprintf(info_buf,
                        "recv_request[%d] not equal to  MPI_REQUEST_NULL(%p/%p) after second Testsome",
                        i, recv_request[i], MPI_REQUEST_NULL);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
