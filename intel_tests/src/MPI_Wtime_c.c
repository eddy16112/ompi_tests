...
int main(int argc, char *argv[])
{
    ...
    if (MPITEST_me == 0) {
        ...
        MPITEST_message(MPITEST_VERIFY,
                        "Verify value of MPI_Wtick below:");
    }
    ...
    if (time1 < 0) {
        sprintf(info_buf, "MPI_Wtime() returned a negative number %f",
                time1);
    ...
    if (time2 < time1) {
        sprintf(info_buf,
                "Successive calls to MPI_Wtime() returned %f, %f, expected incrementing values",
                time1, time2);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    }
    ...
    if (MPITEST_me == 0)
        MPITEST_message(MPITEST_VERIFY,
                        "MPI_Wtime(); sleep(5); MPI_Wtime() returned:");
