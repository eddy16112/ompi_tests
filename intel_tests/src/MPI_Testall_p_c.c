...
-----------------------------------------------------------------------------*/
/******************************************************************************
*           Functional test for Persistent MPI_Testall
*
*  Testall references:
...
#define  PROG_DESC   "Persistent Testall on two nodes"
...
int main(int argc, char *argv[])
{
    ...
    if (MPITEST_me == 0) {
        ...
        while (!flag && !ierr) {
            ierr =
                MPI_Testall(4 * NUMMESG, send_request, &flag, send_stat);
            send_loop_cnt++;
        }
        if (ierr != MPI_SUCCESS) {
            sprintf(info_buf,
                    "Non-zero return code ierr/flag (%d/%d) from MPI_Testall on Isend, after %d calls to MPI_Testall",
 ...
 /*--------------------------------- Receives  ---------------------------*/
    if (MPITEST_me == 1) {
        ...
        while (!flag && !ierr) {
            ierr =
                MPI_Testall(4 * NUMMESG, recv_request, &flag, recv_stat);
            recv_loop_cnt++;
        }

        if (ierr != MPI_SUCCESS) {
            sprintf(info_buf,
                    "Non-zero return code ierr/flag (%d/%d) from MPI_Testall on Irecv, after %d calls to MPI_Testall on Receive",
