...
int main(int argc, char *argv[])
{
    ...
    MPI_Status recv_stat[NUMSHORT + 1], /* MPI  status structure                     */
        send_stat[NUMSHORT + 1];

    MPI_Request recv_request[NUMSHORT + 1],     /* MPI request structure             */
        send_request[NUMSHORT + 1];
    ...
    sprintf(testname, "MPI_Startall1");
    ...
    if (MPITEST_me < 2) {
        ...
        if (MPITEST_me == 0) {  /* sender */
            ...
            ierr = MPI_Bsend_init(&buffer,
                                  LONGLENGTH,
                                  MPI_CHAR,
                                  1, 1, MPI_COMM_WORLD, &send_request[0]);
            ...
            for (i = 0; i < NUMSHORT; i++) {
                ...
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
                            "Non-zero return code (%d) from MPI_Bsend_init(2)",
                            ierr);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            ierr = MPI_Startall(NUMSHORT + 1, &send_request[0]);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Startall",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            /* Start Error Test  */
            ierr =
                MPI_Waitall(NUMSHORT + 1, &send_request[0], &send_stat[0]);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Waitall on bsend",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            ierr = MPI_Recv_init(&buffer,
                                 LONGLENGTH,
                                 MPI_CHAR,
                                 0, 1, MPI_COMM_WORLD, &recv_request[0]);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Recv_init(1)",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            for (i = 0; i < NUMSHORT; i++) {
                ierr = MPI_Recv_init(&shortb[i],
                                     ...
                                     0,
                                     1,
                                     MPI_COMM_WORLD, &recv_request[i + 1]);
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
                            "Non-zero return code (%d) from MPI_Recv_init(2)",
                            ierr);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            ierr = MPI_Startall(NUMSHORT + 1, &recv_request[0]);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Startall on recv",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            /* Start recv Error Test  */
            ierr =
                MPI_Waitall(NUMSHORT + 1, &recv_request[0], &recv_stat[0]);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Waitall on recv",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
