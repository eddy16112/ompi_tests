...
-----------------------------------------------------------------------------*/
/******************************************************************************
		  Test for MPI_Irsend() to MPI_PROC_NULL

This tests the nonblocking MPI_Irsend operation to MPI_PROC_NULL.  No
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Irsend_null: all Irsend to MPI_PROC_NULL");
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
		    ...
		    -------------------------------------------------*/
                    ...
                    if (ierr != MPI_SUCCESS) {
                        ...
                        sprintf(info_buf,
                                "Non-zero return code (%d) from MPI_Irsend",
                                ierr);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                    ...
                    if (ierr != MPI_SUCCESS) {
                        sprintf(info_buf,
                                "Non-zero return code (%d) from MPI_Wait on Irsend",
                                ierr);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        MPI_Error_string(ierr, &info_buf[0], &size);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        if (send_stat.MPI_ERROR != MPI_SUCCESS) {
                            MPI_Error_string(send_stat.MPI_ERROR,
                                             &info_buf[0], &size);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                        }
                        fail++;
