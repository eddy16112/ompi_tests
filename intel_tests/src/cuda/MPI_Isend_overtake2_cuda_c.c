...
int main(int argc, char *argv[])
{
    ...
    if (MPITEST_me < 2) {
        ...
        if (MPITEST_me == 0) {  /* sender */
            ...
            ierr = MPI_Isend((void *)gpu_buffer, LONGLENGTH, MPI_CHAR, 1, 1,
            ...
            for (i = 0; i < NUMSHORT; i++) {
                ...
                ierr = MPI_Isend((void *)gpuptr, 1, MPI_CHAR, 1, i + 2,
                                 MPI_COMM_WORLD, &send_request);
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
            ...
            for (i = 0; i < NUMSHORT; i++) {
                gpuptr = (CUdeviceptr)((char *)gpu_shortb + NUMSHORT - i -1);
                ierr = MPI_Irecv((void *)(void *)gpuptr, 1, MPI_CHAR,
                                 0, NUMSHORT - i + 1,
            ...
            ierr = MPI_Irecv((void *)gpu_buffer, LONGLENGTH, MPI_CHAR, 0, 1,
