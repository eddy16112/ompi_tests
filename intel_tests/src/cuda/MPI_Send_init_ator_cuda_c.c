...
-----------------------------------------------------------------------------*/
/******************************************************************************
       MPI_Send_init Persistent Send:  All send to Root test

                      MPI_Send_init():  All others Send TO Root
                      with MPI_Start    to initiate I/O
                      with MPI_Wait     to check completion
                      and  MPI_Recv_init():  Any source/Any tag

This test is an All send to Root type test with persistent communications.
One receive initialization is done for the root to receive from all other
nodes.
...
{
    int
     flag = 0,                  /* flag for completion of MPI_Test               */
     ...
     senders = 0,               /* INTER-comm #senders for this send/rec choice  */
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        /* set up error handler to return error code when error occurrs
           Change from default of MPI_ERRORS_ARE_FATAL to MPI_ERRORS_RETURN */
        ...
        if (ierr != MPI_SUCCESS) {
            ...
            sprintf(info_buf,
                    "ierr = %d  from Errhandler_set:  comm_count  %d    comm_index %d   comm_type  %d   test_nump ",
                    ierr, comm_count, comm_index, comm_type);
            MPITEST_message(MPITEST_NONFATAL, info_buf);
            loop_fail++;
...
/*  Start the Receive */
                                    ...
                                    if (ierr != MPI_SUCCESS) {
                                        sprintf(info_buf,
                                                "Non-zero return code  ierr/flag =(%d/%d) from MPI_Wait on receive",
                                                ierr, flag);
                                        MPITEST_message(MPITEST_NONFATAL,
                                                        info_buf);
                                        MPI_Error_string(ierr,
 ...
 /*-------------------------------------------------
			            Post Send_init followed by Start
        ...
        /*--------------------- Call MPI_Request_free -----------------------*/
                                ...
                                if (ierr != MPI_SUCCESS) {
                                    sprintf(info_buf,
                                            "Non-zero return code (%d) from MPI_Request_free on send",
                                            ierr);
                                    MPITEST_message(MPITEST_NONFATAL,
                                                    info_buf);
                                    MPI_Error_string(ierr, &info_buf[0],
