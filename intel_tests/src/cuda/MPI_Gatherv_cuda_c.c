        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                MPITEST_get_buffer(test_type, (max_length + EXTRA_SPACE),
                                   &send_buffer);
                CUDATEST_get_gpu_buffer(test_type, (max_length + EXTRA_SPACE),
                                        &gpu_send_buffer, &gpu_send_buffer_size);
                MPITEST_get_buffer(test_type,
                                   (max_length +
                                    EXTRA_SPACE) * MPITEST_nump,
                                   &recv_buffer);
                CUDATEST_get_gpu_buffer(test_type,
                                        (max_length +
                                        EXTRA_SPACE) * MPITEST_nump,
                ...
                for (length_count = 0;
                    ...
                    for (root = 0; root < test_nump; root++) {
                        ...
                        ierr = MPI_Gatherv((void *)gpu_send_buffer, length + j,
                                           MPITEST_mpi_datatypes
                                           [test_type], (void *)gpu_recv_buffer,
                        ...
                        if (ierr != MPI_SUCCESS) {
                            fail++;
                            sprintf(info_buf, "MPI_Gatherv() returned %d",
                                    ierr);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            MPI_Error_string(ierr, &info_buf[0], &size);
                            MPITEST_message(MPITEST_FATAL, info_buf);
