...
-----------------------------------------------------------------------------*/
/******************************************************************************
		  Test for MPI_Send():  ROOT send to ALL version
                       and MPI_Recv():  Specified source/tag


Tests the basic blocking MPI_Send operation, with both Intra/Inter
communicators, as well as MPI_Recv with specified source and tags.
...
int main(int argc, char *argv[])
{
    ...
    *ub,                        /*  Value of maximum tag                              */
    ...
    sprintf(testname, "MPI_Send_rtoa: Root sends TO All");
    ...
    ierr = MPI_Comm_get_attr(MPI_COMM_WORLD, MPI_TAG_UB, &ub, &found);
    if (ierr != MPI_SUCCESS) {
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
			    ...
			    -------------------------------------------------*/
                            ...
                            if ((!inter_comm
                                ...
                                ierr = MPI_Recv((void *)gpu_recv_buffer, length,
			    ...
			    -------------------------------------------------*/
                                    ...
                                    && MPITEST_current_rank == root))
                            {
                                ...
                                for (i = 0; i < send_to; i++) {
                                    ...
                                    ierr = MPI_Send((void *)gpu_send_buffer, length,
                                                    MPITEST_mpi_datatypes
                                                    [test_type], i,
                                                    *ub - root, comm);
