...
int main(int argc, char *argv[])
{
    ...
    if (MPITEST_me < 2) {
        ...
        if (MPITEST_me == 0) {  /* sender */
            ...
            ierr = MPI_Bsend_init((void *)gpu_buffer,
                                  LONGLENGTH,
                                  MPI_CHAR,
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Start(1)",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            /* Start Error Test  */
            ierr = MPI_Bsend_init((void *)gpu_shortb,
                                  1,
                                  MPI_CHAR,
                                  1, 1, MPI_COMM_WORLD, &send_request[1]);
            ...
            for (i = 0; i < NUMSHORT; i++) {
                ...
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
                            "Non-zero return code (%d) from MPI_Start(2)",
                            ierr);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                ...
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
                            "Non-zero return code (%d) from MPI_Wait(2) on bsend",
                            ierr);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Wait(1) on bsend",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            ierr = MPI_Recv_init((void *)gpu_buffer,
                                 LONGLENGTH,
                                 MPI_CHAR,
            ...
            for (i = 0; i < NUMSHORT; i++) {
                ...
                ierr = MPI_Recv_init((void *)gpuptr,
                                     1,
                                     MPI_CHAR,
                                     0,
                                     1,
