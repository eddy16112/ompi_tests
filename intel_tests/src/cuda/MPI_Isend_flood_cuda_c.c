...
int main(int argc, char *argv[])
{
    ...
    CUdeviceptr gpu_irecv_buff;

    char
     info_buf[256],             /* buffer for passing mesages to MPITEST         */
     testname[64];              /* the name of this test                         */

    MPI_Request recv_req, send_request;
    ...
    EXIT_ON_ERROR(cuMemAlloc(&gpu_irecv_buff, sizeof(irecv_buff)));
    ...
    if (MPITEST_me == 0) {
        ...
        for (loop_cnt = 0; loop_cnt < LOOP_SIZE; loop_cnt++) {
            ierr = MPI_Irecv((void *)gpu_irecv_buff, 0,
    ...
    /* rank 0 waits for message */
	    ...
	    -------------------------------------------------*/
            ...
            for (loop_cnt = 0; loop_cnt < LOOP_SIZE; loop_cnt++) {
                ...
                ierr = MPI_Isend((void *)gpu_send_buffer,
                                 max_length,
                                 MPITEST_mpi_datatypes[test_type],
                                 2,
                                 loop_cnt, MPI_COMM_WORLD, &send_request);
                ...
                ierr = MPI_Isend((void *)gpu_send_buffer,
                                 0,
                                 MPI_INT,
                                 0, 1, MPI_COMM_WORLD, &send_request);
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
            ...
            for (loop_cnt = 0; loop_cnt < LOOP_SIZE; loop_cnt++) {
                ...
                ierr = MPI_Recv((void *)gpu_recv_buffer,
                                max_length,
                                MPITEST_mpi_datatypes[test_type],
