...
int main(int argc, char *argv[])
{
    ...
    *recv_counts,               /* Number of messages from each sender   */
    ...
    MPI_Request recv_request, send_request;     /*  MPI request structure               */
    ...
    sprintf(testname, "MPI_Isend_fairness");
    ...
    EXIT_ON_ERROR(cuMemAlloc(&gpu_buffer, sizeof(buffer)));
    ...
    if (MPITEST_me == 0) {
        ...
        for (i = 1; i < MPITEST_nump; i++) {
            ...
            MPITEST_message(MPITEST_INFO0, info_buf);
            for (j = 1; j < MPITEST_nump; j++) {
                sprintf(info_buf, " %3d from rank %3d (%3d%%)",
                        recv_counts[j], j,
                        (100 * recv_counts[j]) / NUM_MSG);
                MPITEST_message(MPITEST_INFO0, info_buf);
                recv_counts[j] = 0;
            }
        }
        ...
        for (i = 1; i <= NUM_MSG; i++) {
            MPI_Isend((void *)gpu_buffer,
                      MSG_SIZE,
                      MPI_INT, 0, i, MPI_COMM_WORLD, &send_request);
