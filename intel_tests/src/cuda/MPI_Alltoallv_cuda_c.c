    ...
    *recv_buffer;
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            values =
                (struct dataTemplate *) malloc(test_nump *
                                               sizeof(struct
                                                      dataTemplate));

            if (!send_counts || !send_displs || !recv_counts
                || !recv_displs || !values) {
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                MPITEST_get_buffer(test_type,
                                   MPITEST_nump * (max_length +
                                                   (LENGTH_FACTOR +
                                                    2) * MPITEST_nump),
                                   &recv_buffer);
                CUDATEST_get_gpu_buffer(test_type, MPITEST_nump * (max_length +
                                                   (LENGTH_FACTOR +
                                                    2) * MPITEST_nump),
                                        &gpu_recv_buffer, &gpu_recv_buffer_size);
                CUDATEST_get_gpu_buffer(test_type, MPITEST_nump * (max_length +
                                                   (LENGTH_FACTOR +
                                                    1) * MPITEST_nump),
                                        &gpu_send_buffer, &gpu_send_buffer_size);
                ...
                for (length_count = 0;
                    ...
                    if (MPITEST_current_rank == 0) {
                        sprintf(info_buf,
                                "length %d commsize %d commtype %d data_type %d",
                    ...
                    MPITEST_init_buffer_v(test_type, test_nump,
                                          send_counts, send_displs, values,
                                          send_buffer);
                    ...
                    MPITEST_init_buffer(test_type,
                                        (recv_counts[test_nump - 1] +
                                         recv_displs[test_nump - 1]),
                                        value, recv_buffer);
                    ...
                    loop_cnt++;
                    ierr =
                        MPI_Alltoallv((void *)gpu_send_buffer, send_counts,
                                      send_displs,
                                      MPITEST_mpi_datatypes[test_type],
                                      (void *)gpu_recv_buffer, recv_counts,
                                      recv_displs,
                                      MPITEST_mpi_datatypes[test_type],
                                      comm);

                    if (ierr != MPI_SUCCESS) {
                        sprintf(info_buf, "MPI_Alltoallv() returned %d",
                                ierr);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        MPI_Error_string(ierr, &info_buf[0], &size);
                        MPITEST_message(MPITEST_FATAL, info_buf);
                    ...
                    if (error) {
                        if (ierr == MPI_SUCCESS)
                            fail++;
                        sprintf(info_buf,
                                "%d errors in buffer, len %d commsize %d commtype %d data_type %d",
