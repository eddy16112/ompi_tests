        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                MPITEST_get_buffer(test_type, max_length * MPITEST_nump,
                                   &send_buffer);
                CUDATEST_get_gpu_buffer(test_type, max_length,
                                        &gpu_recv_buffer, &gpu_recv_buffer_size);
                CUDATEST_get_gpu_buffer(test_type, max_length * MPITEST_nump,
                ...
                for (length_count = 0;
                    ...
                    for (root = 0; root < test_nump; root++) {
                        ...
                        loop_cnt++;
                        ierr = MPI_Scatter((void *)gpu_send_buffer, length,
                        ...
                        if (ierr != MPI_SUCCESS) {
                            sprintf(info_buf, "MPI_Scatter() returned %d",
                                    ierr);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            MPI_Error_string(ierr, &info_buf[0], &size);
                            MPITEST_message(MPITEST_FATAL, info_buf);
