...
int main(int argc, char *argv[])
{
    ...
    if (MPITEST_me < 2) {
        ...
        if (MPITEST_me == 0) {  /* sender */
            ...
            ierr = MPI_Send((void *)gpu_buffer, LONGLENGTH, MPI_CHAR, 1, 1,
                            MPI_COMM_WORLD);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
            ...
            for (i = 0; i < NUMSHORT; i++) {
                ...
                ierr = MPI_Send((void *)gpuptr, 1, MPI_CHAR, 1, 1,
                                MPI_COMM_WORLD);
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
            ...
            ierr =
                MPI_Recv((void *)gpu_buffer, LONGLENGTH, MPI_CHAR, 0, 1,
            ...
            for (i = 0; i < NUMSHORT; i++) {
                len_recv=NUMSHORT-i;
                gpuptr = (CUdeviceptr)((char *)gpu_shortb + i);
                ierr = MPI_Recv((void *)gpuptr, len_recv, MPI_CHAR, 0, 1,
                                MPI_COMM_WORLD, &recv_stat);
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
