...
*/
        ...
        err =
            MPI_Recv(&buffer1, count1, datatype, source, tag1, comm1,
                     &status);
        ...
        err =
            MPI_Isend(&buffer1, count1, datatype, dest, tag1, comm1,
                      &request);
        err =
            MPI_Ibsend(&buffer1, count1, datatype, dest, tag1, comm1,
                       &request);
        err =
            MPI_Issend(&buffer1, count1, datatype, dest, tag1, comm1,
                       &request);
        err =
            MPI_Irsend(&buffer1, count1, datatype, dest, tag1, comm1,
                       &request);
        err =
            MPI_Irecv(&buffer1, count1, datatype, source, tag1, comm1,
                      &request);
        ...
        err =
            MPI_Testany(count1, &request_array[0], &tag1, &flag, &status);
        err = MPI_Waitall(count1, &request_array[0], &status_array[0]);
        err =
            MPI_Testall(count1, &request_array[0], &flag,
                        &status_array[0]);
        err =
            MPI_Waitsome(count1, &request_array[0], &count2,
                         &indicie_array[0], &status_array[0]);
        err =
            MPI_Testsome(count1, &request_array[0], &count2,
                         &indicie_array[0], &status_array[0]);
        ...
        err =
            MPI_Send_init(&buffer1, count1, datatype, dest, tag1, comm1,
                          &request);
        err =
            MPI_Bsend_init(&buffer1, count1, datatype, dest, tag1, comm1,
                           &request);
        err =
            MPI_Ssend_init(&buffer1, count1, datatype, dest, tag1, comm1,
                           &request);
        err =
            MPI_Rsend_init(&buffer1, count1, datatype, dest, tag1, comm1,
                           &request);
        err =
            MPI_Recv_init(&buffer1, count1, datatype, source, tag1, comm1,
                          &request);
        err = MPI_Start(&request);
        err = MPI_Startall(count1, &request_array[0]);
        err =
            MPI_Sendrecv(&buffer1, count1, sendtype, dest, tag1, &buffer2,
                         count2, recvtype, source, tag2, comm1, &status);
        err =
            MPI_Sendrecv_replace(&buffer1, count1, datatype, dest, tag1,
        ...
        err = MPI_Type_hindexed(count1, &length_array[0],
                                &displacement_aarray[0], oldtype,
                                &newtype);
        err =
            MPI_Type_struct(count1, &length_array[0],
...
*/
        ...
        err = MPI_Gatherv(&buffer1, count1, datatype,
                          ...
                          comm1);
        err =
            MPI_Scatter(&buffer1, count1, datatype, &buffer2, count2,
                        recvtype, root, comm1);
        err =
            MPI_Scatterv(&buffer1, &count1, &displs, datatype, &buffer2,
                         count2, recvtype, root, comm1);
        err =
            MPI_Allgather(&buffer1, count1, datatype, &buffer2, count2,
                          recvtype, comm1);
        err =
            MPI_Allgatherv(&buffer1, count1, datatype, &buffer2, &count2,
                           &displs, recvtype, comm1);
        err =
            MPI_Alltoall(&buffer1, count1, datatype, &buffer2, count2,
                         recvtype, comm1);
        err =
            MPI_Alltoallv(&buffer1, &count1, &displs1, datatype, &buffer2,
                          &count2, &displs2, recvtype, comm1);
        err =
            MPI_Reduce(&buffer1, &buffer2, count1, datatype, op, root,
                       comm1);
        err =
            MPI_Op_create((MPI_User_function *) & userfunction, commute,
                          &op);
        err = MPI_Op_free(&op);
        err =
            MPI_Allreduce(&buffer1, &buffer2, count1, datatype, op, comm1);
        err =
            MPI_Reduce_scatter(&buffer1, &buffer2, &count2, datatype, op,
                               comm1);
...
*/
        ...
        err =
            MPI_Group_translate_ranks(group1, count1, &rank1, group2,
                                      &rank2);
        ...
        err =
            MPI_Intercomm_create(comm1, leader1, comm2, leader2, tag1,
                                 &newcomm);
...
*/
        err =
            MPI_Cart_create(comm1, ndims, &dims, &periods, reorder,
                            &comm2);
        err = MPI_Dims_create(nnodes, ndims, &dims);
        err =
            MPI_Graph_create(comm1, nnodes, &index_array[0],
                             &edge_array[0], reorder, &comm2);
        err = MPI_Topo_test(comm1, &result);
        err = MPI_Graphdims_get(comm1, &nnodes, &nedges);
        err =
            MPI_Graph_get(comm1, maxindex, maxedges, &index_array[0],
                          &edge_array[0]);
        ...
        err =
            MPI_Graph_map(comm1, nnodes, &index_array[0], &edge_array[0],
                          &rank1);
...
*/
        ...
        err =
            MPI_Errhandler_create((MPI_Handler_function *) &
