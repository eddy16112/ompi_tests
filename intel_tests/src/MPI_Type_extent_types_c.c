...
int main(int argc, char *argv[])
{
    ...
    MPI_Aint * displs, displ, expected_extent,  /*  expected extent value for type                    */
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (length_count = 0;
                ...
                for (type_count = 0; type_count < num_types - 2;
                     type_count++) {
                ...
                displs[num_types - 1] =
                    displs[num_types - 3] + type_sizes[num_types - 3] +
                    length;
                ...
                if (extent != expected_extent) {
                    fail++;
                    sprintf(info_buf,
                            "MPI_Type_extent() returned unexpected %d Expected: >= %d, Actual: %d",
