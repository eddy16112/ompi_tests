...
-----------------------------------------------------------------------------*/
/******************************************************************************
		  Test for MPI_Irecv() from MPI_PROC_NULL

This tests the basic nonblocking MPI_Irecv operation from MPI_PROC_NULL.  No
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Irecv_null: all Irecv from MPI_PROC_NULL");
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
		    ...
		    -------------------------------------------------*/
                    ...
                    if (ierr != MPI_SUCCESS) {
                        ...
                        MPI_Error_string(ierr, &info_buf[0], &size);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        if (recv_stat.MPI_ERROR != MPI_SUCCESS) {
                            MPI_Error_string(recv_stat.MPI_ERROR,
                                             &info_buf[0], &size);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                        }
                        loop_fail++;
                    ...
                    /*
                     * Print non-fatal error if Received length
                     * not equal to 0
                     */
                    if (cnt_len != 0) {
                        sprintf(info_buf,
