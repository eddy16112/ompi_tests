...
int main(int argc, char *argv[])
{
 ...
 /*---------------------------------  Irecv  -----------------------------*/
    if (MPITEST_me == 1) {
        ...
        if (ierr != MPI_SUCCESS) {
            sprintf(info_buf,
                    "Non-zero return code ierr/flag (%d/%d) from MPI_Testall on Irecv, after %d calls to MPI_Testall",
        ...
        for (i = 0; i < 4 * NUMMESG; i++) {
            if (recv_request[i] != MPI_REQUEST_NULL) {
                sprintf(info_buf,
                        "recv_request[%d] not equal to  MPI_REQUEST_NULL(%p/%p) after second Testall",
