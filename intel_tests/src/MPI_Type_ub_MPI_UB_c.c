...
array (in mpitest_cfg.h) plus MPI_UB having maximum displacement.
All ranks then use call MPI_Type_ub() using the newly created datatype and
verify that the returned displacement is equal to max_displ.
...
int main(int argc, char *argv[])
{
    int
     ...
     max_displ,                 /*  Max displacement used for MPI_UB                  */
    ...
    sprintf(testname, "MPI_Type_ub_MPI_UB");
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (length_count = 0;
                ...
                for (type_count = 0; type_count < num_types - 1;
                    ...
                    ierr = MPI_Type_extent(types[type_count], &xt);
                    if (ierr != MPI_SUCCESS) {
                ...
                max_displ = displs[num_types - 2] + xt + length;

                types[num_types - 1] = MPI_UB;
                displs[num_types - 1] = max_displ;
