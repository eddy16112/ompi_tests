               ...
               /*----------------------  MPI_Graph_neighbors  -------------------*/
                    ...
                    for (i = 0; i < graph_nump; i++) {
                        ...
                        if (ierr != MPI_SUCCESS) {
                            fail++;
                            sprintf(info_buf,
                                    "MPI_Graph_neighbor error (%d)", ierr);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            MPI_Error_string(ierr, &info_buf[0], &size);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                        }
                        ...
                        if (graph_neighbors[0] != edges[2 * i]) {
                            sprintf(info_buf,
                                    "Error in lower neighbor for (%3d):  expected %3d   actual = %3d",
                                    i, edges[2 * i], graph_neighbors[0]);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                            fail++;
                        }
                        ...
                        if (graph_neighbors[1] != edges[2 * i + 1]) {
                            sprintf(info_buf,
                                    "Error in upper neighbor for (%3d):  expected %3d   actual = %3d",
                                    i, edges[2 * i + 1],
                                    graph_neighbors[1]);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                            fail++;
