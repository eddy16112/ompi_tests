...
int main(int argc, char *argv[])
{
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_intersection(group, group)  (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if (result != MPI_IDENT) {
                fail++;
                sprintf(info_buf,
                        "MPI_Group_intersection(group, group) returned UNEQUAL (comm_index %d)",
                        comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
            }
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_intersection(EMPTY, group)  (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if (group2 != MPI_GROUP_EMPTY) {
                fail++;
                sprintf(info_buf,
                        "MPI_Group_intersection(EMPTY, group) did not return empty group (comm_index %d)",
                        comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
            }
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_intersection(group, EMPTY)  (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if (group2 != MPI_GROUP_EMPTY) {
                fail++;
                sprintf(info_buf,
                        "MPI_Group_intersection(group, empty) did not return empty group (comm_index %d)",
                        comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
            }
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_intersection(world, group)  (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if ((result != MPI_IDENT) && (result != MPI_SIMILAR)) {
                fail++;
                sprintf(info_buf,
                        "MPI_Group_intersection(world, group) returned UNEQUAL (comm_index %d)",
                        comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
            }
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_intersection(group, world)  (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if (result != MPI_IDENT) {
                fail++;
                sprintf(info_buf,
                        "MPI_Group_intersection(group, world) did not return identical group (comm_index %d)",
                        comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
            }
