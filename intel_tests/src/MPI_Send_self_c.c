...
-----------------------------------------------------------------------------*/
/******************************************************************************
		  Test for MPI_Send() to self

This tests the basic blocking MPI_Send operation from a node to itself.
...
int main(int argc, char *argv[])
{
    ...
    MPI_Request recv_req, recv_test_req, send_test_req;

    MPI_Status recv_stat, recv_test_stat, send_test_stat;
    ...
    sprintf(testname, "MPI_Send_self: all Send to SELF");
        ...
        if ((MPITEST_current_rank != MPI_UNDEFINED) &&
            ...
            /*------------------  Loop over Data Types  ---------------------*/
		        ...
		        -------------------------------------------------*/
                        ...
                        ierr = MPI_Wait(&send_test_req,&send_test_stat);
                ...
                if (MPITEST_current_rank == 1) {
                    ierr = MPI_Send(send_buffer, 0, MPI_INT, 0, 1, comm);
