...
will call MPI_Type_size using it.  The resulting output size will be
...
MPI Calls dependencies for this test:
  ...
  MPI_Type_struct(), MPI_Type_size(), MPI_Type_extent(),
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Type_size_basic");
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (type_count = 0; type_count < num_types; type_count++) {
                ...
                if (MPITEST_current_rank == 0) {
                    sprintf(info_buf, "(%d) commsize %d commtype %d",
                            comm_count, test_nump, comm_type);
                    MPITEST_message(MPITEST_INFO1, info_buf);
                }

                sprintf(info_buf, "Calling MPI_Type_size on type %d...",
                        MPITEST_mpi_datatypes[test_type]);
                MPITEST_message(MPITEST_INFO1, info_buf);

                ierr =
                    MPI_Type_size(MPITEST_mpi_datatypes[test_type],
                                  &type_size);
                if (ierr != MPI_SUCCESS) {
                    ...
                    fail++;
                    sprintf(info_buf,
                            "MPI_Type_size() returned negative size (%d)",
                            type_size);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                }
                loop_cnt++;
