...
-----------------------------------------------------------------------------*/
/*****************************************************************************
		  Test for MPI_Issend():  ROOT send to ALL version
                       and MPI_Irecv()    specified source, any tag,
                                         length > expected size.

Tests the nonblocking  MPI_Issend and MPI_Irecv operations.
MPI_Wait, and MPI_Get_count are also used in conjunction with the 
nonblocking I/O calls.

MPI Standard:  Section 3.7, 3.7.2. 3.7.3
USING MPI:     Ssection 4.4

Section 3.7 of the MPI Standard states that the synchronous version of the
nonblocking send (Issend) will complete only if a matching receive has
started, i.e. a receive has been posted, and has been matched with the send.
This means the Issend operation is non-local.  Furthermore, the synchronous
nonblocking send may complete, if matched by a nonblocking receive, before the
receive complete call occurs, as it completes as soon as the sender "knows"
the transfer will complete, but before the receiver "knows" the transfer
will complete.
...
An MPI_Wait call is used with the MPI_Isend, and MPI_Irecv to verify that
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Issend_rtoa: Root send TO All others");
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            if (inter_comm) {   /* Do this stuff for INTER-communicators */
                ...
                if (MPITEST_inter == 0) {       /* Do this stuff for left group of an INTER-communicator */
                    ...
                    if (MPITEST_current_rank == 0) {    /* Do this stuff for node zero of left bank of
                                                         * INTER-communicator */
                        print_node_flag = 1;
                    }
			    ...
			    -------------------------------------------------*/
                                    ...
                                    && MPITEST_current_rank == root))
                            {
                                ...
                                for (i = 0; i < send_to; i++) {
                                    ...
                                    if (ierr != MPI_SUCCESS) {
                                        sprintf(info_buf,
                                                "Non-zero return code (%d) from MPI_Issend",
                                                ierr);
                                        MPITEST_message(MPITEST_NONFATAL,
                                                        info_buf);
