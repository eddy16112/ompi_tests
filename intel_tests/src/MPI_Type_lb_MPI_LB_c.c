...
array (in mpitest_cfg.h) plus MPI_LB having diplacement MPI_MIN_DISPL
with the minimum displacement.  All ranks then use call MPI_Type_lb() using
the newly created datatype and verify that the returned displacement is equal
to MPITEST_MIN_DISPL + length.
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Type_lb_MPI_LB");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (length_count = 0;
                ...
                displs[num_types - 1] = MPITEST_MIN_DISPL + length;
                ...
                if (ierr != MPI_SUCCESS) {
                    ...
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_FATAL, info_buf);
                } else if (displ != MPITEST_MIN_DISPL + length) {
                    fail++;
                    sprintf(info_buf,
                            "MPI_Type_lb() returned unexpected displacement  Expected: %d, Actual: %d",
                            MPITEST_MIN_DISPL + length, displ);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                }
