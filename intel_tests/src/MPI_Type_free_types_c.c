...
int main(int argc, char *argv[])
{
    ...
    MPI_Datatype newtype, newtype2, *types, types2[1];
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (length_count = 0;
                ...
                for (root = 0; root < test_nump; root++) {
                    ...
                    ierr =
                        MPI_Type_struct(1, blklens2, displs2, types2,
                                        &newtype2);
                    if (ierr != MPI_SUCCESS) {
                        sprintf(info_buf, "MPI_Type_struct() returned %d",
                                ierr);
                    ...
                    if (ierr != MPI_SUCCESS) {
                        fail++;
                        sprintf(info_buf, "MPI_Type_free() returned %d",
                                ierr);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        MPI_Error_string(ierr, &info_buf[0], &size);
                        MPITEST_message(MPITEST_FATAL, info_buf);
                    ...
                    if (MPITEST_current_rank == root) {
                        ...
                        if (ierr != MPI_SUCCESS) {
                            ...
                            ierr =
                                MPI_Get_count(&status, newtype2, &count);
                            if (ierr != MPI_SUCCESS) {
                                fail++;
                                sprintf(info_buf,
