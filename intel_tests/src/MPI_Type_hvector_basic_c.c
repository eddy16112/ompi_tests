...
All rank will call MPI_Type_hvector() and then MPI_Type_commit() to create
...
The input count to MPI_Type_hvector() is determined by each message length
in mpitest_cfg.h and the input oldtype to MPI_Type_hvector() is determined
...
MPI Calls dependencies for this test:
  ...
  MPI_Type_hvector(), MPI_Type_commit(),
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Type_hvector_basic");
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                for (length_count = 0;
                    ...
                    ierr =
                        MPI_Type_hvector(length, 1, stride,
