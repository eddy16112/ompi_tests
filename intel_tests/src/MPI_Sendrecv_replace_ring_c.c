        ...
        if (MPITEST_current_rank != MPI_UNDEFINED && my_comm_size > 1
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                for (length_count = 0;
                    ...
                    MPITEST_init_buffer(test_type, send_length + 1, value,
                                        send_buffer);
                    ...
                    sprintf(info_buf,
                            ...
                            send_to, send_length, send_length);
                    MPITEST_message(MPITEST_INFO1, info_buf);

                    ierr = MPI_Sendrecv_replace(send_buffer,
                                                send_length,
                                                MPITEST_mpi_datatypes
                                                [test_type], send_to,
                                                MPITEST_current_rank,
                                                recv_from, recv_from, comm,
                                                &stat);
                    ...
                    error =
                        MPITEST_buffer_errors(test_type, send_length,
                                              value, send_buffer);

                    /* check for receive buffer overflow */
                    MPITEST_dataTemplate_init(&value, -1);
                    error +=
                        MPITEST_buffer_errors_ov(test_type, send_length,
                                                 value, send_buffer);
                    ...
                    if (error) {
                        sprintf(info_buf,
                                ...
                                type_count, send_length, my_comm_size,
