...
int main(int argc, char *argv[])
{
    ...
    int                         /* Variables referenced by the calls */
        ...
        displs1,
        displs2,
        keyval, color, location, leader1, leader2, result, state, *bottom;

    int
        ranges[10][3];

    char
        string[MPI_MAX_ERROR_STRING], name[MPI_MAX_PROCESSOR_NAME];

    double
        time;

    MPI_Comm comm1, comm2, newcomm;

    MPI_Datatype
        datatype, newtype, oldtype, sendtype, recvtype, type_array[10];

    MPI_Status status, status_array[10];

    MPI_Request request, request_array[10];


    MPI_Aint
        address,
        astride, displacement, displacement_aarray[10], extent, asize;
...
*/
    ...
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_BUFFER = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err == 0) {
        ...
        pass++;
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_COUNT = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err == 0) {
        ...
        pass++;
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_TYPE = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err == 0) {
        ...
        pass++;
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_TAG = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err == 0) {
        ...
        pass++;
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_COMM = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err == 0) {
        ...
        pass++;
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_RANK = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err == 0) {
        ...
        pass++;
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_REQUEST = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err == 0) {
        ...
        pass++;
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_ROOT = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err == 0) {
        ...
        pass++;
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_GROUP = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err == 0) {
        ...
        pass++;
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_TOPOLOGY = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err == 0) {
        ...
        pass++;
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_DIMS = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err == 0) {
        ...
        pass++;
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_ARG = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err == 0) {
        ...
        pass++;
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_UNKNOWN = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err == 0) {
        ...
        pass++;
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_TRUNCATE = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err == 0) {
        ...
        pass++;
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_OTHER = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err == 0) {
        ...
        pass++;
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_INTERN = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err == 0) {
        ...
        pass++;
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg, "MPI_ERR_PENDING = %d, exceeds MPI_ERR_LASTCODE",
                err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
    ...
    if (err > MPI_ERR_LASTCODE) {
        sprintf(testmsg,
                "MPI_ERR_IN_STATUS = %d, exceeds MPI_ERR_LASTCODE", err);
        MPITEST_message(MPITEST_NONFATAL, testmsg);
        fail++;
    } else
        pass++;
...
*/
        ...
        err =
            PMPI_Recv(&buffer1, count1, datatype, source, tag1, comm1,
                      &status);
        ...
        err =
            PMPI_Isend(&buffer1, count1, datatype, dest, tag1, comm1,
                       &request);
        err =
            PMPI_Ibsend(&buffer1, count1, datatype, dest, tag1, comm1,
                        &request);
        err =
            PMPI_Issend(&buffer1, count1, datatype, dest, tag1, comm1,
                        &request);
        err =
            PMPI_Irsend(&buffer1, count1, datatype, dest, tag1, comm1,
                        &request);
        err =
            PMPI_Irecv(&buffer1, count1, datatype, source, tag1, comm1,
                       &request);
        ...
        err =
            PMPI_Testany(count1, &request_array[0], &tag1, &flag, &status);
        err = PMPI_Waitall(count1, &request_array[0], &status_array[0]);
        err =
            PMPI_Testall(count1, &request_array[0], &flag,
                         &status_array[0]);
        err =
            PMPI_Waitsome(count1, &request_array[0], &count2,
                          &indicie_array[0], &status_array[0]);
        err =
            PMPI_Testsome(count1, &request_array[0], &count2,
                          &indicie_array[0], &status_array[0]);
        ...
        err =
            PMPI_Send_init(&buffer1, count1, datatype, dest, tag1, comm1,
                           &request);
        err =
            PMPI_Bsend_init(&buffer1, count1, datatype, dest, tag1, comm1,
                            &request);
        err =
            PMPI_Ssend_init(&buffer1, count1, datatype, dest, tag1, comm1,
                            &request);
        err =
            PMPI_Rsend_init(&buffer1, count1, datatype, dest, tag1, comm1,
                            &request);
        err =
            PMPI_Recv_init(&buffer1, count1, datatype, source, tag1, comm1,
                           &request);
        err = PMPI_Start(&request);
        err = PMPI_Startall(count1, &request_array[0]);
        err =
            PMPI_Sendrecv(&buffer1, count1, sendtype, dest, tag1, &buffer2,
                          count2, recvtype, source, tag2, comm1, &status);
        err =
            PMPI_Sendrecv_replace(&buffer1, count1, datatype, dest, tag1,
                                  source, tag2, comm1, &status);
        err = PMPI_Type_contiguous(count1, oldtype, &newtype);
        err = PMPI_Type_vector(count1, length, stride, oldtype, &newtype);
        err =
            PMPI_Type_hvector(count1, length, astride, oldtype, &newtype);
        err =
            PMPI_Type_indexed(count1, &length_array[0],
                              &displacement_array[0], oldtype, &newtype);
        err =
            PMPI_Type_hindexed(count1, &length_array[0],
                               &displacement_aarray[0], oldtype, &newtype);
        err =
            PMPI_Type_struct(count1, &length_array[0],
                             &displacement_aarray[0], &type_array[0],
                             &newtype);
...
*/
        ...
        err = PMPI_Gatherv(&buffer1, count1, datatype,
                           &buffer2, &count2, &displs, recvtype, root,
                           comm1);
        err =
            PMPI_Scatter(&buffer1, count1, datatype, &buffer2, count2,
                         recvtype, root, comm1);
        err =
            PMPI_Scatterv(&buffer1, &count1, &displs, datatype, &buffer2,
                          count2, recvtype, root, comm1);
        err =
            PMPI_Allgather(&buffer1, count1, datatype, &buffer2, count2,
                           recvtype, comm1);
        err =
            PMPI_Allgatherv(&buffer1, count1, datatype, &buffer2, &count2,
                            &displs, recvtype, comm1);
        err =
            PMPI_Alltoall(&buffer1, count1, datatype, &buffer2, count2,
                          recvtype, comm1);
        err =
            PMPI_Alltoallv(&buffer1, &count1, &displs1, datatype, &buffer2,
                           &count2, &displs2, recvtype, comm1);
        err =
            PMPI_Reduce(&buffer1, &buffer2, count1, datatype, op, root,
                        comm1);
        err =
            PMPI_Op_create((MPI_User_function *) & userfunction, commute,
                           &op);
        err = PMPI_Op_free(&op);
        err =
            PMPI_Allreduce(&buffer1, &buffer2, count1, datatype, op,
                           comm1);
        err =
            PMPI_Reduce_scatter(&buffer1, &buffer2, &count2, datatype, op,
                                comm1);
...
*/
        ...
        err =
            PMPI_Group_translate_ranks(group1, count1, &rank1, group2,
                                       &rank2);
        ...
        err =
            PMPI_Intercomm_create(comm1, leader1, comm2, leader2, tag1,
                                  &newcomm);
...
*/
        err =
            PMPI_Cart_create(comm1, ndims, &dims, &periods, reorder,
                             &comm2);
        err = PMPI_Dims_create(nnodes, ndims, &dims);
        err =
            PMPI_Graph_create(comm1, nnodes, &index_array[0],
                              &edge_array[0], reorder, &comm2);
        err = PMPI_Topo_test(comm1, &result);
        err = PMPI_Graphdims_get(comm1, &nnodes, &nedges);
        err =
            PMPI_Graph_get(comm1, maxindex, maxedges, &index_array[0],
                           &edge_array[0]);
        ...
        err =
            PMPI_Graph_map(comm1, nnodes, &index_array[0], &edge_array[0],
                           &rank1);
...
*/
        ...
        err =
            PMPI_Errhandler_create((MPI_Handler_function *) &
                                   handlerfunction, &errhandler);
...
#if 0
        /* By the MPI-1 standard, these functions do not have to
           exist.  Open MPI does not have them. */
        time = PMPI_Wtime();
        time = PMPI_Wtick();
#endif
