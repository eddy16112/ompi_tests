...
-----------------------------------------------------------------------------*/
/******************************************************************************
*                 Functional test for  Persistent MPI_Testany
*
*  Testany references:
*
...
#define  PROG_NAME   "MPI_Testany_p"
#define  PROG_DESC   "Persistent Testany on two nodes"
...
int main(int argc, char *argv[])
{
    ...
    MPI_Status one_stat, send_stat[4 * NUMMESG],        /* Send Source/Tag information               */
    ...
    if (MPITEST_me == 0) {
        ...
        for (i = 0; i < NUMMESG; i++) {
            ierr = MPI_Send_init(send_buffer[4 * i],
                                 ...
                                 1,
                                 4 * i,
                                 MPI_COMM_WORLD, &send_request[4 * i]);
        ...
        for (i = 0; i < 4 * NUMMESG; i++) {
            ...
            while (!flag && !ierr) {
                ierr =
                    MPI_Testany(4 * NUMMESG, send_request, &index, &flag,
                                &one_stat);
                send_loop_cnt++;
            }
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code ierr/flag (%d/%d) from MPI_Testall on send, after %d calls to MPI_Testany",
            ...
            sprintf(info_buf,
                    "Testany on send:  Index %d   Error %d  #Testany loops/calls %d/%d ",
                    index, ierr, i, send_loop_cnt);
 ...
 /*-------------------------------  Receives  ----------------------------*/
    if (MPITEST_me == 1) {
        ...
        for (i = 0; i < 4 * NUMMESG; i++) {
            ...
            while (!flag && !ierr) {
                ierr =
                    MPI_Testany(4 * NUMMESG, recv_request, &index, &flag,
                                &one_stat);
                recv_loop_cnt++;
            }

            sprintf(info_buf, "Testany on receive:  Index %d    Error %d ",
                    index, ierr, recv_loop_cnt);
            MPITEST_message(MPITEST_INFO1, info_buf);
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code ierr/flag (%d/%d) from MPI_Testany on Irecv, after %d calls to MPI_Testall",
                ...
                if (errorclass == MPI_ERR_IN_STATUS) {
                    ...
                    sprintf(info_buf,
                            ...
                            index, recv_stat[index].MPI_SOURCE,
                            recv_stat[index].MPI_TAG,
                            recv_stat[index].MPI_ERROR);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(recv_stat[index].MPI_ERROR,
                                     &info_buf[0], &size);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    fail++;
...
/*  At this point all messages that were received have been Testany ed on  */
        ...
        for (i = 0; i < 4 * NUMMESG; i++) {
            ...
            if (recv_stat[i].MPI_TAG != i) {
                sprintf(info_buf,
                        "Unexpected tag for #%d  value=%d, expected=%d", i,
