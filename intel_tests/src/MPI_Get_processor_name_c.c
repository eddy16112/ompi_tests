...
int main(int argc, char *argv[])
{
    ...
    char
     name[MPI_MAX_PROCESSOR_NAME], info_buf[256],       /* buffer for passing mesages to MPITEST         */
    ...
    if (MPITEST_me == 0) {
        ...
        MPITEST_message(MPITEST_VERIFY,
                        "Please verify the following processor names");
    }
    ...
    for (i = 0; i < MPITEST_nump; i++) {
        if (i == MPITEST_me) {
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Get_processor_name",
                        ierr);
                ...
                if ((len > MPI_MAX_PROCESSOR_NAME)
                    ...
                    fail++;
                    sprintf(info_buf,
                            "Returned length=%d, MPI_MAX_PROCESSOR_NAME=%d",
                            len, MPI_MAX_PROCESSOR_NAME);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPITEST_message(MPITEST_INFO0, name);
                } else {
                    sprintf(info_buf,
                            "Returned string length=%d (OK), processor name:",
                            len);
