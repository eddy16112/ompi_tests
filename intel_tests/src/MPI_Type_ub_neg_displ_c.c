...
This test is similar to MPI_Type_ub_pos_displ except the minimum displacement
being the -1 * length.
...
then use call MPI_Type_ub() using the newly created datatype and verify that
the returned displacement is at least the sum of all type sizes + initial
displacement.
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Type_ub_negative_displ");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (length_count = 0;
                ...
                ub = displs[num_types - 1] + type_sizes[num_types - 1];
