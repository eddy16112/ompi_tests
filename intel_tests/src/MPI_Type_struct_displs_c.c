...
(in mpitest_cfg.h) using MPI_Type_struct() using length as displacement value.
...
int main(int argc, char *argv[])
{
    ...
    MPI_Datatype oldtype, newtype, *types, *types1;
...
/****************************************************************************/
                ...
                types1 =
                    (MPI_Datatype *) calloc(length, sizeof(MPI_Datatype));
                if (!types1) {
                    sprintf(info_buf,
                            "Cannot allocate enought space for types1 array");
                    MPITEST_message(MPITEST_FATAL, info_buf);
                }
                ...
                ierr =
                    MPI_Type_struct(length, blklens1, displs1, types1,
                                    &newtype);
                if (ierr != MPI_SUCCESS) {
                    fail++;
