...
int main(int argc, char *argv[])
{
    ...
    MPI_Comm comm[3];           /* A number of communicators to sort over            */
    int
    ...
    if (MPITEST_me == 0) {
         ...
         */
        MPI_Barrier(comm[2]);
        ierr = MPI_Send(send_buffer[0], NUMELM, MPI_INT, 1, 1, comm[2]);
 ...
 /*--------------------------------- Receives  ---------------------------*/
    if (MPITEST_me == 1) {
        ...
        ierr = MPI_Irecv(recv_buffer[3 * NUMMESG],
                         NUMELM, MPI_INT, 0, 1, comm[2], &recv_request);

        total_cnt++;
        if (ierr != MPI_SUCCESS) {
            sprintf(info_buf, "Non-zero return code (%d) from MPI_Irecv",
                    ierr);
            MPITEST_message(MPITEST_NONFATAL, info_buf);
            MPI_Error_string(ierr, &info_buf[0], &size);
            MPITEST_message(MPITEST_NONFATAL, info_buf);
        ...
        for (i = 0; i < 3 * NUMMESG; i++) {
            ...
            ierr = MPI_Recv(recv_buffer[i],
                            NUMELM, MPI_INT, 0, 1, comm[j], &recv_stat);

            total_cnt++;
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Recv for message %d",
                        ierr, i);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
