...
-----------------------------------------------------------------------------*/
/******************************************************************************
                         Test of MPI_Scan_loc()

This code tests the functionality of the MPI_Scan() funtion.  This function
acts much like a Reduce_scatter(), except the reduced data which arrives at
process "i" has only been reduced with data from processes 0, 1, 2, ..., i.
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                if ((send_buffer == NULL) || (recv_buffer == NULL)) {
                    MPITEST_message(MPITEST_FATAL,
                                    "Failed to allocate memory in MPI_Scan_loc");
                }
                ...
                for (length_count = 0;
                    ...
                    for (op_count = 0; op_count < num_ops; op_count++) {
                        ...
                        if (has_op(op_count, test_type) && (length != 0)) {
                            ...
                            if (ierr != MPI_SUCCESS) {
                                sprintf(info_buf, "MPI_Scan() returned %d",
                                        ierr);
