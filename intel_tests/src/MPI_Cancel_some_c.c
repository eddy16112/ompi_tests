                ...
                for (root = 0; root < test_nump; root++) {
                    ...
                    if (MPITEST_current_rank == root) {
                        for (tag = 0; tag < maxtag; tag++) {
                            ...
                            if (tag % 2 == 0) {
                                sprintf(info_buf,
                                        "Cancelling send request from source %d to %d, tag: %d",
                                        root, dest, tag);
                                MPITEST_message(MPITEST_INFO1, info_buf);
                                ...
                                if (ierr != MPI_SUCCESS) {
                                    ...
                                    MPITEST_message(MPITEST_INFO1,
                                                    info_buf);

                                    ierr =
                                        MPI_Wait(&send_request[tag],
                                                 &status[tag]);
