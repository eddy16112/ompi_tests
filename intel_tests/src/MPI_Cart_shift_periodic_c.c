            ...
            /*------------------------ MPI_Cart_shift   ------------------------*/
                ...
                for (j = 0; j < ndims; j++) {
                    ...
                    for (k = -1; k <= 1; k++) {
                        ...
                        loop_cnt++;
                        ierr =
                            MPI_Cart_shift(comm_cart, j, k, &sorc, &dest);
                        ...
                        if (sorc_rank != sorc) {
                            sprintf(info_buf,
                                    "Source (%d/%d)  expected coord/rank (%d/%d)    Actual %d",
                                    j, k, exp_sorc, sorc_rank, sorc);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            fail++;
                        }
                        ...
                        if (dest_rank != dest) {
                            sprintf(info_buf,
                                    "Dest (%d/%d)  expected coord/rank (%d/%d)    Actual %d",
                                    j, k, exp_dest, dest_rank, dest);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            fail++;
                        }
