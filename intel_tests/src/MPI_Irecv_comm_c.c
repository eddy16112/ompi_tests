...
#define  PROG_NAME   "MPI_Irecv_comm"
#define  PROG_DESC   " "
...
int main(int argc, char *argv[])
{
    ...
    if (MPITEST_me == 0) {
        ...
        for (i = 0; i < NUMMESG; i++) {
            ...
            ierr = MPI_Isend(send_buffer[4 * i],
                             NUMELM,
                             MPI_INT, 1, 1, comm[j], &send_request[4 * i]);
 ...
 /*--------------------------------- Receives  ---------------------------*/
    if (MPITEST_me == 1) {
        ...
        ierr = MPI_Irecv(recv_buffer[4 * NUMMESG],
                         NUMELM,
                         MPI_INT,
        ...
        for (i = 0; i < 4 * NUMMESG; i++) {
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Irecv for message %d",
                        ierr, i);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
