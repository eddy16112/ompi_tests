...
int main(int argc, char *argv[])
{
    ...
    *attr, color,               /* For MPI_Comm_split              */
    ...
    sprintf(testname, "MPI_Comm_split2");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            /* Error Test  */
            /* Split the communicator into one new comm with ranks in
               the opposite order. */
            color = 2;
            ierr = MPI_Comm_split(comm, color,
                                  2 * test_nump - MPITEST_current_rank,
                                  &comm2);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
            ...
            /* If there is just one rank in comm, then the new communicator
               is congruent; otherwise it is similar */
            if (test_nump != 1) {       /* More than one rank */
                if (result != MPI_SIMILAR) {
                    sprintf(info_buf,
                            "MPI_Comm_compare returned %d, expected MPI_SIMILAR (comm_index %d)",
                            result, comm_index);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    fail++;
                }
                ...
                if (result != MPI_CONGRUENT) {
                    sprintf(info_buf,
                            "MPI_Comm_compare returned %d, expected MPI_CONGRUENT (comm_index %d)",
                            result, comm_index);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    fail++;
                }
            ...
            else if (resultsplit != test_nump) {
                sprintf(info_buf,
                        "MPI_Comm_size returned %d, expected %d (comm_index %d)",
                        resultsplit, test_nump, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            loop_cnt++;
            ...
            else if (resultsplit != test_nump - MPITEST_current_rank - 1) {
                sprintf(info_buf,
                        "MPI_Comm_rank returned %d, expected %d (comm_index %d)",
                        resultsplit, test_nump - MPITEST_current_rank - 1,
                        comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
