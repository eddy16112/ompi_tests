...
int main(int argc, char *argv[])
{
    ...
    MPI_Status recv_stat[NUMSHORT + 1], send_stat[NUMSHORT + 1];        /* MPI  status structure                  */
    ...
    sprintf(testname, "MPI_Startall2");
    ...
    if (MPITEST_me < 2) {
        ...
        if (MPITEST_me == 0) {  /* sender */
            ...
            for (i = 0; i < NUMSHORT; i++) {
                ierr = MPI_Send_init(&shortb[i], 1, MPI_CHAR, 1, 1,
                                     MPI_COMM_WORLD, &send_request[i + 1]);
                if (ierr != MPI_SUCCESS) {
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    sprintf(info_buf,
                            "Non-zero return code (%d) from MPI_Send_init",
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Startall for send",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            ierr =
                MPI_Recv_init(&buffer, LONGLENGTH, MPI_CHAR, 0, 1,
            ...
            for (i = 0; i < NUMSHORT; i++) {
                len_recv=NUMSHORT-i;
                ierr =
                    MPI_Recv_init(&shortb[i], len_recv, MPI_CHAR, 0, 1,
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Startall for receive",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
