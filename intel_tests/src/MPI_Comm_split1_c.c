...
int main(int argc, char *argv[])
{
    ...
    *attr, color,               /* For MPI_Comm_split              */
     result, resultsplit, size;
    ...
    sprintf(testname, "MPI_Comm_split1");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Attr_put (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            /* Error Test  */
            /* Split the communicator into test_nump new comms */
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_compare #1 (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            /* If there is just one rank in comm, then the new communicator
               is congruent; otherwise it is unequal */
            if (test_nump != 1) {       /* More than one rank */
                if (result != MPI_UNEQUAL) {
                    sprintf(info_buf,
                            "MPI_Comm_compare #1 returned %d, expected MPI_UNEQUAL (comm_index %d)",
                            result, comm_index);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    fail++;
                }
                ...
                if (result != MPI_CONGRUENT) {
                    sprintf(info_buf,
                            "MPI_Comm_compare #1 returned %d, expected MPI_CONGRUENT (comm_index %d)",
                            result, comm_index);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    fail++;
                }
            ...
            /* Now compare to MPI_COMM_SELF; expect CONGRUENT */
            ierr = MPI_Comm_compare(MPI_COMM_SELF, comm2, &result);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
            ...
            if (ierr != MPI_SUCCESS) {
                ...
                sprintf(info_buf,
                        "MPI_Comm_test_inter returned %d, expected 0 (comm_index %d)",
                        resultsplit, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            loop_cnt++;
            ...
            else if (resultsplit != 1) {
                sprintf(info_buf,
                        "MPI_Comm_size returned %d, expected 1 (comm_index %d)",
                        resultsplit, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            if (ierr != MPI_SUCCESS) {
                ...
                sprintf(info_buf,
                        "MPI_Comm_rank returned %d, expected 0 (comm_index %d)",
                        resultsplit, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Bcast (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Attr_get (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                ...
                fail++;
                sprintf(info_buf,
                        "MPI_Attr_get found attribute in split comm; it should not (comm_index %d).",
                        comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            }
            loop_cnt++;
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_free (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                ...
                sprintf(info_buf,
                        "comm = %d after MPI_Comm_free, expected MPI_COMM_NULL (comm_index %d)",
                        comm2, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Attr_delete after delete (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
