...
(in mpitest_cfg.h) using MPI_Type_vector() using length as blklen value.
...
MPI Calls dependencies for this test:
  ...
  MPI_Type_vector(), MPI_Type_struct(), MPI_Type_commit(),
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Type_vector_blklen");
...
/****************************************************************************/
                ...
                ierr =
                    MPI_Type_vector(length, length, length, oldtype,
                                    &newtype);
                if (ierr != MPI_SUCCESS) {
                    fail++;
                    sprintf(info_buf, "MPI_Type_vector() returned %d",
                            ierr);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_FATAL, info_buf);
