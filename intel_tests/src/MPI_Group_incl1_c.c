...
-----------------------------------------------------------------------------*/
/******************************************************************************
			  Test for MPI_Group_incl

This test verifies that MPI_Group_incl creates a correct group from an
arbitrary commuicator.  This test creates a group consisting of the last
and first rank of MPI_COMM_WORLD.  This test also verifies the correct
operation when ranks=0.
...
int main(int argc, char *argv[])
{
    ...
    *ranks,                     /* Array of ranks in group         */
    ...
    sprintf(testname, "MPI_Group_incl1");
    ...
    ierr = MPI_Group_incl(groupworld, newsize, ranks, &group);
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Group_incl #1", ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    ...
    if ((worldsize == 1) && (result != MPI_IDENT) ||
        (worldsize == 2) && (result != MPI_SIMILAR) ||
    ...
    ierr = MPI_Group_incl(groupworld, 0, ranks, &group);
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Group_incl #2", ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    ...
    if (group != MPI_GROUP_EMPTY) {
        sprintf(info_buf,
                "MPI_Group_incl of size 0 did not return MPI_GROUP_EMPTY)");
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    }
