...
-----------------------------------------------------------------------------*/
/*****************************************************************************
	   Persistent  MPI_Ssend_init():  ROOT send to ALL version

                      with MPI_Ssend_init():  Root send to all Others
                           MPI_Start to send/receive requests
                           MPI_Wait  to check completion
                           MPI_Request_free to free send/recv requests
                      and  MPI_Recv_init():  Specified source/Any tag
...
int main(int argc, char *argv[])
{
    int
     ...
     print_node_flag,           /* node 0 of INTRA, node 0 of left comm of
                                 * INTER */
    ...
    sprintf(testname,
            "MPI_Ssend_init_rtoa:  Persistent synchronous root send to all");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
             ...
             * communicator
             * 
             * Set the print_node_flag: This flag tells which node gets to
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
			    ...
			    -------------------------------------------------*/
                                    ...
                                    && MPITEST_inter == recv_group))
                            {
                                ...
                                /* End of Recv_init Error Test  */
                                /* Start the Receive */
                                ierr = MPI_Start(&recv_request);

                                if (ierr != MPI_SUCCESS) {
                                    sprintf(info_buf,
                                            "Non-zero return code (%d) from MPI_Start for MPI_Recv_init",
                                            ierr);
                                    MPITEST_message(MPITEST_NONFATAL,
                                                    info_buf);
                                    MPI_Error_string(ierr, &info_buf[0],
