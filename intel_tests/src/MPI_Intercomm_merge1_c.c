...
int main(int argc, char *argv[])
{
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Interc omm_merge (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if ((resultmerge == MPI_UNEQUAL) || (resultmerge == MPI_IDENT)) {
                sprintf(info_buf,
                        "MPI_Comm_compare returned incorrect value %d, expected MPI_SIMILAR or CONGRUENT (comm_index %d)",
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_test_inter on merged comm (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if (resultmerge != 0) {
                sprintf(info_buf,
                        "MPI_Comm_test_inter on merged comm returned %d, expected 0 (comm_index %d)",
                        resultmerge, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            if ((resultmerge < 0) || (resultmerge >= test_nump)) {
                sprintf(info_buf,
                        "MPI_Comm_rank returned invalid rank %d (comm_index %d)",
                        resultmerge, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
