...
NOTE:  This test assumes than any outstanding Isend not yet matched by a
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Cancel_isend");
            ...
            for (length_count = 0;
                ...
                for (root = 0; root < test_nump; root++) {
                    ...
                    if (MPITEST_current_rank == root) {
                        ...
                        sprintf(info_buf,
                                "Sending message from source %d to %d, tag: %d",
                                root, dest, tag);
                        MPITEST_message(MPITEST_INFO1, info_buf);
