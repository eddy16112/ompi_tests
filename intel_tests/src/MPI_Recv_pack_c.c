...
-----------------------------------------------------------------------------*/
/******************************************************************************
                    Functional test for Recv of type packed

                    with ROOT send to ALL MPI_IRecv():  Specified source/tag

Ref:  MPI Standard:  Sections  3.2.2 on Message Datatypes
                     and       3.13 on Pack and Unpack operations

This is the same test as the basic blocking MPI_Send operation, with both
Intra/Inter communicators, as well as MPI_Recv with specified source and tags.
...
int main(int argc, char *argv[])
{
    ...
    *values;                    /*  Array of dataTemplates for verbose init           */
    ...
    sprintf(testname, "MPI_Recv using Packed type with Root sends TO All");
			    ...
			    -------------------------------------------------*/
                            ...
                            if ((!inter_comm
                                ...
                                ierr = MPI_Unpack(pack_buffer, cnt_len,
                                                  &position, recv_buffer,
                                                  length,
                                                  MPITEST_mpi_datatypes
                                                  [test_type], comm);
                                if (ierr != MPI_SUCCESS) {
                                    sprintf(info_buf,
                                            "Non-zero return code (%d) from MPI_Unpack",
                                            ierr);
                                    MPITEST_message(MPITEST_NONFATAL,
                                                    info_buf);
                                    MPI_Error_string(ierr, &info_buf[0],
