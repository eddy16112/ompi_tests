...
int main(int argc, char *argv[])
{
    ...
    MPI_Group group,            /* Result of difference of group[12]   */
     groupmine,                 /* Goup which should match difference  */
    ...
    sprintf(testname, "MPI_Group_difference3");
    ...
    /* Error Test  */
    ierr = MPI_Group_difference(group1, group2, &group);
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Group_difference(group1, group2)",
                ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
    ...
    if (MPITEST_nump % 2 == 0)
        ranks[0][0] = MPITEST_nump - 3;
    else
        ranks[0][0] = MPITEST_nump - 2;
    ranks[0][1] = 3;
    ranks[0][2] = -2;
    if (MPITEST_nump == 4)
        groupmine = MPI_GROUP_EMPTY;
    else {
        ierr = MPI_Group_range_incl(groupworld, 1, ranks, &groupmine);
        if (ierr != MPI_SUCCESS) {
            sprintf(info_buf,
                    "Non-zero return code (%d) from MPI_Group_range_incl(groupmine)",
                    ierr);
            MPITEST_message(MPITEST_NONFATAL, info_buf);
            MPI_Error_string(ierr, &info_buf[0], &size);
            MPITEST_message(MPITEST_NONFATAL, info_buf);
    ...
    if ((MPITEST_me % 2 == 0) ||
        (MPITEST_me == MPITEST_nump - 1) || (MPITEST_me == 1))
        expect = MPI_UNDEFINED;
    else
        expect = (MPITEST_nump + MPITEST_nump % 2 - MPITEST_me - 3) / 2;
    if (result != expect) {
        fail++;
        sprintf(info_buf,
    ...
    /* Now try with the reverse difference */
    ierr = MPI_Group_difference(group2, group1, &group);
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Group_difference(group2, group1)",
                ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
    ...
    ranks[0][0] = 0;
    ierr = MPI_Group_incl(groupworld, 1, ranks[0], &groupmine);
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf, "Non-zero return code (%d) from MPI_Group_incl",
                ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
