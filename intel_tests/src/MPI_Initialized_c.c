...
int main(int argc, char *argv[])
{
    ...
    if (savedierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Initialized before MPI_Init",
                savedierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(savedierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    }
    if (flag != 0) {
        sprintf(info_buf,
                "MPI_Initialized before MPI_Init returned %d, expected 0 (FALSE)",
                flag);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    }
    ...
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Initialized after MPI_Init",
                ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
    ...
    if (flag == 0) {
        sprintf(info_buf,
                "MPI_Initialized after MPI_Init returned %d, expected !0 (TRUE)",
                flag);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    }
