...
int main(int argc, char *argv[])
{
    ...
    void *buffer = NULL;        /* buffer for sending and receiving                   */
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (root = 0; root < test_nump; root++) {
                ...
                if (MPITEST_current_rank == 0) {
                    sprintf(info_buf,
                            "(%d) commsize %d commtype %d root %d",
                ...
                if (MPITEST_current_rank == root) {
                    ...
                    while (!flag && (++probe_count) < MAX_PROBE) {
                        ...
                        if (!(probe_count % PROBE_REPORT_PERIOD)) {
                            sprintf(info_buf, "Probe number %d",
                                    probe_count);
                            MPITEST_message(MPITEST_INFO1, info_buf);
                        }
                    ...
                    if (flag) {
                        fail++;
                        sprintf(info_buf,
                                "Received a post-barrier msg from rank %d before entering barrier, (%d) commsize %d commtype %d root %d",
                                status.MPI_SOURCE, comm_count, test_nump,
                                comm_type, root);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                    }
                    ...
                    for (i = 0; i < test_nump - 1; i++) {
                        ierr =
                            MPI_Recv(buffer, count, MPI_INT,
                                     MPI_ANY_SOURCE, POST_BARRIER_TAG,
                                     comm, &status);
                        if (ierr != MPI_SUCCESS) {
                    ...
                    sprintf(info_buf,
                            "Root received post-barrier messages");
                    MPITEST_message(MPITEST_INFO2, info_buf);
                }
                ...
                /* to root */
                    ...
                    ierr =
                        MPI_Send(buffer, count, MPI_INT, root,
                                 POST_BARRIER_TAG, comm);
                    if (ierr != MPI_SUCCESS) {
