...
replicate the datatype following by a call to MPI_Pack_size() using the resulting
...
int main(int argc, char *argv[])
{
    ...
    MPI_Datatype newtype, newtype2, *types;

    int *blklens, num_types, pack_size;
    ...
    sprintf(testname, "MPI_Pack_size_types");
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (type_count = 0; type_count < num_types; type_count++) {
                ...
                if (type_count == 0) {
                    ...
                    displs[type_count] =
                        displs[type_count - 1] + type_sizes[type_count -
                                                            1];
                }
                ...
                ierr =
                    MPI_Type_extent(types[type_count],
            ...
            for (length_count = 0;
                ...
                ierr = MPI_Pack_size(1, newtype2, comm, &pack_size);
                if (ierr != MPI_SUCCESS) {
                    ...
                    fail++;
                    sprintf(info_buf,
                            "(incount = %d, mpi type = %d, comm = %d) -> size = %d, MPI_Pack_size() returned negative size (%d)",
                            length, newtype2, comm_index, pack_size,
                            pack_size);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                } else {
                    sprintf(info_buf,
                            "(incount = %d, mpi type = %d, comm = %d) -> size = %d",
                            length, newtype2, comm_index, pack_size);
                    MPITEST_message(MPITEST_INFO1, info_buf);
                }
