...
int main(int argc, char *argv[])
{
    ...
    *ranks,                     /* Array of ranks in group         */
     worldsize, worldrank, newsize, result, expect, size, i;
    ...
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Group_excl #1", ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    ...
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Group_excl #2", ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    ...
    if (group != MPI_GROUP_EMPTY) {
        sprintf(info_buf,
                "MPI_Group_excl of size 0 did not return MPI_GROUP_EMPTY)");
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    }
    ...
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Group_excl #3", ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
