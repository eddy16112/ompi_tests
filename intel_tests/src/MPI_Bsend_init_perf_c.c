...
#define ALIGN	8192
char *malloc_buffer, *buffer, *bufp, *bsend_buffer;
/*
 * Procedure:
 *	runtest(length)
 *
...
double runtest(int length, int count, MPI_Comm comm)
{
    ...
    MPI_Recv_init(bufp, length, MPI_BYTE, MPI_ANY_SOURCE, 0, comm, &rreq);
    MPI_Bsend_init(bufp, length, MPI_BYTE, next, 0, comm, &sreq);
    /*
...
makebuf()
{
    ...
    bsend_buffer = malloc(maxlength + MPI_BSEND_OVERHEAD + 1000);
    MPI_Buffer_attach(bsend_buffer, maxlength + MPI_BSEND_OVERHEAD + 1000);
    ...
    buffer =
        (char *) ((((unsigned long) buffer) + ALIGN - 1) & ~(ALIGN - 1));
    ...
    sprintf(info_buf, "Buffer address %08X  length %d", buffer, maxlength);
    MPITEST_message(MPITEST_INFO1, info_buf);
}
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Bsend_init_perf");
