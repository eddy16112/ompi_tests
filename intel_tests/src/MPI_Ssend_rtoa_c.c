...
-----------------------------------------------------------------------------*/
/*****************************************************************************
		  Test for MPI_Ssend():  ROOT send to ALL version
                       and MPI_Recv()    specified source, any tag,
                                         length > expected size.

Ssend is the synchronous call to the blocking send.
Section 3.4 of the MPI Standard states that a send that uses synchronous mode
can be started whether or not a matching receive was posted. The send wil
only complete when the matching receive has been posted, and has started to
receive the message.  For tese blocking sends and receives, the communication
does not complete at either end until both operations rendezvous at the
communication.  Thus, the Ssend is non-local, and no additional synchronization
is required.
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Ssend_rtoa: Root send TO All others");
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
			    ...
			    -------------------------------------------------*/
                                    ...
                                    && MPITEST_inter == recv_group))
                            {
                                ...
                                ierr = MPI_Recv(recv_buffer, length + 1,
			    ...
			    -------------------------------------------------*/
                                    ...
                                    && MPITEST_current_rank == root))
                            {
                                ...
                                for (i = 0; i < send_to; i++) {
                                    ...
                                    ierr = MPI_Ssend(send_buffer, length,
                                                     MPITEST_mpi_datatypes
                                                     [test_type], i, root,
                                                     comm);
                                    ...
                                    if (ierr != MPI_SUCCESS) {
                                        sprintf(info_buf,
                                                "Non-zero return code (%d) from MPI_Ssend",
                                                ierr);
                                        MPITEST_message(MPITEST_NONFATAL,
                                                        info_buf);
