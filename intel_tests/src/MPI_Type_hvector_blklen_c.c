...
(in mpitest_cfg.h) using MPI_Type_hvector() using length as blklen value.
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Type_hvector_blklen");
...
/*********** This code was added by Debbie Swider 9/17/97 ****************/
                ...
                /* Because (length)*(length)*(extent) is used to determine
                   the number of elements in the length of each block and
                   the stride between the blocks, this product cannot be greater 
                   than the length of the buffer.  If it is - reduce the length
                   to the square root of the (byte_length/extent).  This 
                   will make the call to hvector stay within bounds. */
...
/****************************************************************************/
                ...
                ierr =
                    MPI_Type_hvector(length, length, length * extent,
                                     oldtype, &newtype);
                if (ierr != MPI_SUCCESS) {
                    fail++;
                    sprintf(info_buf, "MPI_Type_hvector() returned %d",
                            ierr);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_FATAL, info_buf);
