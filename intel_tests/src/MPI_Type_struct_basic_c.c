...
All rank will call MPI_Type_struct() and then MPI_Type_commit() to create
...
The input count to MPI_Type_struct() is determined by each message length
in mpitest_cfg.h and the input oldtype to MPI_Type_struct() is determined
...
int main(int argc, char *argv[])
{
    ...
    *types;
    ...
    sprintf(testname, "MPI_Type_struct_basic");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                MPITEST_get_gpu_buffer(test_type,
                ...
                for (length_count = 0;
                    ...
                    types =
                        (MPI_Datatype *) calloc(length,
                                                sizeof(MPI_Datatype));
                    if (!types) {
                        sprintf(info_buf,
                                "Cannot allocate enough space for types array");
                        MPITEST_message(MPITEST_FATAL, info_buf);
                    }
                    ...
                    for (i = 0; i < length; i++) {
                        blklens[i] = 1;
                        displs[i] = i * extent;
                        types[i] = MPITEST_mpi_datatypes[test_type];
                    }

                    /* Create new user defined datatype */
                    ierr =
                        MPI_Type_struct(length, blklens, displs, types,
                                        &newtype);
                    if (ierr != MPI_SUCCESS) {
                        fail++;
...
#if defined (CUDA_TEST_G2G)
                        MPITEST_init_gpu_buffer(test_type, length + 1, value,
                                                buffer);
#else
...
#if defined (CUDA_TEST_G2G)
                            error = 
                                MPITEST_gpu_buffer_errors(test_type, length,
                                                          value, buffer);
#else
                            error =
...
#if defined (CUDA_TEST_G2G)
                            error = 
                                MPITEST_gpu_buffer_errors_ov(test_type, length,
                                                             value, buffer);
#else
                            error +=
