...
is from the types aray defined in mpitest_cfg.h.  The output unpacked message
data will be verified and depending on whether MPITEST_STATUS_CHK #defined
or not, the output status object may or may not be verified.
...
int main(int argc, char *argv[])
{
    ...
    signed char *in_buffer, *packed_buffer, *buf_ptr;   /* message buffer              */
    ...
    MPI_Datatype newtype, *types, type;
    ...
    sprintf(testname, "MPI_Pack_basic_type");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            if (test_nump < 2) {
                ...
                sprintf(info_buf,
                        "Skipping communicator with comm_size < 2 (commtype: %d) for this test",
                        comm_type);
            ...
            for (type_count = 0; type_count < num_types; type_count++) {
                ...
                if (test_type < MPITEST_derived1) {
                    ...
                    ierr =
                        MPI_Type_size(types[basic_types],
                                      &(type_sizes[basic_types]));
                    if (ierr != MPI_SUCCESS) {
            ...
            if (basic_types == 0)
                MPITEST_message(MPITEST_FATAL,
                                "No basic data types configured");

            ierr =
                MPI_Type_struct(basic_types, blklens, displs, types,
                                &newtype);
            if (ierr != MPI_SUCCESS) {
            ...
            if (MPITEST_current_rank == 0) {
                for (type_count = 0; type_count < basic_types;
                     type_count++) {
                    sprintf(info_buf,
                            "blklens[%d] = %d, displs[%d] = %d, "
                            "types[%d] = %d, type_sizes[%d] = %d",
                            type_count, blklens[type_count], type_count,
                            displs[type_count], type_count,
            ...
            /* to continue */
            MPITEST_get_buffer(MPITEST_byte,
            ...
            for (length_count = 0;
                ...
                if (length == 0) {
                    sprintf(info_buf,
                            "element length = 0, using length = 1");
                    MPITEST_message(MPITEST_INFO1, info_buf);
                ...
                for (root = 0; root < test_nump; root++) {
                    ...
                    if (MPITEST_current_rank == 0) {
                        sprintf(info_buf,
                                "(%d, %d) length %d commsize %d commtype %d extent %d root %d",
                                length_count, comm_count, length,
                                test_nump, comm_type, extent, root);
                        MPITEST_message(MPITEST_INFO1, info_buf);
                    }

                    /* Initialize send & recv buffer */
                    if (MPITEST_current_rank != root) {
                        for (i = 0; i < max_length + MPITEST_ADD_BUFSIZE;
                             i++) {
                            in_buffer[i] = -1 * (MPITEST_current_rank % 128);
                        }
                    } else {
                        /* Root's rank */
                        for (i = 0; i < length; i++) {
                    ...
                    sprintf(info_buf,
                            "Calculating space needed for packing message...");
                    MPITEST_message(MPITEST_INFO1, info_buf);

                    ierr =
                        MPI_Pack_size(length, newtype, comm, &pack_size);
                    if (ierr != MPI_SUCCESS) {
                        fail++;
                        sprintf(info_buf, "MPI_Pack_size() returned %d",
                                ierr);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        MPI_Error_string(ierr, &info_buf[0], &size);
                        MPITEST_message(MPITEST_FATAL, info_buf);
                    ...
                    sprintf(info_buf, "Packing message size is %d",
                            pack_size);
                    MPITEST_message(MPITEST_INFO1, info_buf);

                    MPITEST_get_buffer(MPITEST_byte,
                                       pack_size + MPITEST_ADD_BUFSIZE,
                                       (void *) &packed_buffer);
		        ...
		        char *tmp_buf;
  			    ...
  			    buf_ptr = tmp_buf;
                            for (type_count = 0; type_count < basic_types;
                                 type_count++) {
                                test_type =
                                    MPITEST_get_datatype(type_count);

                                type = MPITEST_mpi_datatypes[test_type];

                                ierr =
                                    MPI_Pack(buf_ptr, 1, type,
                                             packed_buffer,
                                             pack_size +
                                             MPITEST_ADD_BUFSIZE,
                                             &position, comm);
                        ...
                        sprintf(info_buf,
                                "Displaying UNPACKED Sender's data buffer:");
                        MPITEST_message(MPITEST_INFO2, info_buf);

                        for (i = 0; i < max_length + MPITEST_ADD_BUFSIZE;
                             i++) {
                            sprintf(info_buf, "buffer[%d] = %d", i,
                                    in_buffer[i]);
                            MPITEST_message(MPITEST_INFO2, info_buf);
                        }

                        sprintf(info_buf,
                                "Displaying PACKED Sender's data buffer:");
                        MPITEST_message(MPITEST_INFO2, info_buf);

                        for (i = 0; i < pack_size; i++) {
                            sprintf(info_buf, "packed_buffer[%d] = %d", i,
                                    packed_buffer[i]);
                            MPITEST_message(MPITEST_INFO2, info_buf);
                        }
#endif
                        ...
                        sprintf(info_buf,
                                "Sending message size from source: %d to %d, tag: %d",
                                root, dest, tag + 1);
                        MPITEST_message(MPITEST_INFO1, info_buf);
                        ...
                        if (ierr != MPI_SUCCESS) {
                            fail++;
                            sprintf(info_buf, "MPI_Send() returned %d",
                                    ierr);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            MPI_Error_string(ierr, &info_buf[0], &size);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                        ...
                        sprintf(info_buf,
                                "Sending PACKED data from source: %d to %d, tag: %d",
                                root, dest, tag);
                        MPITEST_message(MPITEST_INFO1, info_buf);

                        ierr =
                            MPI_Send(packed_buffer, position, MPI_PACKED,
                                     dest, tag, comm);
                        if (ierr != MPI_SUCCESS) {
                            fail++;
                            sprintf(info_buf, "MPI_Send() returned %d",
		        ...
		        char *tmp_buf;
                        ...
                        sprintf(info_buf,
                                "Receiving message size from source: %d to %d, tag: %d",
                                root, dest, tag + 1);
                        MPITEST_message(MPITEST_INFO1, info_buf);

                        ierr =
                            MPI_Recv(&position, 1, MPI_INT, root, tag + 1,
                                     comm, &status);
                        if (ierr != MPI_SUCCESS) {
                            fail++;
                        ...
                        sprintf(info_buf,
                                "Receiving PACKED data from source: %d to %d, tag: %d",
                                root, dest, tag);
                        MPITEST_message(MPITEST_INFO1, info_buf);

			memset( packed_buffer, -1, position );  /* fill with junk */
                        ierr =
                            MPI_Recv(packed_buffer, position, MPI_PACKED,
                        ...
                        if (ierr != MPI_SUCCESS) {
                            ...
                            if (ierr != MPI_SUCCESS) {
                                ...
                                if (count != position) {
                                    fail++;
                                    sprintf(info_buf,
                                            "status object returned from MPI_Recv() contains unexpected length."
                                            "  Expected: %d, Actual: %d",
                                            position, count);
                                    MPITEST_message(MPITEST_NONFATAL,
                                                    info_buf);
			    ...
			    buf_ptr = tmp_buf;
                            for (type_count = 0; type_count < basic_types;
                                ...
                                ierr =
                                    MPI_Unpack(packed_buffer,
                                               pack_size +
                                               MPITEST_ADD_BUFSIZE,
                                               &position, buf_ptr, 1, type,
                                               comm);

                                if (ierr != MPI_SUCCESS) {
                                    fail++;
                                    sprintf(info_buf,
                                            "MPI_Unpack() returned %d",
                                            ierr);
                                    MPITEST_message(MPITEST_NONFATAL,
                                                    info_buf);
                                    MPI_Error_string(ierr, &info_buf[0],
			    ...
			    tmp_buf += extent;
                                    ...
                                    if (in_buffer[pos] != j + 1) {
                                        ...
                                        if (error <= 1) {
                                            ...
                                            MPITEST_message
                                                (MPITEST_NONFATAL,
                                                 info_buf);
					    sprintf( info_buf, "length %d/%d basic_types %d/%d, types_size %d/%d\n",
						     i, length, j, basic_types, k, type_sizes[j] );
                                            MPITEST_message
                                                (MPITEST_NONFATAL,
                                                 info_buf);
#ifdef MPITEST_1ST_ERR
                        ...
                        if (in_buffer[length * extent] !=
                            -1 * (MPITEST_current_rank % 128)) {
                            fail++;
                            error2++;
#ifdef MPITEST_1ST_ERR
                            if (error2 <= 1) {
                        ...
                        if (error + error2) {
...
#ifdef MPITEST_DISP_BUF
                            sprintf(info_buf,
                                    "Displaying UNPACKED received data buffer:");
                            MPITEST_message(MPITEST_INFO2, info_buf);

                            for (i = 0;
                                 i < max_length + MPITEST_ADD_BUFSIZE;
                                 i++) {
                                sprintf(info_buf, "buffer[%d] = %d", i,
                                        in_buffer[i]);
                                MPITEST_message(MPITEST_INFO2, info_buf);
                            ...
                            sprintf(info_buf,
                                    "Displaying PACKED received data buffer:");
                            MPITEST_message(MPITEST_INFO2, info_buf);

                            for (i = 0; i < pack_size; i++) {
                                sprintf(info_buf, "packed_buffer[%d] = %d",
                                        i, packed_buffer[i]);
                                MPITEST_message(MPITEST_INFO2, info_buf);
                            }
#endif
                            sprintf(info_buf,
                                    "%d errors in buffer (%d, %d) len %d commsize %d commtype %d extent %d root %d",
                                    error + error2, length_count,
                                    comm_count, length, test_nump,
                                    comm_type, extent, root);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                        } else {
                    ...
                    if (ierr != MPI_SUCCESS) {
                        sprintf(info_buf, "MPI_Barrier() returned %d",
                                ierr);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        MPI_Error_string(ierr, &info_buf[0], &size);
                        MPITEST_message(MPITEST_FATAL, info_buf);
