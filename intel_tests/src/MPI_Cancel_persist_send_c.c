...
1).
The root rank will create and start a persistent send request to the
...
NOTE: This test assumes that and outstanding Send may be successfully
...
MPI Calls dependencies for this test:
  MPI_Isend(), MPI_Send_init), MPI_Cancel(), MPI_Test_cancelled(), MPI_Recv(),
  MPI_Init(), MPI_Finalize(), MPI_Comm_test_inter(), MPI_Barrier(),
  MPI_Error_string(), MPI_Wait()
...
int main(int argc, char *argv[])
{
    ...
    *values;                    /*  Array of dataTemplates for verbose init           */
    ...
    sprintf(testname, "MPI_Cancel_persist_send");
            ...
            for (length_count = 0;
                ...
                for (root = 0; root < test_nump; root++) {
                    ...
                    if (MPITEST_current_rank == root) {
                        ...
                        sprintf(info_buf,
                                "Constructing persistent send request from  source %d to %d, tag: %d",
                                root, dest, tag);
                        MPITEST_message(MPITEST_INFO1, info_buf);

                        ierr =
                            MPI_Send_init(buffer, length,
                                          MPITEST_mpi_datatypes[test_type],
                                          dest, tag, comm, &send_request);
                        if (ierr != MPI_SUCCESS) {
                            fail++;
                            sprintf(info_buf,
                                    "MPI_Send_init() returned %d", ierr);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            MPI_Error_string(ierr, &info_buf[0], &size);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                        } else {
                            sprintf(info_buf,
                                    "Starting send request from source %d to %d, tag: %d",
                                    root, dest, tag);
                            MPITEST_message(MPITEST_INFO1, info_buf);
                            ...
                            sprintf(info_buf,
                                    "Cancelling persistent send request ...");
                            MPITEST_message(MPITEST_INFO1, info_buf);
                            ...
                            if (ierr != MPI_SUCCESS) {
                                ...
                                sprintf(info_buf,
                                        "Waiting for send request to be cancelled");
                                MPITEST_message(MPITEST_INFO1, info_buf);
                                ...
                                sprintf(info_buf,
                                        "Checking if the send request has been cancelled");
                                MPITEST_message(MPITEST_INFO1, info_buf);
                    ...
                    if (MPITEST_current_rank == root) {
                        ...
                        sprintf(info_buf,
                                "Sending message 2 from source %d to %d, tag: %d",
                                root, dest, tag);
                        MPITEST_message(MPITEST_INFO1, info_buf);

                        ierr =
                            MPI_Isend(buffer2, length,
                                      MPITEST_mpi_datatypes[test_type],
                                      dest, tag, comm, &send_request);
