...
-----------------------------------------------------------------------------*/
/******************************************************************************
		  Test for MPI_Rsend():  ROOT send to ALL version
		      with MPI_Irecv()

Tests the  blocking MPI_Rsend operation, with both Intra/Inter 
communicators.

The Rsend or Ready mode Send may be started only if the matching receive
has already been posted.  The MPI Standard only states that if the matching
receive has not been posted, the outcome of the operation is undefined.  To
avoid this, MPI_Irecv() is used and the ranks synchronized before the call
to MPI_Rsend().

For an INTRA-communicator, the program selects each node, in turn, 
of the communicator to be the root.  the ROOT SENDS to all of the other
nodes. Root does NOT send to itself.
...
int main(int argc, char *argv[])
{
    ...
    *values;                    /*  Array of dataTemplates for verbose init           */
    ...
    MPI_Request recv_req;       /*  MPI request structure                             */
/*------------------------------  MPI_Init  ---------------------------------*/
    cnt_len = -1;
    ...
    sprintf(testname, "MPI_Rsend_rtoa: Root send TO All others");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            /*  Variables, send_size, and recv_size  both default to the total nodes in
               ...
               MPI_Comm_remote_size returns the size of the opposite communicator 
               for all nodes in each respective communicator
             */


            /*  Set the print_node_flag:
               This is the flag that tells which node gets to print.
               For INTRA-communition it is simply node 0 of the communicator.
               For INTER-communication it is node zero of the left sub-communicator.
             */
...
/*-------------------------  Loop over roots  -------------------------------*/
/*
    For an intra-communicator, we loop over the roots, and for each root 
    each of the other nodes sends to that root.
    For an inter-communicator setting, we have two sets of nodes, left and 
    right, and we want to loop through all combinations. 
*/
...
/* grp_lup = 2  send from right sub-communicator  to left  sub-communicator  */
                    ...
                    for (grp_lup = 1; grp_lup <= ntimes; grp_lup++) {
                        ...
                        /* Set up the number of senders and  receivers depending on sub-group of
                           INTER-communicator  */
...
/*-----------------------------------------------------------------------------
		   POST  RECEIVES
INTRA:  All non-root nodes post a receive for a message from the current root
INTER:  Each node of send sub-communicator gets to be root and send to all 
...
-----------------------------------------------------------------------------*/
                                    ...
                                    && MPITEST_inter == recv_group))
                            {
                                ...
                                ierr = MPI_Irecv(recv_buffer, length,
                                                 MPITEST_mpi_datatypes
                                                 [test_type], root, 32767,
                                                 comm, &recv_req);
                                if (ierr != MPI_SUCCESS) {
                                    sprintf(info_buf,
                                ...
                                if (ierr != MPI_SUCCESS) {
                                    sprintf(info_buf,
                                            "Non-zero return code (%d) from MPI_Wait",
                                            ierr);
                                    MPITEST_message(MPITEST_NONFATAL,
                                                    info_buf);
                                    MPI_Error_string(ierr, &info_buf[0],
                                ...
                                /*  Call MPI_Get_Count, and compare value with length  */
                                cnt_len = -1;
                                ...
                                /*  Print non-fatal error if Received lenth not equal to
                                 *  send length
                                 */
...
-----------------------------------------------------------------------------*/
                            ...
                            if ((!inter_comm
                                ...
                                /*  Since root doesn't receive a message from itself, it should
                                   receive messages from test_nump-1 other nodes, but for INTER
                                ...
                                for (i = 0; i < send_to; i++) {
                                    ...
                                    if (ierr != MPI_SUCCESS) {
                                        sprintf(info_buf,
                                                "Non-zero return code (%d) from MPI_Send",
                                                ierr);
                                        MPITEST_message(MPITEST_NONFATAL,
                                                        info_buf);
                            ...
                            } /* End of test for not root node, root node only sends */
                            else if ((inter_comm
                                      && MPITEST_inter == send_group
                                      && MPITEST_current_rank != root)) {
                                /* Need a matching barrier for INTER comms in the send group
                                   not the root, since they won't hit the Barrier in either
                                   the send or receive loops above */
                                ierr = MPI_Barrier(barrier_comm);
                                if (ierr != MPI_SUCCESS) {
                                    sprintf(info_buf,
                                            "Non-zero return code (%d) from MPI_Barrier",
