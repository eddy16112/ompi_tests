...
/******************************************************************************
               ...
               MPI_Sendrecv funtional Root to All test
...
int main(int argc, char *argv[])
{
    int
     ...
     node_cnt_len,              /*  received length returned by MPI_Get_Count         */
     root_cnt_len, comm_count,  /*  loop counter for communicator loop                */
     ...
     node_send_length,          /*  message buffer length for node to send            */
     node_recv_length,          /*  message buffer length for node receive            */
     root_send_length,          /*  message buffer length for root to send            */
     root_recv_length,          /*  message buffer length for root to receive         */
    ...
    *ub,                        /*  Pointer to value of maximum tag                   */
    ...
    void *node_recv_buffer;
    void *node_send_buffer;     /* message buffer                              */
    void *root_recv_buffer;
    void *root_send_buffer;     /* message buffer                              */
    ...
    MPI_Status node_stat, root_stat;
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        /* Initialize INTER-communicator variables   */
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {    /* Skip to barrier if
            ...
            if (inter_comm) {   /* Do this stuff for INTER-communicators */
                ...
                if (MPITEST_inter == 0) {       /* Do this stuff for left group of an INTER-communicator */
                    ...
                    if (MPITEST_current_rank == 0) {    /* Node zero of left bank of INTER-communicator */
                        print_node_flag = 1;
                    }
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                for (length_count = 0;
                    ...
                    node_send_length = root_send_length;
                    root_recv_length = root_send_length;
                    node_recv_length = root_send_length;
                    ...
                    for (grp_lup = 1; grp_lup <= ntimes; grp_lup++) {
                        ...
                        for (root = 0; root < senders; root++) {
                            ...
                            if (print_node_flag) {
                                sprintf(info_buf,
                                        "(%d,%d,%d) node_recv_length %d commsize %d commtype %d data_type %d root %d",
                                        length_count, comm_count,
                                        type_count, node_send_length,
			    ...
			    -------------------------------------------------*/
                            ...
                            if ((!inter_comm
                                ...
                                MPITEST_dataTemplate_init(&node_value,
                                ...
                                ierr = MPI_Sendrecv(node_send_buffer,
                                                    node_send_length,
 ...
 /*-------------------------------------------------
			    ...
			    -------------------------------------------------*/
                                    ...
                                    && MPITEST_current_rank == root))
                            {
                                ...
                                MPITEST_dataTemplate_init(&root_value,
                                                          MPITEST_current_rank);
                                ...
                                for (i = 0; i < send_to; i++) {
                                    ...
                                    ierr = MPI_Sendrecv(root_send_buffer,
                                                        root_send_length,
                                                        MPITEST_mpi_datatypes
                                                        [test_type], i,
                                                        root,
                                    ...
                                    if (ierr != MPI_SUCCESS) {
                                        sprintf(info_buf,
                                                "Non-zero return code (%d) from root MPI_Sendrecv",
                                                ierr);
                                        MPITEST_message(MPITEST_NONFATAL,
                                                        info_buf);
                                    ...
                                    error =
                                        ...
                                        (test_type, root_recv_length,
                                         root_value, root_recv_buffer);
                                     ...
                                     * correct.
                                     */
                                    if (root_stat.MPI_TAG !=
                                        root_stat.MPI_SOURCE) {
                                        sprintf(info_buf,
                                                "Unexpected root recvtag root_value=%d, expected=%d",
                                                root_stat.MPI_TAG,
                                                root_stat.MPI_SOURCE);
                                        MPITEST_message(MPITEST_NONFATAL,
                                                        info_buf);
                ...
                free(root_send_buffer);
                free(root_recv_buffer);
                free(node_send_buffer);
                free(node_recv_buffer);
            }                   /* Loop over Data Types  */
