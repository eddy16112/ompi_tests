...
-----------------------------------------------------------------------------*/
/******************************************************************************
	    Test for MPI_Sendrecv_replace() to/from MPI_PROC_NULL

This tests MPI_Sendrecv_replace operation to/from MPI_PROC_NULL.  No
...
int main(int argc, char *argv[])
{
    ...
    MPI_Status sendrecv_stat;   /* MPI status structure                      */
    /*-----------------------------  MPI_Init  ------------------------------*/
    ...
    sprintf(testname, "MPI_Sendrecv_replace_null");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                MPITEST_get_buffer(test_type, max_length, &iobuffer);
                ...
                for (length_count = 0;
                    ...
                    sprintf(info_buf,
                            ...
                            send_length, test_nump, comm_type, test_type);
                    if (MPITEST_current_rank == 0)
                        MPITEST_message(MPITEST_INFO1, info_buf);

                    /*-------------------------------------------------------
	                             Sendrecv_replace
		    All nodes send/receive a message to/from MPI_PROC_NULL
		    --------------------------------------------------------*/
                    ...
                    ierr = MPI_Sendrecv_replace(iobuffer,
                                                ...
                                                [test_type], MPI_PROC_NULL,
                                                1, MPI_PROC_NULL, 2, comm,
                                                &sendrecv_stat);
                    ...
                    if (ierr != MPI_SUCCESS) {
                        ...
                        sprintf(info_buf,
                                "Non-zero return code (%d) from MPI_Sendrecv_replace",
                                ierr);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                     ...
                     */
                    cnt_len = -1;
                    ierr = MPI_Get_count(&sendrecv_stat,
                    ...
                    /*
                     * Print non-fatal error if Received lenth not equal to 0
                     */
                    if (cnt_len != 0) {
                        sprintf(info_buf,
                                "Receive length = %d, expected 0",
                                cnt_len);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        loop_fail++;
                    }
                     ...
                     */
                    if (sendrecv_stat.MPI_TAG != MPI_ANY_TAG) {
                        sprintf(info_buf,
                                "Unexpected tag value=%d, expected MPI_ANY_TAG",
                                sendrecv_stat.MPI_TAG);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        loop_fail++;
                    }
                    /*
                     ...
                     */
                    if (sendrecv_stat.MPI_SOURCE != MPI_PROC_NULL) {
                        sprintf(info_buf,
                                "Unexpected source value=%d, expected MPI_PROC_NULL",
                                sendrecv_stat.MPI_SOURCE);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        loop_fail++;
                    }
