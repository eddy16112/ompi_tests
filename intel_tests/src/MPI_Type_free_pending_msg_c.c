...
Test to verify that pending communication using user defined type will be
completed even after the user type has been marked for deallocation
using MPI_Type_free().

All ranks will first do a MPI_Type_struct() of all types in the type config
array (in mpitest_cfg.h).  Root's rank will then do a Isend() using the newly
created type followed by a MPI_Type_free() of that type before any recv has
been posted.  All participating will then synchronize and the destination rank
will then receive and verify the data using the user datatype.
...
MPI Calls dependencies for this test:
  MPI_Isend(), MPI_Irecv(), MPI_Init(), MPI_Finalize()
  MPI_Comm_test_inter(), MPI_Error_string(), MPI_Wait(),
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (length_count = 0;
                ...
                for (root = 0; root < test_nump; root++) {
                    ierr =
                        MPI_Type_struct(basic_types, blklens, displs,
                                        types, &newtype);
                    if (ierr != MPI_SUCCESS) {
                    ...
                    if (MPITEST_current_rank == root) {
...
#endif
                        ierr = MPI_Isend(buffer, length, newtype,
                        ...
                        sprintf(info_buf, "Freeing user defined type");
                        MPITEST_message(MPITEST_INFO1, info_buf);
                        ...
                        if (ierr != MPI_SUCCESS) {
                            ...
                            fail++;
                            sprintf(info_buf,
                                    "type is not set to MPI_DATATYPE_NULL by MPI_Type_free()");
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                        }
                    }

                    /* Synchronzing so we can be sure that the recv has not been posted yet */
                    sprintf(info_buf, "Synchronzing with all ranks...");
                    MPITEST_message(MPITEST_INFO1, info_buf);
                    ...
                    if (MPITEST_current_rank == dest) {
                        ...
                        ierr = MPI_Irecv(buffer, length, newtype,
                        ...
                        /* Wait for the recv request to be finished */
                        sprintf(info_buf,
                                "Waiting for recv request to be completed");
                        MPITEST_message(MPITEST_INFO1, info_buf);
                        ...
                        if (ierr != MPI_SUCCESS) {
                            ...
                            if (status.MPI_SOURCE != root) {
                                fail++;
                                sprintf(info_buf,
                                        "status object returned from MPI_Wait() has unexpected MPI_SOURCE field"
                            ...
                            if (status.MPI_TAG != tag) {
                                fail++;
                                sprintf(info_buf,
                                        "status object returned	from MPI_Wait() has uncexpected MPI_TAG."
                                        "  Expected: %d, Actual: %d",
                                        tag, status.MPI_TAG);
                                MPITEST_message(MPITEST_NONFATAL,
                                                info_buf);
                            ...
                            /* Also not checking the receive length since user type has been freed */
#else
                            sprintf(info_buf,
