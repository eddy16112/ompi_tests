...
-----------------------------------------------------------------------------*/
/******************************************************************************
        Test for Persistent  MPI_Irecv_init() from MPI_PROC_NULL

This tests the persistent  MPI_Recv_init  operation from MPI_PROC_NULL.  No
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname,
            "MPI_Recv_init_null: all persistent Recv from MPI_PROC_NULL");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
		    ...
		    -------------------------------------------------*/
                    ...
                    /* Irecv Init Error Test  */
                    /* Start the Receive */
                    ierr = MPI_Start(&recv_request);

                    if (ierr != MPI_SUCCESS) {
                        sprintf(info_buf,
                                "Non-zero return code (%d) from MPI_Start",
                                ierr);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        MPI_Error_string(ierr, &info_buf[0], &size);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
 ...
 /*------------- Call MPI_Request_free --------------*/
                    ...
                    if (recv_stat.MPI_SOURCE != MPI_PROC_NULL) {
                        sprintf(info_buf,
                                "Unexpected source value=%d, expected MPI_PROC_NULL(%d)",
                                recv_stat.MPI_SOURCE, MPI_PROC_NULL);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        loop_fail++;
                    }
