...
-----------------------------------------------------------------------------*/
/******************************************************************************
		  Test for MPI_Bsend():  ALL Bsend to ROOT
                  and MPI_Recv(): Any source/tag specified

Bsend Inner buffering, SEND before RECEIVE, ALL-to-ROOT.

Tests the buffered blocking MPI_Bsend operation, for both Intra and Inter
communicators.
...
before a matching receive is posted.  Unlike the basic send, BSEND is local.
...
Each Bsend receives from  all other nodes, including itself.
...
Bsend seperately for each root.  After the root is chosen, the buffer is
...
The MPI_Bsend function sends a message from one node to each other
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Bsend_ator: All Bsend TO Root");
