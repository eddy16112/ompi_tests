...
int main(int argc, char *argv[])
{
    int
     ...
     inter_comm,                /* Intercommunicator flag, true if intercomm         */
   ...
   /*------------------------------  MPI_Init  -----------------------------*/
    ...
    sprintf(testname, "MPI_Graph_create with reorder = false");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            /*--------------------  MPI_Graph_create  ----------------------*/
                ...
                if (MPITEST_current_rank != rank_graph) {
                    fail++;
                    sprintf(info_buf,
                            "ERROR -- Ranks reordered  old/new = %3d/%3d  ",
                            MPITEST_current_rank, rank_graph);
                    MPITEST_message(MPITEST_FATAL, info_buf);
                }
