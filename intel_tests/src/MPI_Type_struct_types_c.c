...
array (in mpitest_cfg.h).  Rank 0 then send data using the new type of length
determined by the message length array in mpitest_cfg.h.  The destination rank
will then receive and verify the data byte by byte.
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Type_struct_types");
            ...
            for (length_count = 0;
                ...
                for (root = 0; root < test_nump; root++) {
                    ...
                    if (MPITEST_current_rank == root) {
                        ...
                        ierr = MPI_Send(buffer_gpu, length, newtype,
                                        dest, tag, comm);
#else 
                        ierr = MPI_Send(buffer, length, newtype,
                                        dest, tag, comm);
#endif
                        if (ierr != MPI_SUCCESS) {
                            fail++;
...
#if defined (CUDA_TEST_G2G)
                        ierr = MPI_Recv(buffer_gpu, length, newtype,
                                        root, tag, comm, &status);
#else
                        ierr = MPI_Recv(buffer, length, newtype,
                                        root, tag, comm, &status);
#endif
                        if (ierr != MPI_SUCCESS) {
                            fail++;
