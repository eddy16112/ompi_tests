...
-----------------------------------------------------------------------------*/
/******************************************************************************
	  Test for MPI_Send()/MPI_Recv; flood one node with messages

This tests the basic blocking MPI_Send operation, with one node flooding
another with messages before they are received.  This tests the buffering
of Sends and Receives (if any), and the ability to recover if the buffers
are filled.
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Send_flood");
    ...
    if (MPITEST_nump < 3) {
        MPITEST_message(MPITEST_INFO0,
                        "This test requires at least 3 ranks");
    ...
    if (MPITEST_me == 0) {
        ...
        for (loop_cnt = 0; loop_cnt < LOOP_SIZE; loop_cnt++) {
            ...
            while (loop_fail < LOOP_SIZE) {
                ...
                if (loop_fail == 45) {
                    sprintf(info_buf,
                            "Rank 1 sent %d mesasages containing %d characters before filling buffer on node 2",
                            loop_cnt, max_length);
                    MPITEST_message(MPITEST_INFO0, info_buf);
                }
                loop_fail++;
    ...
    /* rank 0 waits for message */
        ...
        if (MPITEST_me == 1) {  /* rank 1 does sends */
            /*-------------------------------------------------
	                       Send
	    ...
	    -------------------------------------------------*/
            ...
            for (loop_cnt = 0; loop_cnt < LOOP_SIZE; loop_cnt++) {
                ...
                ierr = MPI_Send(send_buffer, max_length,
                                MPITEST_mpi_datatypes[test_type],
                                2, loop_cnt, MPI_COMM_WORLD);
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
                ...
                ierr =
                    MPI_Send(send_buffer, 0, MPI_INT, 0, 1,
                             MPI_COMM_WORLD);
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
                            "Non-zero return code (%d) from MPI_Send signal",
                            ierr);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            for (loop_cnt = 0; loop_cnt < LOOP_SIZE; loop_cnt++) {
                ...
                ierr = MPI_Recv(recv_buffer, max_length,
                                MPITEST_mpi_datatypes[test_type],
                                1, loop_cnt, MPI_COMM_WORLD, &recv_stat);
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
                ...
                if (error) {
                    sprintf(info_buf, "%d errors in received buffer",
                            error);
                    loop_fail++;
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                }
                ...
                if (error) {
                    sprintf(info_buf,
                            ...
                            recv_stat.MPI_SOURCE, max_length, MPITEST_me,
                            cnt_len);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    loop_fail++;
                }
