...
int main(int argc, char *argv[])
{
    ...
    *attr,                      /* Returned value of the attribute */
     ...
     result, resultsplit, resultinter, size, send1,     /* Used for traffic while          */
     send2,                     /*  generating the intercomm       */
     send3, recv1, recv2, recv3;
    ...
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Keyval_create", ierr);
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_test_inter (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_split (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if (MPITEST_current_rank < 2) {
                ...
                send3 = 29;
                ierr =
                    MPI_Isend(&send1, 1, MPI_INT, 1 - MPITEST_current_rank,
                              26, comm, &req1);
                ierr =
                    MPI_Isend(&send2, 1, MPI_INT, 1 - MPITEST_current_rank,
                              28, comm, &req2);
                ierr =
                    MPI_Isend(&send3, 1, MPI_INT, 1 - MPITEST_current_rank,
                              0, comm, &req3);
            }
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Intercomm_create (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if (MPITEST_current_rank < 2) {
                ierr =
                    MPI_Recv(&recv1, 1, MPI_INT, 1 - MPITEST_current_rank,
                             26, comm, &stat1);
                ierr =
                    MPI_Recv(&recv2, 1, MPI_INT, 1 - MPITEST_current_rank,
                             28, comm, &stat2);
                ierr =
                    MPI_Recv(&recv3, 1, MPI_INT, 1 - MPITEST_current_rank,
                             0, comm, &stat3);
                ...
                if ((recv1 != send1) || (recv2 != send2) ||
                    (recv3 != send3)) {
                    sprintf(info_buf,
                            "Receive data corrupted by MPI_Intercomm_create: %d/%d/%d (expected 19/29/39) (commm_index %d)",
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_compare (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if (result != MPI_UNEQUAL) {
                sprintf(info_buf,
                        "MPI_Comm_compare returned %d, expected MPI_UNEQUAL (comm_index %d)",
                        result, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Attr_put",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Attr_get(comminter)",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                ...
                fail++;
                sprintf(info_buf,
                        "MPI_Attr_get(comminter) did not find attribute (comm_index %d).",
                        comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            } else if (*attr != 27) {
                fail++;
                sprintf(info_buf,
                        "MPI_Attr_get(comminter) returned value %d, expected 27 (comm_index %d).",
                        *attr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            }
            loop_cnt++;
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_test_inter #2 (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if (resultinter != 1) {
                sprintf(info_buf,
                        "MPI_Comm_test_inter returned %d, expected 1 (comm_index %d)",
                        resultinter, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            loop_cnt++;
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_size (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if (resultinter != result) {
                sprintf(info_buf,
                        "MPI_Comm_size returned %d, expected %d (comm_index %d)",
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_remote_size (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if (resultinter != result) {
                sprintf(info_buf,
                        "MPI_Comm_remote_size returned %d, expected %d (comm_index %d)",
                        resultinter, result, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_rank (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if (resultinter != MPITEST_current_rank / 2) {
                sprintf(info_buf,
                        "MPI_Comm_rank returned %d, expected %d (comm_index %d)",
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Bcast", ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
                ...
                fail++;
                sprintf(info_buf,
                        "MPI_Bcast received incorrect data %d, expected %d (comm_index %d).",
                        result, color, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            }
            loop_cnt++;
#endif
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_dup (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Attr_get(comm2)",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                ...
                fail++;
                sprintf(info_buf,
                        "MPI_Attr_get(comm2) did not find attribute (comm_index %d).",
                        comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            } else if (*attr != 27) {
                fail++;
                sprintf(info_buf,
                        "MPI_Attr_get(comm2) returned value %d, expected 27 (comm_index %d).",
                        *attr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            }
            loop_cnt++;
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Attr_delete after delete",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_compare #2 (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if (result != MPI_CONGRUENT) {
                sprintf(info_buf,
                        "MPI_Comm_compare #2 returned %d, expected MPI_CONGRUENT (comm_index %d)",
                        result, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_free(commsplit) (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (commsplit != MPI_COMM_NULL) {
                sprintf(info_buf,
                        "comm = %d after MPI_Comm_free(commsplit), expected MPI_COMM_NULL (comm_index %d)",
                        commsplit, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            loop_cnt++;
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_free(comminter) (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (comminter != MPI_COMM_NULL) {
                sprintf(info_buf,
                        "comm = %d after MPI_Comm_free(comminter), expected MPI_COMM_NULL (comm_index %d)",
                        comminter, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            loop_cnt++;
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_free(comm2) (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (comm2 != MPI_COMM_NULL) {
                sprintf(info_buf,
                        "comm = %d after MPI_Comm_free(comm2), expected MPI_COMM_NULL (comm_index %d)",
                        comm2, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
    ...
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf, "Non-zero return code (%d) from MPI_Keyval_free",
                ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
