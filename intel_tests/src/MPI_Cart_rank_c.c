            ...
            /*---------------------- MPI_Cart_rank   ------------------------*/
                ...
                if (rank_cart == 0) {
                    ...
                    for (k = 0; k < nnodes; k++) {
                        ...
                        if (ierr != MPI_SUCCESS) {
                            fail++;
                            sprintf(info_buf, "MPI_Cartdim_get error (%d)",
                                    ierr);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            MPI_Error_string(ierr, &info_buf[0], &size);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                        ...
                        if (test_rank < 0 || test_rank >= nnodes) {
                            sprintf(info_buf,
                                    "ERROR  MPI_Cart_rank: (Out of range)  Actual = %3d   Expected = 0 - %3d",
                                    test_rank, nnodes);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                        }
                        ranks[test_rank] = 1;
                        ...
                        sprintf(info_buf,
                                "%5d.   Counter:  %3d%3d%3d%3d    Rank:  %3d ",
                                k, counter[0], counter[1], counter[2],
                                counter[3], test_rank);
                    ...
                    for (i = 0; i < nnodes; i++) {
                        if (ranks[i] != 1) {
                            sprintf(info_buf, "ERROR:  Missing node = %d",
                                    i);
                            MPITEST_message(MPITEST_FATAL, info_buf);
                        }
                    }
