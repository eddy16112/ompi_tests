...
(in mpitest_cfg.h) using MPI_Type_indexed().  The root's rank will then send
a message using the new type to the destination rank (circular).  The
destination rank will then do a recv().  The received data will then be
...
#define MPITEST_ADD_BUFSIZE   200
...
int main(int argc, char *argv[])
{
    ...
    int *blklens, *blklens1, *displs1, num_types, *type_sizes, basic_types;
    ...
    sprintf(testname, "MPI_Type_indexed_types");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (length_count = 0;
                ...
                if (!displs1) {
                    sprintf(info_buf,
                            "Cannot allocate enough memory for displs1 array");
                    MPITEST_message(MPITEST_FATAL, info_buf);
                }
                ...
                if (!blklens1) {
                    sprintf(info_buf,
                            "Cannot allocate enough memory for blklens1 array");
                    MPITEST_message(MPITEST_FATAL, info_buf);
                }
