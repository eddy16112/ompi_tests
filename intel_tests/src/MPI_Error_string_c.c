...
test_error(int error, char *errstr, int *fail)
{
    int
    ...
    char
     string[MPI_MAX_ERROR_STRING], info_buf[256];       /* buffer for passing mesages to MPITEST         */

    MPITEST_message(MPITEST_VERIFY, errstr);
    ierr = MPI_Error_string(error, string, &len);
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Error_string", ierr);
        ...
        *fail = *fail + 1;
        ...
        if ((len > MPI_MAX_ERROR_STRING) || (len != strlen(string))) {
            *fail = *fail + 1;
            sprintf(info_buf,
                    "Returned length=%d, MPI_MAX_ERROR_STRING=%d", len,
                    MPI_MAX_PROCESSOR_NAME);
            MPITEST_message(MPITEST_NONFATAL, info_buf);
            MPITEST_message(MPITEST_INFO0, string);
        } else {
            sprintf(info_buf, "Returned string length=%d (OK), string:",
                    len);
...
int main(int argc, char *argv[])
{
    ...
    if (MPITEST_me == 0) {
        ...
        MPITEST_message(MPITEST_VERIFY,
                        "Please verify the following error strings");
    }
