...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Isend_overtake");
    ...
    if (MPITEST_me < 2) {
        ...
        if (MPITEST_me == 0) {  /* sender */
            ...
            for (i = 0; i < NUMSHORT; i++) {
                ierr = MPI_Isend(&shortb[i], 1, MPI_CHAR, 1, 1,
                                 MPI_COMM_WORLD, &send_request);
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
            ...
            ierr = MPI_Irecv(&buffer, LONGLENGTH, MPI_CHAR, 0, MPI_ANY_TAG,
            ...
            /* Wait Error Test  */
            for (i = 0; i < NUMSHORT; i++) {
                ierr = MPI_Irecv(&shortb[i], 1, MPI_CHAR, 0,
