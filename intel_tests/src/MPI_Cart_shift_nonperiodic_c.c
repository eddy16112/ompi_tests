...
/******************************************************************************
 ...
 All entries in the periods array are set to false.  Thus, MPI_Cart_shift
 should return MPI_PROC_NULL  for the rank_source and/or the rank_dest
 parameters if the shift brings one or both of the fields out of range

 11-04-02
 This test is incorrect.  The standard clearly says under the discussion 
 of MPI_Cart_rank that "Out-of-range coordinates are erroneous for 
...
int main(int argc, char *argv[])
{
   ...
   if (MPITEST_me == 0)
  {
    ...
    {
      fail++;
      sprintf(info_buf, "WARNING --  nodes = %3d   Need %3d nodes to run test", world_nump, MPITEST_MINNODE);
...

  {
	...
	for (j = 0; j < ndims; j++)
	{
	  ...
	  for (k = -1; k <= 1; k++)
	  {
	    ...
	    if (k < 0)
	    {
	      /* sorc is coords[j]|k|, MPI_PROC_NULL, or periodic  value */
	      exp_sorc = coords[j] - k;
	    ...
	    if (k > 0)
	    {
	      /* sorc is coords[j] - k, MPI_PROC_NULL, or periodic valu */
	      exp_sorc = coords[j] - k;
	    ...
	    /* MPI_PROC_NULL is not valid for a coordinate to MPI_Cart_rank */
	  ...
	  }	/* End of k loop over displacements for each coordinate  */

	}	/* End of j loop over all dimensions for this node */
