...
-----------------------------------------------------------------------------*/
/******************************************************************************
                 MPI_Send_init Persistent Send:   Root to All test

                      MPI_Send_init():  Root to all
                      with MPI_Start to send/receive an array of requests
                      with MPI_Wait  to check completion
                      and  MPI_Recv_init()
...
and a MPI_Start, or MPI_Startall is then used to initiate each communication.
...
int main(int argc, char *argv[])
{
    ...
    *values;                    /* Array of dataTemplates for verbose init           */
    ...
    sprintf(testname, "MPI_Send_init_rtoa:  Persistent Send  Root to All");
    ...
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf, "Non-zero return code (%d) from MPI_Attr_get",
                ierr);
    ...
    if (found != 1) {
        MPITEST_message(MPITEST_FATAL,
                        "MPI_Attr_get(MPI_TAG_UB) returned FALSE");
    }
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {    /* Skip to barrier if
             ...
             * communicator
             * 
             * Set the print_node_flag: This is flag tells which rank is to
             * print.  For INTRA-communition it is simply node 0.  For
             * INTER-communication it is node zero of the left group.
             */

            if (inter_comm) {   /* Do this stuff for INTER-communicators */

                if (MPITEST_inter == 0) {       /* Do this stuff for left group of an
                                                 * INTER-communicator */
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                for (length_count = 0;
                     ...
                     * other nodes. For an inter-communicator setting, we
                     * have two sets of nodes, in left and right
                     * sub-communicators, and each node in both
                     * sub-communicators gets to be the root, and send to all
                     * the nodes of the other sub-communicator.
                     */
 ...
 /*-------- Call MPI_Request_free -------*/
                                    ...
                                    if (send_request != MPI_REQUEST_NULL) {
                                         ...
                                         * current root to all other nodes  */
