...
operation when n (number of ranges)=0.
...
int main(int argc, char *argv[])
{
    int
     ...
     ranges[3][3],              /* Array of rank ranges in group   */
    ...
    sprintf(testname, "MPI_Group_range_incl1");
    ...
    ierr = MPI_Group_range_incl(groupworld, 0, ranges, &group);
    if (ierr != MPI_SUCCESS) {
        sprintf(info_buf,
                "Non-zero return code (%d) from MPI_Group_range_incl #2",
                ierr);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        MPI_Error_string(ierr, &info_buf[0], &size);
        MPITEST_message(MPITEST_NONFATAL, info_buf);
    ...
    if (group != MPI_GROUP_EMPTY) {
        sprintf(info_buf,
                "MPI_Group_range_incl of size 0 did not return MPI_GROUP_EMPTY)");
        MPITEST_message(MPITEST_NONFATAL, info_buf);
        fail++;
    }
