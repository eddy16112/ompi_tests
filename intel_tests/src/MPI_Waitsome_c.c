...
int main(int argc, char *argv[])
{
    ...
    MPI_Status temp_stat[4 * NUMMESG], send_stat[4 * NUMMESG],  /* Send Source/Tag information               */
    ...
    if (MPITEST_me == 0) {
        ...
        while (send_cnt != MPI_UNDEFINED) {
            ierr =
                MPI_Waitsome(4 * NUMMESG, send_request, &send_cnt,
                             send_index, temp_stat);
            total_cnt++;
            i++;
            sprintf(info_buf,
                    "Waitsome on send: Loop  %d   Index_cnt %d   Error %d",
                    i, send_cnt, ierr);
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Waitsome on send",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
 ...
 /*---------------------------------  Irecv  -----------------------------*/
    if (MPITEST_me == 1) {
        ...
        while (recv_cnt != MPI_UNDEFINED) {
            ierr =
                MPI_Waitsome(4 * NUMMESG, recv_request, &recv_cnt,
                             recv_index, temp_stat);
            j++;
            total_cnt++;
            sprintf(info_buf,
                    "Waitsome on receive: Loop %d  recv_cnt %d  Error %d ",
                    i, recv_cnt, ierr);
            MPITEST_message(MPITEST_INFO1, info_buf);
            ...
            if (ierr != MPI_SUCCESS) {
                fail++;
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Waitsome on receive",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
        ...
        for (i = 0; i < 4 * NUMMESG; i++) {
            ...
            if (recv_stat[i].MPI_TAG != i) {
                ...
                /*-------------------  Second  Waitsome  ------------------- */
        ierr =
            MPI_Waitsome(4 * NUMMESG, recv_request, &recv_cnt, recv_index,
                         recv_stat);

        sprintf(info_buf,
                "Waitsome on receive: Loop %d  recv_cnt %d  Error %d ", i,
                recv_cnt, ierr);
        MPITEST_message(MPITEST_INFO1, info_buf);
        ...
        if (ierr != MPI_SUCCESS) {
            sprintf(info_buf,
                    "Non-zero return code (%d) from second MPI_Waitsome on receive",
                    ierr);
            MPITEST_message(MPITEST_NONFATAL, info_buf);
        ...
        for (i = 0; i < 4 * NUMMESG; i++) {
            if (recv_request[i] != MPI_REQUEST_NULL) {
                sprintf(info_buf,
                        "recv_request[%d] not equal to  MPI_REQUEST_NULL(%p/%p) after second Waitsome",
