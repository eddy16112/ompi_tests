...
int main(int argc, char *argv[])
{
    ...
    MPI_Aint extent;            /*  extent returned from MPI_Type_extent              */
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            if (extent != 0) {
                fail++;
                sprintf(info_buf,
                        "Unexpected return calling MPI_Type_extent() w/ MPI_LB Expected: %d, Actual: %d",
                        0, extent);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            }
            ...
            if (extent != 0) {
                fail++;
                sprintf(info_buf,
                        "MPI_Type_extent() w/ MPI_UB returned unexpected %d Expected: %d, Actual: %d",
                        0, extent);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            }
        }
