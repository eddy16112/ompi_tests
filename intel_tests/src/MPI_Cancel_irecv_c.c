...
1).
The root rank will submit Irecv() to the next rank (circular).
...
NOTE: This test assumes that any Irecv that has not yet matched a send
request can be successfully cancelled.
...
int main(int argc, char *argv[])
{
    ...
    *values;                    /*  Array of dataTemplates for verbose init           */
    ...
    sprintf(testname, "MPI_Cancel_irecv");
            ...
            for (length_count = 0;
                ...
                for (root = 0; root < test_nump; root++) {
                    ...
                    if (MPITEST_current_rank == root) {
                        ...
                        sprintf(info_buf,
                                "Receiving message from %d, tag: %d",
                                source, tag);
                        MPITEST_message(MPITEST_INFO1, info_buf);
                    ...
                    if (MPITEST_current_rank == root) {
                        /* Reusing the recv request handle for another transmission */
                        sprintf(info_buf,
