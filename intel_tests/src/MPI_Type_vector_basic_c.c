...
All rank will call MPI_Type_vector() and then MPI_Type_commit() to create
...
The input count to MPI_Type_vector() is determined by each message length
in mpitest_cfg.h and the input oldtype to MPI_Type_vector() is determined
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Type_vector_basic");
