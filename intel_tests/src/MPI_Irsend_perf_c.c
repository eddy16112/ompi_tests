...
/******************************************************************************
 ...
 so that the results can be reliably interpreted.  This test assumes that
 an Irecv can be posted before the message can propogate around the ring.
...
#define ALIGN	8192
char *malloc_buffer1, *buffer1, *bufp1;
char *malloc_buffer2, *buffer2, *bufp2;
/*
 * Procedure:
 *	runtest(length)
 *
...
double runtest(int length, int count, MPI_Comm comm)
{
    ...
    if (odd_flag) {
        bufp1 = &buffer1[odd_flag];
        bufp2 = &buffer2[odd_flag];
    } else {
        bufp1 = buffer1;
        bufp2 = buffer2;
    }
    ...
    if (rank == 0) {
        MPI_Barrier(comm);
        sleep(2);
        MPI_Irecv(bufp2, length, MPI_BYTE, MPI_ANY_SOURCE, 0, comm, &rreq);
        starttime = MPI_Wtime();
        MPI_Irsend(bufp1, length, MPI_BYTE, next, 0, comm, &sreq);
        ...
        if (msg_count != 1)
            MPI_Irecv(bufp2, length, MPI_BYTE, MPI_ANY_SOURCE, 0, comm,
                      &rreq);
        MPI_Irsend(bufp1, length, MPI_BYTE, next, 0, comm, &sreq);
        MPI_Wait(&sreq, &status);
        msg_count--;
...
makebuf()
{
    ...
    malloc_buffer1 = malloc(maxlength + ALIGN + 1);
    malloc_buffer2 = malloc(maxlength + ALIGN + 1);
    buffer1 = malloc_buffer1;
    buffer2 = malloc_buffer2;

    if ((buffer1 == 0) || (buffer2 == 0)) {
        sprintf(info_buf, "not enough memory for buffer %d",
                maxlength + ALIGN + 1);
    ...
    buffer1 =
        (char *) ((((unsigned long) buffer1) + ALIGN - 1) & ~(ALIGN - 1));
    buffer2 =
        (char *) ((((unsigned long) buffer2) + ALIGN - 1) & ~(ALIGN - 1));
    ...
    sprintf(info_buf, "Buffer address %08X  length %d",
            buffer1, maxlength);
    MPITEST_message(MPITEST_INFO1, info_buf);
}
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Irsend_perf");
    ...
    if (MPITEST_nump < 2)
        MPITEST_message(MPITEST_FATAL,
                        "This test requires at least 2 ranks");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank == 0) {
            sprintf(info_buf, "Start commsize %d (%d messages)", test_nump,
                    test_msg_count);
            MPITEST_message(MPITEST_INFO0, info_buf);
        }
        ...
        for (length_count = 0;
            ...
            for (ri = test_repeat; ri; ri--) {
                ...
                for (si = test_steps, byte_length = lstart;
                     si; si--, byte_length += test_increment) {
