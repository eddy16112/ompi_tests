...
rank will then do a MPI_Iprobe() followed by a MPI_Recv().  The buffer
...
this source).  The status object returned from MPI_Iprobe() may or may not
...
MPI Calls dependencies for this test:
  MPI_Isend(), MPI_Recv(), MPI_Cancel(), MPI_Iprobe(), MPI_Init(), MPI_Finalize()
...
int main(int argc, char *argv[])
{
    ...
    *values;                    /*  Array of dataTemplates for verbose init           */
    ...
    sprintf(testname, "MPI_Iprobe_source");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (root = 0; root < test_nump; root++) {
                ...
                MPITEST_init_buffer(test_type, length, value, buffer);
                ...
                if (MPITEST_current_rank != root) {
                    sprintf(info_buf, "Sending from source: %d, tag: %d",
                            MPITEST_current_rank, tag);
                    MPITEST_message(MPITEST_INFO1, info_buf);

                    ierr =
                        MPI_Isend(buffer, length,
                    ...
                    /* Call MPI_Iprobe() with MPI_ANY_SOURCE first */

                    sprintf(info_buf,
                            "Probing source: MPI_ANY_SOURCE (%d), tag: %d",
                            MPI_ANY_SOURCE, tag);
                    MPITEST_message(MPITEST_INFO1, info_buf);

                    do {
                        ierr =
                            MPI_Iprobe(MPI_ANY_SOURCE, tag, comm, &flag,
                                       &status);
                        if (ierr != MPI_SUCCESS) {
                            fail++;
                            sprintf(info_buf,
                                    "MPI_Iprobe() returned %d, status.MPI_ERROR = %d",
                    ...
                    if (status.MPI_SOURCE < 0) {
                        fail++;
                        sprintf(info_buf,
                                "status object returned from MPI_Iprobe() has negative MPI_SOURCE (%d) field",
                                status.MPI_SOURCE);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                    } else if (status.MPI_SOURCE >= test_nump) {
                        fail++;
                        sprintf(info_buf,
                                "status object returned from MPI_Iprobe() has too large MPI_SOURCE (%d) field",
                                status.MPI_SOURCE);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                    }
                    ...
                    if (ierr != MPI_SUCCESS) {
                        ...
                        if (count != length) {
                            fail++;
                            sprintf(info_buf,
                                    "status object returned from MPI_Iprobe() contains unexpected length."
                                    "  Expected: %d, Actual: %d", length,
                                    count);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                        }
                    }
                    ...
                    /* Call MPI_Iprobe() with other source ranks */
                    for (source = 0; source < test_nump; source++) {
                        if (source != root) {
                            sprintf(info_buf,
                                    "Probing source: %d, tag: %d", source,
                                    tag);
                            MPITEST_message(MPITEST_INFO1, info_buf);
                ...
                if (MPITEST_current_rank != root) {
                    /* Wait on message sent to root from each rank */
                ...
                if (MPITEST_current_rank != root) {
                    ...
                    sprintf(info_buf,
                            "Cancelling message from source: %d, tag: %d",
                            MPITEST_current_rank, tag);
                    MPITEST_message(MPITEST_INFO1, info_buf);
                    ...
                    if (ierr != MPI_SUCCESS) {
                        fail++;
                        sprintf(info_buf, "MPI_Cancel() returned %d",
                                ierr);
                        MPITEST_message(MPITEST_NONFATAL, info_buf);
                        MPI_Error_string(ierr, &info_buf[0], &size);
                        MPITEST_message(MPITEST_FATAL, info_buf);
