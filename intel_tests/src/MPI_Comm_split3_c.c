...
int main(int argc, char *argv[])
{
    ...
    *attr, color,               /* For MPI_Comm_split              */
    ...
    sprintf(testname, "MPI_Comm_split3");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            ierr = MPI_Comm_split(comm, color, -37, &comm2);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
            ...
            else if (resultsplit != MPITEST_current_rank / 2) {
                sprintf(info_buf,
                        "MPI_Comm_rank returned %d, expected %d (comm_index %d)",
                        resultsplit, MPITEST_current_rank / 2, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            loop_cnt++;
