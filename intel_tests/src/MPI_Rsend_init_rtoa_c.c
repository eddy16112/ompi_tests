...
nonblocking I/O calls.
       ...
       Functional test for MPI persistent I/O  Requests
                      with MPI_Rsend_init():  Root send to all Others
...
int main(int argc, char *argv[])
{
    ...
    *values;                    /* Array of dataTemplates for verbose init           */
    ...
    sprintf(testname,
            "MPI_Rsend_init: Persistent root ready send to all others");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            /*
             * Set the print_node_flag: This is the flag that tells which
             * node gets to print. For INTRA-communition it is simply node 0
             * of the communicator. For INTER-communication it is node zero
             * of the left sub-communicator.
             */
            ...
            for (type_count = 0; type_count < MPITEST_num_datatypes();
                ...
                for (length_count = 0;
                    ...
                    if (inter_comm)
                        ntimes = 2;
                    /*
                     * grp_lup = 1  send from left  sub-communicator  to
 ...
 /*-------------------------------------------------
			    ...
			    -------------------------------------------------*/
                            ...
                            if ((!inter_comm
                                 ...
                                 * everybody in the sub-communicator
                                 * receives.
                                 */
                                ...
                                if (ierr != MPI_SUCCESS) {
                                    sprintf(info_buf,
                                            "Non-zero return code (%d) from MPI_Barrier before send",
                                            ierr);
                                    MPITEST_message(MPITEST_NONFATAL,
                                                    info_buf);
                                    MPI_Error_string(ierr, &info_buf[0],
                                ...
                                for (i = 0; i < send_to; i++) {
                                    ...
                                    ierr = MPI_Rsend_init(send_buffer,
                                                          length,
                                                          MPITEST_mpi_datatypes
                                                          [test_type], i,
                                                          32767, comm,
 ...
 /*------- Call MPI_Request_free ------*/
                                 ...
                                 * won't hit the Barrier in either the send
                                 * or receive loops above
                                 */
                                ierr = MPI_Barrier(barrier_comm);
                                if (ierr != MPI_SUCCESS) {
                                    sprintf(info_buf,
