...
All rank will call MPI_Type_indexed() and then MPI_Type_commit() to create
...
Depending on whether MPITEST_STATUS_CHK is #defined or not, the output status
object from MPI_Recv() may ot may not be verified. 

The input count to MPI_Type_indexed() is determined by each message length
in mpitest_cfg.h and the input oldtype to MPI_Type_indexed() is determined
...
/* Additional bufer size */
#define MPITEST_ADD_BUFSIZE   20

static int *blklens, *displs;
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Type_indexed_basic");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (type_count = 0; type_count < 1;
                ...
                ierr =
                    MPI_Type_extent(MPITEST_mpi_datatypes[test_type],
                                    &extent);
                if (ierr != MPI_SUCCESS) {
                    fail++;
                ...
                for (length_count = 0;
                     length_count < 1/*MPITEST_num_message_lengths()*/;
                     length_count++) {
                    ...
                    displs = (int *) calloc(length, sizeof(int));
                    if (!displs) {
                        sprintf(info_buf,
                                "Cannot allocate enough space for displs array");
                        MPITEST_message(MPITEST_FATAL, info_buf);
                    }
                    ...
                    if (!blklens) {
                        sprintf(info_buf,
                                "Cannot allocate enough space for blklens array");
                        MPITEST_message(MPITEST_FATAL, info_buf);
                    }
               ...
               //     ierr =
            //            MPI_Type_indexed(length, blklens, displs,
             ...
             //                            &newtype);
                  ...
                  //  ierr = MPI_Type_commit(&newtype);
                    ...
                    upper_matrix(length, &newtype);
                    cudaMalloc((void **)&buffer, sizeof(double)*length*length);
                    cudaMallocHost((void **)&buffer_host, sizeof(double)*length*length);
                    fill_upper_matrix(buffer_host, length);
                    cudaMemcpy(buffer, buffer_host, sizeof(double)*length*length, cudaMemcpyHostToDevice);
                  ...
                  //      MPITEST_init_buffer(test_type, length + 1, value,
                    //                        buffer);
                      ...
                      //      MPITEST_dataTemplate_init(&value,
                        ...
                        //        MPITEST_buffer_errors_ov(test_type, length,
                         //                                value, buffer);
                            ...
                            cudaMemcpy(buffer_host, buffer, sizeof(double)*length*length, cudaMemcpyDeviceToHost);
                            ...
                            if (error) {
                                if (ierr == MPI_SUCCESS)
                                    fail++;
                                sprintf(info_buf,
                                        "%d errors in buffer (%d, %d) len %d commsize %d commtype %d data_type %d root %d",
