 ...
 /*------------------------ MPI_Cart_get   -----------------------*/
                ...
                for (i = 0; i < ndims; i++) {
                    if (cart_dim_size[i] != dim_size[i]) {
                        sprintf(info_buf,
                                "Error in MPI_Cart_get for node = %d,  Acutal = %d, Expected = %d",
                                i, cart_dim_size[i], dim_size[i]);
                        MPITEST_message(MPITEST_FATAL, info_buf);
                    }
                }
                ...
                for (j = 0; j < ndims; j++) {
                    if (cart_periods[j] != periods[j]) {
                        sprintf(info_buf,
                                "Error in MPI_Cart_get period flags for node = %d,  Acutal = %d, Expected = %d",
                                j, cart_periods[j], periods[j]);
                        MPITEST_message(MPITEST_FATAL, info_buf);
                    }
                }
                ierr = MPI_Gather(cart_coords,
                                  ...
                                  MPI_INT,
                                  node_coords,
                                  MPITEST_MAXDIMS, MPI_INT, 0, comm_cart);
                ...
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
                            "Non-zero return code (%d) from MPI_Comm_size",
                            ierr);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_FATAL, info_buf);
                ...
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
                            "Non-zero return code (%d) from MPI_Comm_rank",
                            ierr);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_FATAL, info_buf);
                ...
                if (rank_cart == 0) {
                    ...
                    for (i = 0; i < cart_nump; i++) {
                        for (j = 0; j < ndims; j++) {
                            ...
                            if (node_coords[i][j] < 0
                                || node_coords[i][j] > dim_size[j]) {
                                sprintf(info_buf,
                                        "Error in node coordinates for rank = %d, Dimension = %d",
                                        i, j);
                                MPITEST_message(MPITEST_NONFATAL,
                                                info_buf);
                                sprintf(info_buf,
                                        "Coordinte value = %d,  Bound = %d",
                                        node_coords[i][j], dim_size[j]);
                                MPITEST_message(MPITEST_FATAL, info_buf);
                            }
                        }
