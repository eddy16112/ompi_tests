...
-----------------------------------------------------------------------------*/
/******************************************************************************
		  Test for MPI_Sendrecv_replace() to self

This tests MPI_Sendrecv_replace operation from a node to itself.
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname,
            "MPI_Sendrecv_replace_self: all Sendrecv_replace to SELF");
        ...
        if ((MPITEST_current_rank != MPI_UNDEFINED) &&
            ...
            /*------------------  Loop over Data Types  ---------------------*/
		        ...
		        -------------------------------------------------*/
                        ...
                        MPITEST_init_buffer(test_type, length, value,
                                            iobuffer);
                        ...
                        if (ierr != MPI_SUCCESS) {
                            ...
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            sprintf(info_buf,
                                    "Non-zero return code (%d) from MPI_Sendrecv",
                                    ierr);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                            MPI_Error_string(ierr, &info_buf[0], &size);
                            MPITEST_message(MPITEST_NONFATAL, info_buf);
                ...
                if (MPITEST_current_rank == 1) {
                    ierr = MPI_Send(iobuffer, 0, MPI_INT, 0, 1, comm);
