                    ...
                    if (MPITEST_current_rank == root) {
                        ...
                        if (ierr != MPI_SUCCESS) {
                            ...
                            if (ierr != MPI_SUCCESS) {
                                ...
                                fail++;
                                sprintf(info_buf,
                                        "MPI_Test_cancelled() return non-zero flag with a completed send request");
                                MPITEST_message(MPITEST_NONFATAL,
                                                info_buf);
                            }
                        }
                        ...
                        if (ierr != MPI_SUCCESS) {
                            ...
                            sprintf(info_buf,
                                    "Checking if the recv request has been cancelled");
                            MPITEST_message(MPITEST_INFO1, info_buf);
                            ...
                            if (ierr != MPI_SUCCESS) {
                                ...
                                fail++;
                                sprintf(info_buf,
                                        "MPI_Test_cancelled() return non-zero flag with a completed recv request");
                                MPITEST_message(MPITEST_NONFATAL,
                                                info_buf);
                            } else {
#ifdef MPITEST_STATUS_CHK
                                ...
                                sprintf(info_buf,
                                        "Verifying output status object from MPI_Recv()");
                                MPITEST_message(MPITEST_INFO1, info_buf);
...
#else
                                sprintf(info_buf,
                                        "output status object from MPI_Recv() *not* verified");
                                MPITEST_message(MPITEST_INFO1, info_buf);
#endif
