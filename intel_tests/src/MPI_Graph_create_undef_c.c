    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            /*-----------------------------  MPI_Graph_create  --------------------------*/
                ...
                ierr =
                    MPI_Reduce(&undefined, &sum, 1, MPI_INT, MPI_SUM, 0,
                               comm);
                if (ierr != MPI_SUCCESS) {
                    sprintf(info_buf,
                            "Non-zero return code (%d) from MPI_Reduce",
                            ierr);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_FATAL, info_buf);
                ...
                if (rank_comm == 0) {
                    if (sum != 1) {
                        sprintf(info_buf,
                                "Error:  Number of undefined nodes not 1 sum = %d",
                                sum);
                        MPITEST_message(MPITEST_FATAL, info_buf);
                        fail++;
                    }
                    sprintf(info_buf,
                            "Reduce results:  sum = %d    graph_nump = %d comm_size = %d",
                            sum, graph_nump, test_nump);
