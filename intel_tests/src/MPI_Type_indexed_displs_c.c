...
(in mpitest_cfg.h) using MPI_Type_indexed() using length as displacement value.
...
MPI Calls dependencies for this test:
  ...
  MPI_Type_indexed(), MPI_Type_struct(), MPI_Type_commit(),
...
int main(int argc, char *argv[])
{
    ...
    signed char *buffer;        /* message buffer                                       */
#if defined (CUDA_TEST)
    signed char *buffer_gpu;
#endif
    ...
    MPI_Datatype oldtype, newtype, *types;

    int *blklens, num_types, *type_sizes, *blklens1, *displs1, basic_types;
    ...
    sprintf(testname, "MPI_Type_indexed_displs");
    ...
    printf("rank %d, process id %d, size int, %d, size long %d, size double %d\n", wei_rank, getpid(), sizeof(int), sizeof(long), sizeof(double));
    sleep(15);
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            MPITEST_get_buffer(MPITEST_byte,
                               ...
                               (void *) &buffer);
#if defined (CUDA_TEST)
            MPITEST_get_gpu_buffer(MPITEST_byte,
                                   max_length + MPITEST_ADD_BUFSIZE,
                                   (void *) &buffer_gpu);   
#endif                
            ...
            for (length_count = 0;
                 ...
                 length_count++) {
                long long possible_size; /* Need 64 bits to handle 64k * 64k */
...
/*********** This code was added by Debbie Swider 9/17/97 ****************/
                ...
                /* Because ( (length)*(length)*(extent) - (length)*(extent))
                ...
                possible_size = (long long)length * (long long)(length) * (long long)extent;
                possible_size = possible_size - (length * extent);
                if (possible_size > (max_length + MPITEST_ADD_BUFSIZE)) {
...
/****************************************************************************/
                ...
                displs1 = (int *) calloc(length, sizeof(int));
                if (!displs1) {
                    sprintf(info_buf,
                            "Cannot allocate enought space for displs1 array");
                    MPITEST_message(MPITEST_FATAL, info_buf);
                }
                ...
                if (!blklens1) {
                    sprintf(info_buf,
                            "Cannot allocate enought space for blklens1 array");
                    MPITEST_message(MPITEST_FATAL, info_buf);
                }
                ...
                ierr =
                    MPI_Type_indexed(length, blklens1, displs1, oldtype,
                                     &newtype);
                if (ierr != MPI_SUCCESS) {
                    fail++;
                    sprintf(info_buf, "MPI_Type_indexed() returned %d",
                            ierr);
                    MPITEST_message(MPITEST_NONFATAL, info_buf);
                    MPI_Error_string(ierr, &info_buf[0], &size);
                    MPITEST_message(MPITEST_FATAL, info_buf);
                ...
                for (root = 0; root < test_nump; root++) {
                    ...
                    for (i = 0; i < max_length + MPITEST_ADD_BUFSIZE; i++) {
                        buffer[i] = -1 * (MPITEST_current_rank % 128);
                    }
...
#if defined (CUDA_TEST)
                    cudaMemcpy(buffer_gpu, buffer, max_length + MPITEST_ADD_BUFSIZE, cudaMemcpyHostToDevice);
#endif
                    ...
                    if (MPITEST_current_rank == root) {
                        sprintf(info_buf,
                                "Sending from source: %d to %d, tag: %d",
                                root, dest, tag);
                        MPITEST_message(MPITEST_INFO1, info_buf);

#ifdef MPITEST_DISP_BUF
                        sprintf(info_buf,
                                "Displaying Sender's data buffer:");
                        MPITEST_message(MPITEST_INFO2, info_buf);
                        ...
                        ierr = MPI_Send(buffer_gpu, 1, newtype,
                                        dest, tag, comm);
#else 
                        ...
                        sprintf(info_buf,
                                "Receiving from source: %d to %d, tag: %d",
                                root, dest, tag);
                        MPITEST_message(MPITEST_INFO1, info_buf);

#if defined (CUDA_TEST_G2G)
                        ierr = MPI_Recv(buffer_gpu, 1, newtype,
...
#if defined (CUDA_TEST)
                        cudaMemcpy(buffer, buffer_gpu, max_length + MPITEST_ADD_BUFSIZE, cudaMemcpyDeviceToHost);
#endif
                        ...
                        if (buffer[length * length * extent] !=
                            ((signed char) -1 * (MPITEST_current_rank % 128))) {
                            fail++;
                            error2++;
#ifdef MPITEST_1ST_ERR
                            if (error2 <= 1) {
