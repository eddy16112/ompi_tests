...
-----------------------------------------------------------------------------*/
/******************************************************************************
		  Test for MPI_Type_get_name()

This test calls MPI_Type_get_name() on each rank and prints it.
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Type_get_name");
    ...
    if (MPITEST_me == 0) {
        ...
        MPITEST_message(MPITEST_VERIFY,
                        "Please verify the following datatype names");
    }
    ...
    ierr = MPI_Type_get_name(MPI_INT, name, &len);
    for (i = 0; i < MPITEST_nump; i++) {
        if (i == MPITEST_me) {
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Type_get_name",
                        ierr);
                ...
                if ((len > MPI_MAX_OBJECT_NAME)
                    ...
                    sprintf(info_buf,
                            "Returned string length=%d (OK), datatype name:",
                            len);
    ...
    ++loop_cnt;
    ierr = MPI_Type_get_name(MPI_FLOAT, name, &len);
    for (i = 0; i < MPITEST_nump; i++) {
        if (i == MPITEST_me) {
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
