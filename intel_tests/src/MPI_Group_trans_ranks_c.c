...
int main(int argc, char *argv[])
{
    ...
    *ranks2,                    /* Array of ranks in group         */
     ...
     commsize, result, size, i;

    MPI_Group group, group2, groupself;
        ...
        if (MPITEST_current_rank != MPI_UNDEFINED) {
            ...
            if ((ranks1 == 0) || (ranks2 == 0))
                MPITEST_message(MPITEST_FATAL,
                                "Not enough memory for test buffers");
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_size #2 (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_incl #1",
                        ierr);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_FATAL, info_buf);
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_size (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (result != commsize) {
                sprintf(info_buf,
                        "MPI_Group_size returned %d, expected %d (comm_index %d)",
                        result, commsize, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            loop_cnt++;
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_rank (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (result != commsize - 1 - MPITEST_current_rank) {
                sprintf(info_buf,
                        "MPI_Group_rank returned %d, expected %d (comm_index %d)",
                        result, MPITEST_current_rank, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            loop_cnt++;
            ...
            for (i = 0; i < commsize; i++)
                ranks1[i] = i;
            ierr =
                MPI_Group_translate_ranks(group, commsize, ranks1, group2,
                                          ranks2);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_translate_ranks #1 (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (verify != 0) {
                sprintf(info_buf,
                        "%d errors detected in translated ranks #1 (comm_index %d)",
                        verify, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            ierr =
                MPI_Group_translate_ranks(group2, commsize, ranks1, group,
                                          ranks2);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_translate_ranks #2 (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (verify != 0) {
                sprintf(info_buf,
                        "%d errors detected in translated ranks #2 (comm_index %d)",
                        verify, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            ierr =
                MPI_Group_translate_ranks(group2, 1, ranks1, group,
                                          ranks2);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_translate_ranks #3 (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (ranks2[0] != commsize - 1 - MPITEST_current_rank) {
                sprintf(info_buf,
                        "1 error detected in translated ranks #3 %d, expected %d (comm_index %d)",
                        MPITEST_current_rank,
                        commsize - 1 - MPITEST_current_rank, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Comm_group(self) (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            ranks1[0] = MPITEST_current_rank;
            ierr =
                MPI_Group_translate_ranks(group, 1, ranks1, groupself,
                                          ranks2);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_translate_ranks #4 (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (ranks2[0] != 0) {
                sprintf(info_buf,
                        "1 error detected in translated ranks #4 %d, expected %d (comm_index %d)",
                        MPITEST_current_rank,
                        commsize - 1 - MPITEST_current_rank, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            ...
            ranks1[0] = 0;
            ierr =
                MPI_Group_translate_ranks(groupself, 1, ranks1, group2,
                                          ranks2);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_translate_ranks #5 (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (ranks2[0] != commsize - 1 - MPITEST_current_rank) {
                sprintf(info_buf,
                        "1 error detected in translated ranks #5 %d, expected %d (comm_index %d)",
                        MPITEST_current_rank,
                        commsize - 1 - MPITEST_current_rank, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_translate_ranks #6 (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (verify != 0) {
                sprintf(info_buf,
                        "%d errors detected in translated ranks #6 (comm_index %d)",
                        verify, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            for (i = 0; i < commsize; i++)
                ranks1[i] = i;
            ierr =
                MPI_Group_translate_ranks(group, commsize, ranks1,
                                          groupself, ranks2);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_translate_ranks #7 (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (verify != 0) {
                sprintf(info_buf,
                        "%d errors detected in translated ranks #7 (comm_index %d)",
                        verify, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            ierr =
                MPI_Group_translate_ranks(group, commsize, ranks1, group,
                                          ranks2);
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_translate_ranks #7 (comm_index %d)",
            ...
            if (verify != 0) {
                sprintf(info_buf,
                        "%d errors detected in translated ranks #8 (comm_index %d)",
                        verify, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                fail++;
            }
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_free #1 (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_free #2 (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
            ...
            if (ierr != MPI_SUCCESS) {
                sprintf(info_buf,
                        "Non-zero return code (%d) from MPI_Group_free #3 (comm_index %d)",
                        ierr, comm_index);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
                MPI_Error_string(ierr, &info_buf[0], &size);
                MPITEST_message(MPITEST_NONFATAL, info_buf);
