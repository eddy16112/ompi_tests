...
(in mpitest_cfg.h) using MPI_Type_hvector() using MPITEST_STRIDE + extent as
stride value. The root's rank will then send a message using the new type to the
...
/* Stride (in bytes) between each block */
#define MPITEST_STRIDE        5
...
int main(int argc, char *argv[])
{
    ...
    sprintf(testname, "MPI_Type_hvector_stride");
    ...
    for (comm_count = 0; comm_count < MPITEST_num_comm_sizes();
        ...
        if (comm != MPI_COMM_NULL) {
            ...
            for (length_count = 0;
                ...
                length = byte_length / (extent + MPITEST_STRIDE);
                ...
                ierr =
                    MPI_Type_hvector(length, 1, extent + MPITEST_STRIDE,
                                     oldtype, &newtype);
                if (ierr != MPI_SUCCESS) {
                    fail++;
                ...
                for (root = 0; root < test_nump; root++) {
                    ...
                    if (MPITEST_current_rank == root) {
                        ...
                        if (buffer[length * (extent + MPITEST_STRIDE)] !=
                            ...
                            if (error2 <= 1) {
                                ...
                                sprintf(info_buf,
                                        ...
                                        " Actual buffer[%d]: %d",
                                        length * (MPITEST_STRIDE + extent),
                                        -1 * (MPITEST_current_rank % 128),
                                        length * (MPITEST_STRIDE + extent),
                                        buffer[length *
                                               (MPITEST_STRIDE + extent)]);
                                MPITEST_message(MPITEST_NONFATAL,
                                                info_buf);
#ifdef MPITEST_1ST_ERR
                            }
